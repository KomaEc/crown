...
- [ ] Is the handling of nested pointer correct? `p = q` should introduce more constraints when `p` is nested!
...
- [x] The analysis can be improved. Specifically, right now the inferencer treats newly defined locals as unconstrained (more concretely, in `let mut a = ...; a = ...`, in the second statement, `a` is redefined, and hence a new local will be defined in the SSA; this newly defined local is unconstrained). Since they are unconstrained, a user local variable may have different type signatures in different context. My intuition is that, if the type signature changes, we can replace assignment by a new binding, which is more flexible. However, this is too coarse: a local can only be thinner not fatter. The analysis will be more precise if we generate an additional constraint when defining a new SSA local stating that `new ≤ old`.
- [ ] The logic of handling boundary constraints and return place ssa indices should be moved to the core ssa rename procedure. Also, we should add additional constraints for each method stating that all return places should have the same fat/thin signatures.
- [ ] Cleverer modelling of extern calls. Currently libc external calls introduce almost no constraint except for `calloc`, `realloc`. The reason is that, strict handlings may affect fat/thin signatures of field definitions. I believe those signatures are the most important signatures, and they must originate solely from `calloc` and `realloc`. This leads to less precise solutions (see functions `buffer_new_with_copy`, `buffer_compact`, .etc). A tentative approach to avoid this: handle extern calls separately when solving constraints. More concretely, if an input of an extern call is non-global, then we enforce it with a stronger assumption (this corresponds to calling `as_mut_ptr()` in the fix).
