...
- [ ] Cleverer modelling of extern calls. Currently libc external calls except `calloc`, `realloc` introduce few constraints. This leads to less precise solution (see functions `buffer_new_with_copy`, `buffer_compact`, .etc). A tentative solution: handle extern calls separately when solving constraints. More concretely, if an input of an extern call is non-global, then we enforce it with a stronger assumption (this corresponds to calling `as_mut_ptr()` in the fix).
