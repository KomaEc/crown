...
unsafe extern "C" fn BrotliBitReaderRestoreState(
    mut to: Option<&mut crate::src::dec::bit_reader::BrotliBitReader>,
...
#[inline(always)]
unsafe extern "C" fn BrotliFillBitWindow16(mut br: Option<&mut crate::src::dec::bit_reader::BrotliBitReader>) {
    BrotliFillBitWindow(br.as_deref_mut(), 17 as libc::c_int as uint32_t);
}
#[inline(always)]
unsafe extern "C" fn BrotliPullByte(mut br: Option<&mut crate::src::dec::bit_reader::BrotliBitReader>) -> libc::c_int {
...
#[inline(always)]
unsafe extern "C" fn BrotliGet16BitsUnmasked(mut br: Option<&mut crate::src::dec::bit_reader::BrotliBitReader>) -> uint32_t {
...
#[inline(always)]
unsafe extern "C" fn BrotliDropBits(mut br: Option<&mut crate::src::dec::bit_reader::BrotliBitReader>, mut n_bits: uint32_t) {
        ...
        match  (*h).substate_huffman as libc::c_uint {
            0 => {
                ...
                if (*h).sub_loop_counter != 1 as libc::c_int as libc::c_uint {
                    ...
                    memset(
                        &raw mut *(*h).code_length_histo.as_mut_ptr()
                    ...
                    memset(
                        &raw mut *(*h).code_length_code_lengths.as_mut_ptr()
...
unsafe extern "C" fn InverseMoveToFrontTransform(
    ...
    let mut mtf: *mut uint32_t = &raw mut *(*state).mtf.as_mut_ptr()
...
unsafe extern "C" fn DecodeBlockTypeAndLength(
    ...
    let mut type_tree: *const HuffmanCode = &raw mut *(*s).block_type_trees
        .offset((tree_type * 632 as libc::c_int) as isize) as *mut HuffmanCode;
    let mut len_tree: *const HuffmanCode = &raw mut *(*s).block_len_trees
        .offset((tree_type * 396 as libc::c_int) as isize) as *mut HuffmanCode;
    let mut br: *mut crate::src::dec::bit_reader::BrotliBitReader = core::ptr::addr_of_mut!((*s).br);
    let mut ringbuffer: *mut uint32_t = &raw mut *(*s).block_type_rb.as_mut_ptr()
    ...
    if safe == 0 {
        ...
        if SafeReadBlockLength(
            s,
            &raw mut *(*s).block_length.as_mut_ptr().offset(tree_type as isize),
            len_tree,
            br,
        ) == 0
        {
    ...
    while i < (*s.as_deref().unwrap()).num_block_types[0 as libc::c_int as usize] as libc::c_ulong {
        ...
        while j < ((1 as libc::c_uint) << 6 as libc::c_int) as libc::c_ulong {
            if 4 as libc::c_int & 1 as libc::c_int != 0 as libc::c_int {
                ...
                j = j.wrapping_add(1);
                error
                    |= *((*s.as_deref().unwrap()).context_map).offset(offset.wrapping_add(fresh36) as isize)
            ...
            if 4 as libc::c_int & 2 as libc::c_int != 0 as libc::c_int {
                ...
                j = j.wrapping_add(1);
                error
                    |= *((*s.as_deref().unwrap()).context_map).offset(offset.wrapping_add(fresh37) as isize)
                        as libc::c_ulong ^ sample;
                let fresh38 = j;
                j = j.wrapping_add(1);
                error
                    |= *((*s.as_deref().unwrap()).context_map).offset(offset.wrapping_add(fresh38) as isize)
                        as libc::c_ulong ^ sample;
            }
    ...
    loop {
        ...
        match  (*s).substate_uncompressed as libc::c_uint {
            0 => {
                ...
                BrotliCopyBytes(
                    &raw mut *(*s).ringbuffer.offset((*s).pos as isize),
                    ...
                    if (*s).distance_code > (*s).max_distance {
                        ...
                        if i >= 4 as libc::c_int && i <= 24 as libc::c_int {
                                ...
                                < (*transforms).num_transforms as libc::c_int
                            {
                                ...
                                {
                                    memcpy(
                                        &raw mut *(*s).ringbuffer.offset(pos as isize) as *mut uint8_t
                                            as *mut libc::c_void,
                                    ...
                                    len= crate::src::common::transform::BrotliTransformDictionaryWord(
                                        &raw mut *(*s).ringbuffer.offset(pos as isize),
                                        word,
                                        len,
                                        transforms,
                                        transform_idx,
                        ...
                        let mut src_start = pos - (*s).distance_code
                            & (*s).ringbuffer_mask;
                        let mut copy_dst: *mut uint8_t = &raw mut *(*s).ringbuffer
                            .offset(pos as isize) as *mut uint8_t;
                        let mut copy_src: *mut uint8_t = &raw mut *(*s).ringbuffer
...
pub unsafe extern "C" fn BrotliDecoderDecompressStream(
    ...
    if (*s).buffer_length == 0 as libc::c_int as libc::c_uint {
        ...
        result= BROTLI_DECODER_NEEDS_MORE_INPUT;
        (*br).next_in= &raw mut *(*s).buffer.u8_0.as_mut_ptr()
            .offset(0 as libc::c_int as isize) as *mut uint8_t;
    }
    loop {
        if result as libc::c_int != BROTLI_DECODER_SUCCESS as libc::c_int {
            ...
            match current_block_175 {
                ...
                10798328910445268477 => {
                    ...
                    (*s).loop_counter= 0 as libc::c_int;
                    (*h).sub_loop_counter= 0 as libc::c_int as uint32_t;
                    (*h).symbol_lists= &raw mut *(*h).symbols_lists_array.as_mut_ptr()
                ...
                3212124759251176530 => {
                    if (*s).loop_counter >= 3 as libc::c_int {
                        ...
                        result= DecodeVarLenUint8(
                            s,
                            br,
                            &raw mut *(*s).num_block_types.as_mut_ptr()
            ...
            match current_block_175 {
                13253659531982233645 => {
                    ...
                    result= ReadHuffmanCode(
                        alphabet_size,
                        alphabet_size,
                        &raw mut *(*s).block_type_trees.offset(tree_offset as isize),
                        None,
                        s,
                    );
                    if result as libc::c_int != BROTLI_DECODER_SUCCESS as libc::c_int {
            ...
            match current_block_175 {
                10241167629170301496 => {
                    ...
                    result= ReadHuffmanCode(
                        alphabet_size_0,
                        alphabet_size_0,
                        &raw mut *(*s).block_len_trees.offset(tree_offset_0 as isize),
                        None,
                        s,
                    );
                    if result as libc::c_int != BROTLI_DECODER_SUCCESS as libc::c_int {
            ...
            match current_block_175 {
                ...
                13256895345714485905 => {
                    ...
                    if SafeReadBlockLength(
                        s,
                        &raw mut *(*s).block_length.as_mut_ptr()
                            .offset((*s).loop_counter as isize),
                        &raw mut *(*s).block_len_trees.offset(tree_offset_1 as isize),
                        br,
                    ) == 0
                    {
                        result= BROTLI_DECODER_NEEDS_MORE_INPUT;
