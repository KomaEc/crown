...
struct ErasedByPreprocessor62;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor63;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor64;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor65;
#[derive(Copy, Clone)]
#[repr(C)]
    ...
    if n < 13 as libc::c_int as libc::c_ulong {
        ...
        while i < n {
            ...
            while comparator
                ...
                )(core::ptr::addr_of!(tmp), &raw mut *items.offset(j as isize)) != 0
            {
            ...
            while i_0 < n {
                ...
                while j_0 >= gap
                    && comparator
                        .expect(
                            "non-null function pointer",
                        )(core::ptr::addr_of!(tmp_0), &raw mut *items.offset(j_0.wrapping_sub(gap) as isize))
                        != 0
                {
                    *items
                        .offset(
...
unsafe extern "C" fn GetBlockLengthPrefixCode(
    ...
    *n_extra.as_deref_mut().unwrap()= _kBrotliPrefixCodeRanges[*code.as_deref().unwrap() as usize].nbits as uint32_t;
    *extra.as_deref_mut().unwrap()= len
        .wrapping_sub(_kBrotliPrefixCodeRanges[*code.as_deref().unwrap() as usize].offset as libc::c_uint);
}
...
unsafe extern "C" fn EncodeContextMap(
    ...
    while i < num_rle_symbols {
        histogram[(*rle_symbols.offset(i as isize) & kSymbolMask)
            as usize]= histogram[(*rle_symbols.offset(i as isize) & kSymbolMask)
            as usize]
            .wrapping_add(1);
        i= i.wrapping_add(1);
    }
...
unsafe extern "C" fn BuildAndStoreBlockSplitCode(
    ...
    if num_types > 1 as libc::c_int as libc::c_ulong {
        BuildAndStoreHuffmanTree(
            &raw mut *type_histo.as_mut_ptr().offset(0 as libc::c_int as isize),
            num_types.wrapping_add(2 as libc::c_int as libc::c_ulong),
            num_types.wrapping_add(2 as libc::c_int as libc::c_ulong),
            tree,
            &raw mut *(*code).type_depths.as_mut_ptr().offset(0 as libc::c_int as isize),
            &raw mut *(*code).type_bits.as_mut_ptr().offset(0 as libc::c_int as isize),
            storage_ix,
            storage,
        );
        BuildAndStoreHuffmanTree(
            &raw mut *length_histo.as_mut_ptr().offset(0 as libc::c_int as isize),
            26 as libc::c_int as size_t,
            26 as libc::c_int as size_t,
            tree,
            &raw mut *(*code).length_depths.as_mut_ptr().offset(0 as libc::c_int as isize),
            &raw mut *(*code).length_bits.as_mut_ptr().offset(0 as libc::c_int as isize),
            storage_ix,
            storage,
        );
        StoreBlockSwitch(
...
unsafe extern "C" fn InitBlockEncoder(
    mut self_0: Option<&mut BlockEncoder>,
    ...
    (*self_0.as_deref_mut().unwrap()).histogram_length_= histogram_length;
    (*self_0.as_deref_mut().unwrap()).num_block_types_= num_block_types;
    (*self_0.as_deref_mut().unwrap()).block_types_= block_types;
    (*self_0.as_deref_mut().unwrap()).block_lengths_= block_lengths;
    (*self_0.as_deref_mut().unwrap()).num_blocks_= num_blocks;
    InitBlockTypeCodeCalculator(Some(&mut (*self_0.as_deref_mut().unwrap()).block_split_code_.type_code_calculator));
    (*self_0.as_deref_mut().unwrap()).block_ix_= 0 as libc::c_int as size_t;
    (*self_0.as_deref_mut().unwrap()).block_len_= (if num_blocks == 0 as libc::c_int as libc::c_ulong {
        0 as libc::c_int as libc::c_uint
    } else {
        *block_lengths.offset(0 as libc::c_int as isize)
    }) as size_t;
    (*self_0.as_deref_mut().unwrap()).entropy_ix_= 0 as libc::c_int as size_t;
    (*self_0.as_deref_mut().unwrap()).depths_= 0 as *mut uint8_t;
    (*self_0.as_deref_mut().unwrap()).bits_= 0 as *mut uint16_t;
}
...
unsafe extern "C" fn BuildAndStoreEntropyCodesDistance(
    ...
    while i < histograms_size {
        ...
        BuildAndStoreHuffmanTree(
            ...
            alphabet_size,
            tree,
            &raw mut *(*self_0).depths_.offset(ix as isize),
            &raw mut *(*self_0).bits_.offset(ix as isize),
            storage_ix,
            storage,
        );
        i= i.wrapping_add(1);
...
pub unsafe extern "C" fn BrotliStoreUncompressedMetaBlock(
        ...
        > mask.wrapping_add(1 as libc::c_int as libc::c_ulong)
    {
        ...
        memcpy(
            &raw mut *storage.offset(((*storage_ix) >> 3 as libc::c_int) as isize)
    ...
    memcpy(
        &raw mut *storage.offset(((*storage_ix) >> 3 as libc::c_int) as isize) as *mut uint8_t
            as *mut libc::c_void,
        &*input.offset(masked_pos as isize) as *const uint8_t as *const libc::c_void,
        len,
    );
