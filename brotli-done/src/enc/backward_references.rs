...
pub struct BrotliEncoderDictionary {
    pub words: *const crate::src::common::dictionary::BrotliDictionary,
...
pub struct H40 {
    ...
    pub max_hops: size_t,
    pub extra: *mut libc::c_void,
    pub common: *mut /* owning */ HasherCommon,
}
impl Default for H40 {fn default() -> Self {Self {
free_slot_idx: [Default::default(); 1],
max_hops: Default::default(),
extra: std::ptr::null_mut(),
common: std::ptr::null_mut(),
}}}
impl H40 {pub fn take(&mut self) -> Self {core::mem::take(self)}}
...
impl Default for H41 {fn default() -> Self {Self {
free_slot_idx: [Default::default(); 1],
max_hops: Default::default(),
extra: std::ptr::null_mut(),
common: std::ptr::null_mut(),
}}}
impl H41 {pub fn take(&mut self) -> Self {core::mem::take(self)}}
...
impl Default for H42 {fn default() -> Self {Self {
free_slot_idx: [Default::default(); 512],
max_hops: Default::default(),
extra: std::ptr::null_mut(),
common: std::ptr::null_mut(),
}}}
impl H42 {pub fn take(&mut self) -> Self {core::mem::take(self)}}
...
unsafe extern "C" fn InitCommand(
    ...
    (*self_0.as_deref_mut().unwrap()).copy_len_= (copylen | (delta << 25 as libc::c_int) as libc::c_ulong)
        as uint32_t;
    PrefixEncodeCopyDistance(
        distance_code,
        (*dist).num_direct_distance_codes as size_t,
        (*dist).distance_postfix_bits as size_t,
        core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).dist_prefix_),
        core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).dist_extra_),
    );
    GetLengthCode(
        insertlen,
        (copylen as libc::c_int + copylen_code_delta) as size_t,
        if (*self_0.as_deref().unwrap()).dist_prefix_ as libc::c_int & 0x3ff as libc::c_int
            == 0 as libc::c_int
        {
            1 as libc::c_int
        } else {
...
unsafe extern "C" fn TestStaticDictionaryItem(
    ...
    mut out: Option<&mut HasherSearchResult>,
) -> libc::c_int {
    ...
    if score < (*out.as_deref().unwrap()).score {
        return 0 as libc::c_int;
    }
    (*out.as_deref_mut().unwrap()).len= matchlen;
    (*out.as_deref_mut().unwrap()).len_code_delta= len as libc::c_int - matchlen as libc::c_int;
    (*out.as_deref_mut().unwrap()).distance= backward;
    (*out.as_deref_mut().unwrap()).score= score;
    return 1 as libc::c_int;
}
#[inline(always)]
unsafe extern "C" fn SearchInStaticDictionary(
    mut dictionary: *const BrotliEncoderDictionary,
    mut common: Option<&mut HasherCommon>,
    mut data: *const uint8_t,
    mut max_length: size_t,
    mut max_backward: size_t,
    mut max_distance: size_t,
    ...
    let mut i: size_t = 0;
    if (*common.as_deref().unwrap()).dict_num_matches < (*common.as_deref().unwrap()).dict_num_lookups >> 7 as libc::c_int {
        return;
    }
    ...
    while i
        < (if shallow != 0 { 1 as libc::c_uint } else { 2 as libc::c_uint })
            as libc::c_ulong
    {
        (*common.as_deref_mut().unwrap()).dict_num_lookups= (*common.as_deref().unwrap()).dict_num_lookups.wrapping_add(1);
            ...
            != 0 as libc::c_int
        {
            ...
            if item_matches != 0 {
                (*common.as_deref_mut().unwrap()).dict_num_matches= (*common.as_deref().unwrap()).dict_num_matches.wrapping_add(1);
            }
        }
        i= i.wrapping_add(1);
...
unsafe extern "C" fn StoreRangeH2(
    ...
    i= ix_start;
    while i < ix_end {
        StoreH2(self_0.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), data, mask, i);
        i= i.wrapping_add(1);
    }
}
#[inline(always)]
...
unsafe extern "C" fn FindLongestMatchH2(
    ...
    let best_len_in = (*out.as_deref().unwrap()).len;
    ...
    let mut min_score = (*out.as_deref().unwrap()).score;
    let mut best_score = (*out.as_deref().unwrap()).score;
    ...
    (*out.as_deref_mut().unwrap()).len_code_delta= 0 as libc::c_int;
    if prev_ix < cur_ix {
            ...
            == *data.offset(prev_ix.wrapping_add(best_len) as isize) as libc::c_int
        {
            ...
            if len >= 4 as libc::c_int as libc::c_ulong {
                ...
                if best_score < score {
                    (*out.as_deref_mut().unwrap()).len= len;
                    (*out.as_deref_mut().unwrap()).distance= cached_backward;
                    (*out.as_deref_mut().unwrap()).score= score;
                    if (1 as libc::c_int) << 0 as libc::c_int == 1 as libc::c_int {
        ...
        *buckets.offset(key as isize) = cur_ix as uint32_t;
        ...
        if len_0 >= 4 as libc::c_int as libc::c_ulong {
            ...
            if best_score < score_0 {
                (*out.as_deref_mut().unwrap()).len= len_0;
                (*out.as_deref_mut().unwrap()).distance= backward;
                (*out.as_deref_mut().unwrap()).score= score_0;
                return;
            }
        }
    } else {
                    ...
                    || backward_0 > max_backward) as libc::c_int as libc::c_long != 0)
                {
                    ...
                    if len_1 >= 4 as libc::c_int as libc::c_ulong {
                        ...
                        if best_score < score_1 {
                            best_len = len_1;
                            (*out.as_deref_mut().unwrap()).len = len_1;
                            compare_char = *data
                                .offset(cur_ix_masked.wrapping_add(len_1) as isize)
                                as libc::c_int;
                            best_score = score_1;
                            (*out.as_deref_mut().unwrap()).score = score_1;
                            (*out.as_deref_mut().unwrap()).distance = backward_0;
                        }
                    }
                }
            }
    ...
    if 1 as libc::c_int != 0 && min_score == (*out.as_deref().unwrap()).score {
        SearchInStaticDictionary(
            dictionary,
            (*self_0).common.as_mut(),
            &*data.offset(cur_ix_masked as isize),
            max_length,
            dictionary_distance,
            max_distance,
...
unsafe extern "C" fn StoreRangeH3(
    ...
    i= ix_start;
    while i < ix_end {
        StoreH3(self_0.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), data, mask, i);
        i= i.wrapping_add(1);
    }
}
#[inline(always)]
...
unsafe extern "C" fn FindLongestMatchH3(
    ...
    if 0 as libc::c_int != 0 && min_score == (*out.as_deref().unwrap()).score {
        SearchInStaticDictionary(
            dictionary,
            (*self_0).common.as_mut(),
            &*data.offset(cur_ix_masked as isize),
...
unsafe extern "C" fn StoreRangeH4(
    ...
    i= ix_start;
    while i < ix_end {
        StoreH4(self_0.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), data, mask, i);
        i= i.wrapping_add(1);
    }
}
#[inline(always)]
...
unsafe extern "C" fn StoreRangeH5(
    ...
    i= ix_start;
    while i < ix_end {
        StoreH5(self_0.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), data, mask, i);
        i= i.wrapping_add(1);
    }
}
#[inline(always)]
unsafe extern "C" fn PrepareDistanceCacheH5(
    mut self_0: *mut H5,
    mut distance_cache: *mut /* owning */ libc::c_int,
) {
...
unsafe extern "C" fn FindLongestMatchH5(
    ...
    let mut min_score = (*out.as_deref().unwrap()).score;
    let mut best_score = (*out.as_deref().unwrap()).score;
    let mut best_len = (*out.as_deref().unwrap()).len;
    let mut i: size_t = 0;
    (*out.as_deref_mut().unwrap()).len= 0 as libc::c_int as size_t;
    (*out.as_deref_mut().unwrap()).len_code_delta= 0 as libc::c_int;
    i= 0 as libc::c_int as size_t;
                            ...
                            && i < 2 as libc::c_int as libc::c_ulong
                    {
                        ...
                        if best_score < score {
                            ...
                            if best_score < score {
                                best_score= score;
                                best_len= len;
                                (*out.as_deref_mut().unwrap()).len= best_len;
                                (*out.as_deref_mut().unwrap()).distance= backward;
                                (*out.as_deref_mut().unwrap()).score= best_score;
                            }
                        }
                    }
                }
    ...
    let mut bucket: *mut uint32_t = &raw mut *buckets
    ...
    *num.offset(key as isize) = (*num.offset(key as isize)).wrapping_add(1);
    if min_score == (*out.as_deref().unwrap()).score {
        SearchInStaticDictionary(
            dictionary,
            (*self_0).common_.as_mut(),
            &*data.offset(cur_ix_masked as isize),
            max_length,
            dictionary_distance,
            max_distance,
...
unsafe extern "C" fn StoreRangeH6(
    ...
    i= ix_start;
    while i < ix_end {
        StoreH6(self_0.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), data, mask, i);
        i= i.wrapping_add(1);
    }
}
#[inline(always)]
...
unsafe extern "C" fn StoreH40(
    ...
    let fresh7 = (*self_0).free_slot_idx[bank as usize];(*self_0).free_slot_idx[bank as usize]= (*self_0).free_slot_idx[bank as usize].wrapping_add(1);
...
unsafe extern "C" fn StoreRangeH40(
    ...
    i= ix_start;
    while i < ix_end {
        StoreH40(self_0.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), data, mask, i);
        i= i.wrapping_add(1);
    }
}
#[inline(always)]
...
unsafe extern "C" fn FindLongestMatchH40(
    ...
    let mut addr = AddrH40((*self_0.as_deref().unwrap()).extra);
    let mut head = HeadH40((*self_0.as_deref().unwrap()).extra);
    let mut tiny_hashes = TinyHashH40((*self_0.as_deref().unwrap()).extra);
    let mut banks = BanksH40((*self_0.as_deref().unwrap()).extra);
    let cur_ix_masked = cur_ix & ring_buffer_mask;
    let mut min_score = (*out.as_deref().unwrap()).score;
    let mut best_score = (*out.as_deref().unwrap()).score;
    let mut best_len = (*out.as_deref().unwrap()).len;
    ...
    let mut hops = (*self_0.as_deref().unwrap()).max_hops;
    ...
    StoreH40(self_0.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), data, ring_buffer_mask, cur_ix);
    if (*out.as_deref().unwrap()).score == min_score {
        SearchInStaticDictionary(
            dictionary,
            (*self_0.as_deref_mut().unwrap()).common.as_mut(),
            &*data.offset(cur_ix_masked as isize),
            max_length,
            dictionary_distance,
            max_distance,
...
unsafe extern "C" fn StoreH41(
    ...
    let bank = key & (1 as libc::c_int - 1 as libc::c_int) as libc::c_ulong;
    let fresh10 = (*self_0).free_slot_idx[bank as usize];(*self_0).free_slot_idx[bank as usize]= (*self_0).free_slot_idx[bank as usize].wrapping_add(1);
...
unsafe extern "C" fn StoreRangeH41(
    ...
    i= ix_start;
    while i < ix_end {
        StoreH41(self_0.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), data, mask, i);
        i= i.wrapping_add(1);
    }
}
#[inline(always)]
...
unsafe extern "C" fn FindLongestMatchH41(
    ...
    let mut addr = AddrH41((*self_0.as_deref().unwrap()).extra);
    let mut head = HeadH41((*self_0.as_deref().unwrap()).extra);
    let mut tiny_hashes = TinyHashH41((*self_0.as_deref().unwrap()).extra);
    let mut banks = BanksH41((*self_0.as_deref().unwrap()).extra);
    let cur_ix_masked = cur_ix & ring_buffer_mask;
    let mut min_score = (*out.as_deref().unwrap()).score;
    let mut best_score = (*out.as_deref().unwrap()).score;
    let mut best_len = (*out.as_deref().unwrap()).len;
    ...
    StoreH41(self_0.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), data, ring_buffer_mask, cur_ix);
    if (*out.as_deref().unwrap()).score == min_score {
        SearchInStaticDictionary(
            dictionary,
            (*self_0.as_deref_mut().unwrap()).common.as_mut(),
...
unsafe extern "C" fn StoreH42(
    ...
    let fresh13 = (*self_0).free_slot_idx[bank as usize];(*self_0).free_slot_idx[bank as usize]= (*self_0).free_slot_idx[bank as usize].wrapping_add(1);
...
unsafe extern "C" fn StoreRangeH42(
    ...
    i= ix_start;
    while i < ix_end {
        StoreH42(self_0.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), data, mask, i);
        i= i.wrapping_add(1);
    }
}
#[inline(always)]
...
unsafe extern "C" fn FindLongestMatchH42(
    ...
    let mut addr = AddrH42((*self_0.as_deref().unwrap()).extra);
    let mut head = HeadH42((*self_0.as_deref().unwrap()).extra);
    let mut tiny_hashes = TinyHashH42((*self_0.as_deref().unwrap()).extra);
    let mut banks = BanksH42((*self_0.as_deref().unwrap()).extra);
    let cur_ix_masked = cur_ix & ring_buffer_mask;
    let mut min_score = (*out.as_deref().unwrap()).score;
    let mut best_score = (*out.as_deref().unwrap()).score;
    let mut best_len = (*out.as_deref().unwrap()).len;
    ...
    StoreH42(self_0.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), data, ring_buffer_mask, cur_ix);
    if (*out.as_deref().unwrap()).score == min_score {
        SearchInStaticDictionary(
            dictionary,
            (*self_0.as_deref_mut().unwrap()).common.as_mut(),
...
unsafe extern "C" fn StoreRangeH54(
    ...
    i= ix_start;
    while i < ix_end {
        StoreH54(self_0.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), data, mask, i);
        i= i.wrapping_add(1);
    }
}
#[inline(always)]
            ...
            *(*self_0).table.offset(code as isize) = pos as uint32_t;
            if pos == cur_ix && found_ix != kInvalidPosHROLLING_FAST as libc::c_ulong {
                ...
                if backward <= max_backward {
                    ...
                    if len >= 4 as libc::c_int as libc::c_ulong && len > (*out.as_deref().unwrap()).len {
...
unsafe extern "C" fn StoreH35(
    mut self_0: Option<&mut H35>,
    mut data: *const uint8_t,
    mask: size_t,
    ix: size_t,
) {
    StoreH3(core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).ha), data, mask, ix);
    StoreHROLLING_FAST(core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).hb), data, mask, ix);
}
#[inline(always)]
unsafe extern "C" fn StoreRangeH35(
    ...
    StoreRangeH3(Some(&mut (*self_0.as_deref_mut().unwrap()).ha), data, mask, ix_start, ix_end);
    StoreRangeHROLLING_FAST(core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).hb), data, mask, ix_start, ix_end);
}
#[inline(always)]
unsafe extern "C" fn PrepareDistanceCacheH35(
    ...
    PrepareDistanceCacheH3(core::ptr::addr_of_mut!((*self_0).ha), distance_cache);
    PrepareDistanceCacheHROLLING_FAST(core::ptr::addr_of_mut!((*self_0).hb), distance_cache);
}
#[inline(always)]
unsafe extern "C" fn FindLongestMatchH35(
    ...
    FindLongestMatchH3(
        core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).ha),
        dictionary,
        data,
        ring_buffer_mask,
        distance_cache,
    ...
    FindLongestMatchHROLLING_FAST(
        core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).hb),
        dictionary,
        data,
        ring_buffer_mask,
        distance_cache,
...
unsafe extern "C" fn StoreH55(
    mut self_0: Option<&mut H55>,
    mut data: *const uint8_t,
    mask: size_t,
    ix: size_t,
) {
    StoreH54(core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).ha), data, mask, ix);
    StoreHROLLING_FAST(core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).hb), data, mask, ix);
}
#[inline(always)]
unsafe extern "C" fn StoreRangeH55(
    ...
    StoreRangeH54(Some(&mut (*self_0.as_deref_mut().unwrap()).ha), data, mask, ix_start, ix_end);
    StoreRangeHROLLING_FAST(core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).hb), data, mask, ix_start, ix_end);
}
#[inline(always)]
unsafe extern "C" fn PrepareDistanceCacheH55(
    ...
    PrepareDistanceCacheH54(core::ptr::addr_of_mut!((*self_0).ha), distance_cache);
    PrepareDistanceCacheHROLLING_FAST(core::ptr::addr_of_mut!((*self_0).hb), distance_cache);
}
#[inline(always)]
...
unsafe extern "C" fn StoreH65(
    mut self_0: Option<&mut H65>,
    mut data: *const uint8_t,
    mask: size_t,
    ix: size_t,
) {
    StoreH6(core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).ha), data, mask, ix);
    StoreHROLLING(core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).hb), data, mask, ix);
}
#[inline(always)]
unsafe extern "C" fn StoreRangeH65(
    ...
    StoreRangeH6(Some(&mut (*self_0.as_deref_mut().unwrap()).ha), data, mask, ix_start, ix_end);
    StoreRangeHROLLING(core::ptr::addr_of_mut!((*self_0.as_deref_mut().unwrap()).hb), data, mask, ix_start, ix_end);
}
#[inline(always)]
unsafe extern "C" fn PrepareDistanceCacheH65(
    ...
    PrepareDistanceCacheH6(core::ptr::addr_of_mut!((*self_0).ha), distance_cache);
    PrepareDistanceCacheHROLLING(core::ptr::addr_of_mut!((*self_0).hb), distance_cache);
}
#[inline(always)]
...
unsafe extern "C" fn CreateBackwardReferencesNH55(
    ...
    let mut privat: *mut H55 = core::ptr::addr_of_mut!((*hasher).privat._H55);
    ...
    while position.wrapping_add(HashTypeLengthH55()) < pos_end {
        ...
        if sr.score > kMinScore {
            ...
            *num_literals= ((*num_literals) as libc::c_ulong).wrapping_add(insert_length)
            ...
            StoreRangeH55(privat.as_mut(), ringbuffer, ringbuffer_mask, range_start, range_end);
            ...
            if position > apply_random_heuristics {
                        ...
                        )
                {
                    ...
                    while position < pos_jump {
                        StoreH55(privat.as_mut(), ringbuffer, ringbuffer_mask, position);
...
unsafe extern "C" fn CreateBackwardReferencesNH54(
    ...
    let mut privat: *mut H54 = core::ptr::addr_of_mut!((*hasher).privat._H54);
    let max_backward_limit = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as libc::c_ulong);
    let position_offset = (*params).stream_offset;
    let orig_commands: *const Command = commands;
    ...
    while position.wrapping_add(HashTypeLengthH54()) < pos_end {
        ...
        if sr.score > kMinScore {
            ...
            *num_literals= ((*num_literals) as libc::c_ulong).wrapping_add(insert_length)
            ...
            StoreRangeH54(privat.as_mut(), ringbuffer, ringbuffer_mask, range_start, range_end);
            position= (position as libc::c_ulong).wrapping_add(sr.len) as size_t
                as size_t;
        } else {
            insert_length= insert_length.wrapping_add(1);
...
unsafe extern "C" fn CreateBackwardReferencesNH42(
    ...
    let mut privat: *mut H42 = core::ptr::addr_of_mut!((*hasher).privat._H42);
    let max_backward_limit = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as libc::c_ulong);
    let position_offset = (*params).stream_offset;
    let orig_commands: *const Command = commands;
    ...
    while position.wrapping_add(HashTypeLengthH42()) < pos_end {
        ...
        if sr.score > kMinScore {
            ...
            *num_literals= ((*num_literals) as libc::c_ulong).wrapping_add(insert_length)
            ...
            StoreRangeH42(privat.as_mut(), ringbuffer, ringbuffer_mask, range_start, range_end);
            position= (position as libc::c_ulong).wrapping_add(sr.len) as size_t
                as size_t;
        } else {
            insert_length= insert_length.wrapping_add(1);
...
unsafe extern "C" fn CreateBackwardReferencesNH41(
    ...
    let mut privat: *mut H41 = core::ptr::addr_of_mut!((*hasher).privat._H41);
    let max_backward_limit = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as libc::c_ulong);
    let position_offset = (*params).stream_offset;
    let orig_commands: *const Command = commands;
    ...
    while position.wrapping_add(HashTypeLengthH41()) < pos_end {
        ...
        if sr.score > kMinScore {
            ...
            *num_literals= ((*num_literals) as libc::c_ulong).wrapping_add(insert_length)
            ...
            StoreRangeH41(privat.as_mut(), ringbuffer, ringbuffer_mask, range_start, range_end);
            position= (position as libc::c_ulong).wrapping_add(sr.len) as size_t
                as size_t;
        } else {
            insert_length= insert_length.wrapping_add(1);
...
unsafe extern "C" fn CreateBackwardReferencesNH40(
    ...
    let mut privat: *mut H40 = core::ptr::addr_of_mut!((*hasher).privat._H40);
    let max_backward_limit = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as libc::c_ulong);
    let position_offset = (*params).stream_offset;
    let orig_commands: *const Command = commands;
    ...
    while position.wrapping_add(HashTypeLengthH40()) < pos_end {
        ...
        if sr.score > kMinScore {
            ...
            *num_literals= ((*num_literals) as libc::c_ulong).wrapping_add(insert_length)
            ...
            StoreRangeH40(privat.as_mut(), ringbuffer, ringbuffer_mask, range_start, range_end);
            position= (position as libc::c_ulong).wrapping_add(sr.len) as size_t
                as size_t;
        } else {
            insert_length= insert_length.wrapping_add(1);
...
unsafe extern "C" fn CreateBackwardReferencesNH6(
    ...
    let mut privat: *mut H6 = core::ptr::addr_of_mut!((*hasher).privat._H6);
    let max_backward_limit = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as libc::c_ulong);
    let position_offset = (*params).stream_offset;
    let orig_commands: *const Command = commands;
    ...
    while position.wrapping_add(HashTypeLengthH6()) < pos_end {
        ...
        if sr.score > kMinScore {
            ...
            *num_literals= ((*num_literals) as libc::c_ulong).wrapping_add(insert_length)
            ...
            StoreRangeH6(privat.as_mut(), ringbuffer, ringbuffer_mask, range_start, range_end);
            position= (position as libc::c_ulong).wrapping_add(sr.len) as size_t
                as size_t;
        } else {
            insert_length= insert_length.wrapping_add(1);
...
unsafe extern "C" fn CreateBackwardReferencesNH5(
    ...
    let mut privat: *mut H5 = core::ptr::addr_of_mut!((*hasher).privat._H5);
    let max_backward_limit = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as libc::c_ulong);
    let position_offset = (*params).stream_offset;
    let orig_commands: *const Command = commands;
    ...
    while position.wrapping_add(HashTypeLengthH5()) < pos_end {
        ...
        if sr.score > kMinScore {
            ...
            *num_literals= ((*num_literals) as libc::c_ulong).wrapping_add(insert_length)
            ...
            StoreRangeH5(privat.as_mut(), ringbuffer, ringbuffer_mask, range_start, range_end);
            position= (position as libc::c_ulong).wrapping_add(sr.len) as size_t
                as size_t;
        } else {
            insert_length= insert_length.wrapping_add(1);
...
unsafe extern "C" fn CreateBackwardReferencesNH4(
    ...
    let mut privat: *mut H4 = core::ptr::addr_of_mut!((*hasher).privat._H4);
    let max_backward_limit = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as libc::c_ulong);
    let position_offset = (*params).stream_offset;
    let orig_commands: *const Command = commands;
    ...
    while position.wrapping_add(HashTypeLengthH4()) < pos_end {
        ...
        if sr.score > kMinScore {
            ...
            *num_literals= ((*num_literals) as libc::c_ulong).wrapping_add(insert_length)
            ...
            StoreRangeH4(privat.as_mut(), ringbuffer, ringbuffer_mask, range_start, range_end);
            position= (position as libc::c_ulong).wrapping_add(sr.len) as size_t
                as size_t;
        } else {
            insert_length= insert_length.wrapping_add(1);
...
unsafe extern "C" fn CreateBackwardReferencesNH3(
    ...
    let mut privat: *mut H3 = core::ptr::addr_of_mut!((*hasher).privat._H3);
    let max_backward_limit = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as libc::c_ulong);
    let position_offset = (*params).stream_offset;
    let orig_commands: *const Command = commands;
    ...
    while position.wrapping_add(HashTypeLengthH3()) < pos_end {
        ...
        if sr.score > kMinScore {
            ...
            *num_literals= ((*num_literals) as libc::c_ulong).wrapping_add(insert_length)
            ...
            StoreRangeH3(privat.as_mut(), ringbuffer, ringbuffer_mask, range_start, range_end);
            position= (position as libc::c_ulong).wrapping_add(sr.len) as size_t
                as size_t;
        } else {
            insert_length= insert_length.wrapping_add(1);
...
unsafe extern "C" fn CreateBackwardReferencesNH2(
    ...
    let mut privat: *mut H2 = core::ptr::addr_of_mut!((*hasher).privat._H2);
    let max_backward_limit = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as libc::c_ulong);
    let position_offset = (*params).stream_offset;
    let orig_commands: *const Command = commands;
    ...
    while position.wrapping_add(HashTypeLengthH2()) < pos_end {
        ...
        if sr.score > kMinScore {
            ...
            *num_literals= ((*num_literals) as libc::c_ulong).wrapping_add(insert_length)
            ...
            StoreRangeH2(privat.as_mut(), ringbuffer, ringbuffer_mask, range_start, range_end);
            position= (position as libc::c_ulong).wrapping_add(sr.len) as size_t
                as size_t;
        } else {
            insert_length= insert_length.wrapping_add(1);
...
unsafe extern "C" fn CreateBackwardReferencesNH35(
    ...
    let mut privat: *mut H35 = core::ptr::addr_of_mut!((*hasher).privat._H35);
    let max_backward_limit = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as libc::c_ulong);
    let position_offset = (*params).stream_offset;
    let orig_commands: *const Command = commands;
    ...
    while position.wrapping_add(HashTypeLengthH35()) < pos_end {
        ...
        if sr.score > kMinScore {
            ...
            *num_literals= ((*num_literals) as libc::c_ulong).wrapping_add(insert_length)
            ...
            StoreRangeH35(privat.as_mut(), ringbuffer, ringbuffer_mask, range_start, range_end);
            position= (position as libc::c_ulong).wrapping_add(sr.len) as size_t
                as size_t;
        } else {
            insert_length= insert_length.wrapping_add(1);
            ...
            if position > apply_random_heuristics {
                        ...
                        )
                {
                    ...
                    while position < pos_jump {
                        StoreH35(privat.as_mut(), ringbuffer, ringbuffer_mask, position);
                        insert_length= (insert_length as libc::c_ulong)
                            .wrapping_add(4 as libc::c_int as libc::c_ulong) as size_t
                            as size_t;
                        position= (position as libc::c_ulong)
...
unsafe extern "C" fn CreateBackwardReferencesNH65(
    ...
    let mut privat: *mut H65 = core::ptr::addr_of_mut!((*hasher).privat._H65);
    let max_backward_limit = ((1 as libc::c_int as size_t) << (*params).lgwin)
        .wrapping_sub(16 as libc::c_int as libc::c_ulong);
    let position_offset = (*params).stream_offset;
    let orig_commands: *const Command = commands;
    ...
    while position.wrapping_add(HashTypeLengthH65()) < pos_end {
        ...
        if sr.score > kMinScore {
            ...
            *num_literals= ((*num_literals) as libc::c_ulong).wrapping_add(insert_length)
            ...
            StoreRangeH65(privat.as_mut(), ringbuffer, ringbuffer_mask, range_start, range_end);
            position= (position as libc::c_ulong).wrapping_add(sr.len) as size_t
                as size_t;
        } else {
            insert_length= insert_length.wrapping_add(1);
            ...
            if position > apply_random_heuristics {
                        ...
                        )
                {
                    ...
                    while position < pos_jump {
                        StoreH65(privat.as_mut(), ringbuffer, ringbuffer_mask, position);
                        insert_length= (insert_length as libc::c_ulong)
                            .wrapping_add(4 as libc::c_int as libc::c_ulong) as size_t
                            as size_t;
                        position= (position as libc::c_ulong)
