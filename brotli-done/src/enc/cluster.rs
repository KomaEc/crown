...
struct ErasedByPreprocessor66;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor67;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor68;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor69;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor70;
#[inline(always)]
...
pub unsafe extern "C" fn BrotliCompareAndPushToQueueLiteral(
        ...
        == 0 as libc::c_int as libc::c_ulong
    {
        ...
        HistogramAddHistogramLiteral(Some(&mut combo), &*out.offset(idx2 as isize));
    ...
    if is_good_pair != 0 {
        ...
        if (*num_pairs.as_deref().unwrap()) > 0 as libc::c_int as libc::c_ulong
            && HistogramPairIsLess(&raw mut *pairs.offset(0 as libc::c_int as isize), core::ptr::addr_of!(p))
                != 0
        {
...
pub unsafe extern "C" fn BrotliCompareAndPushToQueueCommand(
        ...
        == 0 as libc::c_int as libc::c_ulong
    {
        ...
        let mut combo = *out.offset(idx1 as isize);
        let mut cost_combo: libc::c_double = 0.;
        HistogramAddHistogramCommand(Some(&mut combo), &*out.offset(idx2 as isize));
...
pub unsafe extern "C" fn BrotliCompareAndPushToQueueDistance(
        ...
        == 0 as libc::c_int as libc::c_ulong
    {
        ...
        let mut combo = *out.offset(idx1 as isize);
        let mut cost_combo: libc::c_double = 0.;
        HistogramAddHistogramDistance(Some(&mut combo), &*out.offset(idx2 as isize));
        ...
        while idx2 < num_clusters {
            BrotliCompareAndPushToQueueDistance(
                ...
                *clusters.offset(idx2 as isize),
                max_num_pairs,
                &raw mut *pairs.offset(0 as libc::c_int as isize),
                Some(&mut num_pairs),
            );
    ...
    while num_clusters > min_cluster_size {
        ...
        if (*pairs.offset(0 as libc::c_int as isize)).cost_diff >= cost_diff_threshold {
            ...
            HistogramAddHistogramDistance(
                Some(&mut *out.offset(best_idx1 as isize)),
                &raw mut *out.offset(best_idx2 as isize),
            );
            ...
            *cluster_size.offset(best_idx1 as isize) = (*cluster_size.offset(best_idx1 as isize) as libc::c_uint)
            ...
            while i < num_clusters {
                if *clusters.offset(i as isize) == best_idx2 {
                    memmove(
                        &raw mut *clusters.offset(i as isize) as *mut uint32_t
                            as *mut libc::c_void,
                        &raw mut *clusters
                            .offset(
            ...
            i= 0 as libc::c_int as size_t;
            while i < num_pairs {
                let mut p: *mut crate::src::enc::block_splitter::HistogramPair = &raw mut *pairs.offset(i as isize)
...
pub unsafe extern "C" fn BrotliHistogramBitCostDistanceDistance(
    ...
    if (*histogram).total_count_ == 0 as libc::c_int as libc::c_ulong {
        ...
        HistogramAddHistogramDistance(Some(&mut tmp), candidate);
...
pub unsafe extern "C" fn BrotliHistogramBitCostDistanceCommand(
    ...
    if (*histogram).total_count_ == 0 as libc::c_int as libc::c_ulong {
        ...
        let mut tmp = (*histogram);
        HistogramAddHistogramCommand(Some(&mut tmp), candidate);
...
pub unsafe extern "C" fn BrotliHistogramBitCostDistanceLiteral(
    ...
    if (*histogram).total_count_ == 0 as libc::c_int as libc::c_ulong {
        ...
        let mut tmp = (*histogram);
        HistogramAddHistogramLiteral(Some(&mut tmp), candidate);
...
pub unsafe extern "C" fn BrotliHistogramRemapCommand(
    ...
    while i < in_size {
        ...
        let mut best_bits = BrotliHistogramBitCostDistanceCommand(
            ...
            &raw mut *out.offset(best_out as isize),
        );
        let mut j: size_t = 0;
        ...
        while j < num_clusters {
            let cur_bits = BrotliHistogramBitCostDistanceCommand(
                &*in_0.offset(i as isize),
                &raw mut *out.offset(*clusters.offset(j as isize) as isize),
            );
    ...
    i= 0 as libc::c_int as size_t;
    while i < num_clusters {
        HistogramClearCommand(&raw mut *out.offset(*clusters.offset(i as isize) as isize));
        i= i.wrapping_add(1);
    }
    i= 0 as libc::c_int as size_t;
    while i < in_size {
        HistogramAddHistogramCommand(
            Some(&mut *out.offset(*symbols.offset(i as isize) as isize)),
            &*in_0.offset(i as isize),
        );
        i= i.wrapping_add(1);
    }
...
pub unsafe extern "C" fn BrotliHistogramRemapDistance(
    ...
    i= 0 as libc::c_int as size_t;
    while i < num_clusters {
        HistogramClearDistance(&raw mut *out.offset(*clusters.offset(i as isize) as isize));
        i= i.wrapping_add(1);
    }
    i= 0 as libc::c_int as size_t;
    while i < in_size {
...
pub unsafe extern "C" fn BrotliHistogramRemapLiteral(
    ...
    i= 0 as libc::c_int as size_t;
    while i < num_clusters {
        HistogramClearLiteral(&raw mut *out.offset(*clusters.offset(i as isize) as isize));
        i= i.wrapping_add(1);
    }
    i= 0 as libc::c_int as size_t;
    while i < in_size {
...
pub unsafe extern "C" fn BrotliClusterHistogramsLiteral(
    ...
    while i < in_size {
        ...
        num_new_clusters= BrotliHistogramCombineLiteral(
            out,
            cluster_size,
            &raw mut *histogram_symbols.offset(i as isize),
            &raw mut *clusters.offset(num_clusters as isize),
            pairs,
            num_to_combine,
            num_to_combine,
            max_histograms,
