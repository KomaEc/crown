...
struct ErasedByPreprocessor43;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor44;
#[derive(Copy, Clone)]
...
unsafe extern "C" fn HistogramAddVectorCommand(
    ...
    (*self_0.as_deref_mut().unwrap()).total_count_= ((*self_0.as_deref().unwrap()).total_count_ as libc::c_ulong).wrapping_add(n) as size_t as size_t;
    ...
    loop {
        ...
        let fresh7 = p;
        p= p.offset(1);
        (*self_0.as_deref_mut().unwrap()).data_[(*fresh7) as usize]= (*self_0.as_deref().unwrap()).data_[(*fresh7) as usize].wrapping_add(1);
    };
}
#[inline(always)]
unsafe extern "C" fn HistogramAddVectorLiteral(
    ...
    loop {
        ...
        let fresh10 = p;
        p= p.offset(1);
        (*self_0.as_deref_mut().unwrap()).data_[(*fresh10) as usize]= (*self_0.as_deref().unwrap()).data_[(*fresh10) as usize].wrapping_add(1);
    };
}
#[inline(always)]
unsafe extern "C" fn HistogramAddVectorDistance(
    ...
    loop {
        ...
        let fresh13 = p;
        p= p.offset(1);
        (*self_0.as_deref_mut().unwrap()).data_[(*fresh13) as usize]= (*self_0.as_deref().unwrap()).data_[(*fresh13) as usize].wrapping_add(1);
    };
}
#[inline(always)]
unsafe extern "C" fn HistogramAddHistogramCommand(
    ...
    let mut i: size_t = 0;
    (*self_0.as_deref_mut().unwrap()).total_count_= ((*self_0.as_deref().unwrap()).total_count_ as libc::c_ulong).wrapping_add((*v).total_count_) as size_t
        as size_t;
    i= 0 as libc::c_int as size_t;
    while i < 704 as libc::c_int as libc::c_ulong {
        (*self_0.as_deref_mut().unwrap()).data_[i as usize]= ((*self_0.as_deref().unwrap()).data_[i as usize] as libc::c_uint).wrapping_add((*v).data_[i as usize])
...
unsafe extern "C" fn InitialEntropyCodesCommand(
    ...
    while i < num_histograms {
        ...
        HistogramAddVectorCommand(
            Some(&mut *histograms.offset(i as isize)),
...
unsafe extern "C" fn RandomSampleDistance(
    ...
    mut length: size_t,
    mut stride: size_t,
    mut sample: Option<&mut crate::src::enc::bit_cost::HistogramDistance>,
) {
    let mut pos = 0 as libc::c_int as size_t;
    if stride >= length {
        stride= length;
    ...
    HistogramAddVectorDistance(sample.as_deref_mut(), data.offset(pos as isize), stride);
}
unsafe extern "C" fn RandomSampleLiteral(
    ...
    mut length: size_t,
    mut stride: size_t,
    mut sample: Option<&mut crate::src::enc::bit_cost::HistogramLiteral>,
) {
    let mut pos = 0 as libc::c_int as size_t;
    if stride >= length {
        stride= length;
    ...
    HistogramAddVectorLiteral(sample.as_deref_mut(), data.offset(pos as isize), stride);
}
unsafe extern "C" fn RandomSampleCommand(
    ...
    mut length: size_t,
    mut stride: size_t,
    mut sample: Option<&mut crate::src::enc::bit_cost::HistogramCommand>,
) {
    let mut pos = 0 as libc::c_int as size_t;
    if stride >= length {
        stride= length;
    ...
    HistogramAddVectorCommand(sample.as_deref_mut(), data.offset(pos as isize), stride);
}
unsafe extern "C" fn RefineEntropyCodesLiteral(
    ...
    while iter < iters {
        ...
        RandomSampleLiteral(Some(&mut seed), data, length, stride, Some(&mut sample));
        HistogramAddHistogramLiteral(
            Some(& mut *histograms.offset(iter.wrapping_rem(num_histograms) as isize)),
...
unsafe extern "C" fn RefineEntropyCodesDistance(
    ...
    while iter < iters {
        ...
        RandomSampleDistance(Some(&mut seed), data, length, stride, Some(&mut sample));
...
unsafe extern "C" fn RefineEntropyCodesCommand(
    ...
    while iter < iters {
        ...
        RandomSampleCommand(Some(&mut seed), data, length, stride, Some(&mut sample));
...
unsafe extern "C" fn ClusterBlocksLiteral(
    ...
    while i < num_blocks {
        ...
        while j < num_to_combine {
            let mut k: size_t = 0;
            HistogramClearLiteral(&raw mut *histograms.offset(j as isize));
            k= 0 as libc::c_int as size_t;
            ...
            (*histograms.offset(j as isize))
                ...
                &raw mut *histograms.offset(j as isize),
            );
    ...
    while i < num_blocks {
        ...
        best_bits= crate::src::enc::cluster::BrotliHistogramBitCostDistanceLiteral(
            ...
            &raw mut *all_histograms.offset(best_out as isize),
        );
        j_0= 0 as libc::c_int as size_t;
        while j_0 < num_final_clusters {
            let cur_bits = crate::src::enc::cluster::BrotliHistogramBitCostDistanceLiteral(
                core::ptr::addr_of!(histo),
                &raw mut *all_histograms.offset(*clusters.offset(j_0 as isize) as isize),
            );
...
unsafe extern "C" fn ClusterBlocksDistance(
    ...
    while i < num_blocks {
        ...
        while j < num_to_combine {
            let mut k: size_t = 0;
            HistogramClearDistance(&raw mut *histograms.offset(j as isize));
            k= 0 as libc::c_int as size_t;
            while k < *block_lengths.offset(i.wrapping_add(j) as isize) as libc::c_ulong
            {
                let fresh38 = pos;
...
unsafe extern "C" fn ClusterBlocksCommand(
    ...
    while i < num_blocks {
        ...
        while j < num_to_combine {
            let mut k: size_t = 0;
            HistogramClearCommand(&raw mut *histograms.offset(j as isize));
            k= 0 as libc::c_int as size_t;
            while k < *block_lengths.offset(i.wrapping_add(j) as isize) as libc::c_ulong
            {
                let fresh48 = pos;
