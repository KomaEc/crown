...
struct ErasedByPreprocessor28;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor29;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor30;
#[derive(Copy, Clone)]
...
unsafe extern "C" fn InitCommand(
    mut self_0: Option<&mut crate::src::enc::backward_references::Command>,
...
unsafe extern "C" fn FindAllMatchesH10(
    ...
    if crate::src::enc::static_dict::BrotliFindAllStaticDictionaryMatches(
        ...
        minlen,
        max_length,
        &raw mut *dict_matches.as_mut_ptr().offset(0 as libc::c_int as isize),
    ) != 0
    {
...
unsafe extern "C" fn StoreRangeH10(
    ...
    if ix_start.wrapping_add(512 as libc::c_int as libc::c_ulong) <= i {
        while j < i {
            StoreH10(self_0.as_deref_mut(), data, mask, j);
            j= (j as libc::c_ulong).wrapping_add(8 as libc::c_int as libc::c_ulong)
                as size_t as size_t;
        }
    }
    while i < ix_end {
        StoreH10(self_0.as_deref_mut(), data, mask, i);
        i= i.wrapping_add(1);
    }
}
...
unsafe extern "C" fn ZopfliCostModelSetFromLiteralCosts(
    ...
    crate::src::enc::literal_cost::BrotliEstimateBitCostsForLiterals(
        ...
        ringbuffer_mask,
        ringbuffer,
        &raw mut *literal_costs.offset(1 as libc::c_int as isize),
    );
    *literal_costs.offset(0 as libc::c_int as isize) = 0.0f64 as libc::c_float;
    i= 0 as libc::c_int as size_t;
    while i < num_bytes {
...
unsafe extern "C" fn UpdateZopfliNode(
    ...
    let mut next: *mut ZopfliNode = &raw mut *nodes.offset(pos.wrapping_add(len) as isize)
        as *mut ZopfliNode;
    (*next).length= (len
...
unsafe extern "C" fn ComputeShortestPathFromNodes(
    ...
    while index != 0 as libc::c_int as libc::c_ulong {
        let mut len = ZopfliNodeCommandLength(&raw mut *nodes.offset(index as isize))
            as size_t;
...
pub unsafe extern "C" fn BrotliZopfliCreateCommands(
    ...
    while offset != !(0 as libc::c_int as uint32_t) {
        ...
        InitCommand(
            Some(&mut *commands.offset(i as isize)),
            &(*params).dist,
            insert_length,
            copy_length,
...
pub unsafe extern "C" fn BrotliZopfliComputeShortestPath(
        ...
        .wrapping_sub(1 as libc::c_int as libc::c_ulong) < num_bytes
    {
        ...
        num_matches= FindAllMatchesH10(
            ...
            dictionary_start.wrapping_add(gap),
            params,
            &raw mut *matches.as_mut_ptr().offset(lz_matches_offset as isize),
        );
        if num_matches > 0 as libc::c_int as libc::c_ulong
            && BackwardMatchLength(
                &raw mut *matches.as_mut_ptr()
                    .offset(
        ...
        if num_matches == 1 as libc::c_int as libc::c_ulong
            && BackwardMatchLength(
                &raw mut *matches.as_mut_ptr().offset(0 as libc::c_int as isize),
            ) > max_zopfli_len
        {
            skip= brotli_max_size_t(
                BackwardMatchLength(
...
pub unsafe extern "C" fn BrotliCreateHqZopfliBackwardReferences(
        ...
        .wrapping_sub(1 as libc::c_int as libc::c_ulong) < num_bytes
    {
        ...
        num_found_matches= FindAllMatchesH10(
            ...
            dictionary_start.wrapping_add(gap),
            params,
            &raw mut *matches.offset(cur_match_pos.wrapping_add(shadow_matches) as isize),
        );
