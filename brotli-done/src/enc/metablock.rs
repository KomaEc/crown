...
struct ErasedByPreprocessor124;
...
struct ErasedByPreprocessor125;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor126;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor127;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor128;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor129;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor130;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor131;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor132;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor133;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor134;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor135;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor136;
#[derive(Copy, Clone)]

struct ErasedByPreprocessor137;
#[derive(Copy, Clone)]
#[repr(C)]
...
unsafe extern "C" fn ShannonEntropy(
    ...
    if size & 1 as libc::c_int as libc::c_ulong != 0 {
        current_block= 5650351514014046545;
    } else {
...
unsafe extern "C" fn RecomputeDistancePrefixes(
    ...
    while i < num_commands {
        let mut cmd: *mut crate::src::enc::backward_references::Command = &raw mut *cmds.offset(i as isize) as *mut crate::src::enc::backward_references::Command;
...
pub unsafe extern "C" fn BrotliBuildMetaBlock(
    ...
    RecomputeDistancePrefixes(
        ...
        num_commands,
        core::ptr::addr_of!(orig_params.dist),
        core::ptr::addr_of!((*params.as_deref().unwrap()).dist),
    );
...
unsafe extern "C" fn InitBlockSplitterDistance(
    ...
    *histograms= if (*histograms_size) > 0 as libc::c_int as libc::c_ulong {
    ...
    HistogramClearDistance(
        &raw mut *(*self_0.as_deref().unwrap()).histograms_.offset(0 as libc::c_int as isize),
    );
    ...
    if (*self_0).num_blocks_ == 0 as libc::c_int as libc::c_ulong {
        ...
        *last_entropy
        ...
        if (*self_0).curr_histogram_ix_ < (*(*self_0).histograms_size_) {
            HistogramClearDistance(
                &raw mut *histograms.offset((*self_0).curr_histogram_ix_ as isize),
            );
        }
        ...
        while j < 2 as libc::c_int as libc::c_ulong {
            ...
            HistogramAddHistogramDistance(
                Some(&mut *combined_histo.as_mut_ptr().offset(j as isize)),
                &raw mut *histograms.offset(last_histogram_ix as isize),
            );
            combined_entropy[j
                as usize]= BitsEntropy(
                &raw mut *((*combined_histo.as_mut_ptr().offset(j as isize)).data_)
                    .as_mut_ptr()
        ...
        while i_0 < num_contexts {
            ...
            while j < 2 as libc::c_int as libc::c_ulong {
                ...
                *combined_histo
                ...
                HistogramAddHistogramLiteral(
                    Some(&mut *combined_histo.offset(jx as isize)),
                    &raw mut *histograms.offset(last_histogram_ix as isize),
                );
                combined_entropy[jx
                    as usize]= BitsEntropy(
                    &raw mut *((*combined_histo.offset(jx as isize)).data_)
                        .as_mut_ptr()
                        .offset(0 as libc::c_int as isize),
                    (*self_0).alphabet_size_,
                );
