...
impl rustc_span::source_map::FileLoader for Libtree {
    fn file_exists(&self, path: &Path) -> bool {
        path == Path::new("lib.rs") || path == Path::new("src/libtree.rs")
    }
...
pub struct LibtreeCompiler<'callbacks>(pub &'callbacks mut (dyn Callbacks + Send));

impl rustc_driver::Callbacks for LibtreeCompiler<'_> {
    fn config(&mut self, config: &mut Config) {
        config.file_loader = Some(Box::new(Libtree));
    }
...
unsafe extern "C" fn string_table_store(
    ...
    *fresh11 = (*fresh11 as libc::c_ulong).wrapping_add(n) as size_t as size_t;
}
unsafe extern "C" fn string_table_copy_from_file(
    ...
    loop {
        ...
        *((*t).arr).offset(fresh13 as isize) = c as libc::c_char;
    }
    string_table_maybe_grow(t, 1 as libc::c_int as size_t);
    let ref mut fresh14 = (*t).n;
    let fresh15 = *fresh14;
    *fresh14 = (*fresh14).wrapping_add(1);
    *((*t).arr).offset(fresh15 as isize) = '\0' as i32 as libc::c_char;
}
...
unsafe extern "C" fn tree_preamble(mut s: *mut libtree_state_t, mut depth: size_t) {
    ...
    fputs(
        if (*s)
            .found_all_needed[depth.wrapping_sub(1 as libc::c_int as libc::c_ulong)
    ...
    loop {
        ...
        memcpy(
            &mut *((*st).arr).offset((*s).string_table.n as isize) as *mut libc::c_char
                as *mut libc::c_void,
            &mut *((*st).arr).offset(prev_src as isize) as *mut libc::c_char
                as *const libc::c_void,
            bytes_to_dollar,
        );
        let ref mut fresh19 = (*s).string_table.n;
        *fresh19 = (*fresh19 as libc::c_ulong).wrapping_add(bytes_to_dollar) as size_t
            as size_t;
        prev_src = curr_src;
        memcpy(
            &mut *((*st).arr).offset((*s).string_table.n as isize) as *mut libc::c_char
        ...
        *fresh20 = (*fresh20 as libc::c_ulong).wrapping_add(var_len) as size_t as size_t;
    }
    if prev_src != src {
        ...
        *fresh21 = (*fresh21 as libc::c_ulong).wrapping_add(n) as size_t as size_t;
        return 1 as libc::c_int;
    }
    return 0 as libc::c_int;
}
...
unsafe extern "C" fn recurse(
    ...
    while cont != 0 {
        ...
        match d_tag {
            ...
            1879048187 => {
                no_def_lib
                    |= (0x800 as libc::c_int as libc::c_ulong & d_val
    ...
    let mut should_recurse = (depth < (*s).max_depth
        && (seen_before == 0 && in_exclude_list == 0
            || seen_before == 0 && in_exclude_list != 0
                && (*s).verbosity >= 2 as libc::c_int
            || (*s).verbosity >= 3 as libc::c_int)) as libc::c_int;
    if should_recurse == 0 {
