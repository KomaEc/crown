...
pub fn rewrite_structs(
    ...
    for did in structs {
        ...
        let item = tcx.hir_expect_item(did.expect_local());
...
pub fn rewrite_struct<'hir>(
    ...
    let ItemKind::Struct(_, _generics, variant_data) = &r#struct.kind else {
        panic!()
    };
    for (field, decision) in itertools::izip!(variant_data.fields(), decision) {
        ...
        if let rustc_hir::TyKind::Ptr(pointee) = &field_ty.kind {
            ...
            fn array_ty_replacement(
                tcx: TyCtxt,
                ty: &rustc_hir::Ty,
                array_len: &rustc_hir::ConstArg,
            ) -> String {
                let rustc_hir::ConstArgKind::Anon(constant) = array_len.kind else {
                    unreachable!()
                };
                ...
                if let rustc_hir::TyKind::Array(inner_ty, inner_array_len) = &ty.kind {
                    ...
                    format!(
                        "[{inner}; {}]",
                        rustc_hir_pretty::id_to_string(&tcx, constant.hir_id)
                    )
                } else {
                    format!(
                        "[Default::default(); {}]",
    ...
    if let rustc_hir::TyKind::Path(rustc_hir::QPath::Resolved(_, path)) = ty.kind {
        ...
        if let rustc_hir::def::Res::Def(rustc_hir::def::DefKind::TyAlias, did) = res {
            if let Some(local_did) = did.as_local() {
                let item = tcx
                    .hir_node_by_def_id(local_did)
                    .as_owner()
                    .unwrap()
                    .expect_item();
                let rustc_hir::ItemKind::TyAlias(_, _, aliased_ty) = &item.kind else {
                    unreachable!()
                };
    ...
    match ty.kind {
        ...
        hir::TyKind::Path(ref qpath) => {
            ...
            if "Option" == path.segments[0].ident.as_str() {
                ...
                let span = ty.span;
                let snippet = utils::rewrite::get_snippet(tcx, span).text.1;
                return snippet;
            }
            if let Some(aliased_ty) = try_dealias(ty, tcx) {
                return retype(aliased_ty, decision, tcx);
            }
            rustc_hir_pretty::qpath_to_string(&tcx, qpath)
        }
        hir::TyKind::Array(ty, ref length) => {
            let hir::ConstArgKind::Anon(constant) = length.kind else {
                unreachable!()
            };
            format!(
                "[{}; {}]",
        ...
        hir::TyKind::Tup(tuples) => {
            ...
            rustc_hir_pretty::id_to_string(&tcx, ty.hir_id)
        }
        _ => unreachable!("{:?}", ty),
    }
}
