...
fn rewrite_fn_sig<'tcx>(
    ...
    let fn_item = tcx.hir_expect_item(body.source.def_id().expect_local());
    let ItemKind::Fn {
        sig: fn_sig,
        body: body_id,
        ..
    ...
    let fn_body = tcx.hir_body(*body_id);
...
impl<'tcx, 'me> FnRewriteCtxt<'tcx, 'me> {
    ...
    fn rewrite_operand_at(
        ...
        match operand {
            ...
            Operand::Constant(const_operand) => {
                self.rewrite_constant(const_operand, span, required, rewriter);
            }
        }
    }
    ...
    fn rewrite_rvalue_at(
        ...
        match rvalue {
            ...
            Rvalue::BinaryOp(_, box (operand1, operand2)) => {
                ...
                if let syn::Expr::Assign(assign) = parsed_expr {
                    // special case
                ...
                match self.try_rewrite_alloc_from_dest(
                    ...
                    Ok(result) => {
                        if let Some(malloc_span) = result {
                            let replacement = {
                                let ty = ty.builtin_deref(true).unwrap();
            ...
            Rvalue::RawPtr(rustc_mutability, place) => {
                if matches!(rustc_mutability, RawPtrKind::Mut) {
    ...
    fn rewrite_constant(
        ...
        if let Some(scalar) = const_operand.const_.try_to_scalar() {
            match scalar {
                rustc_const_eval::interpret::Scalar::Int(scalar) => {
                    if scalar.is_null() {
                        if let PlaceCtxt::Ptr(ptr_kinds) = required {
                            let ptr_kind = ptr_kinds[0];
                            let span = const_operand.span.until(stmt_span.shrink_to_hi());
                            if ptr_kind.is_safe() {
