...
use super::{SSAIdx, dom::DominanceFrontier};
...
/// Property: the set of join points must satisfy that
...
impl JoinPoints<PhiNode> {
    pub fn new<'tcx>(
        ...
        def_sites: &IndexVec<Local, MixedBitSet<BasicBlock>>,
    ) -> Self {
        let mut join_points = JoinPoints::from_raw(IndexVec::from_elem(
            BasicBlockNodes::new(),
            &body.basic_blocks,
        ));
        let mut already_added = MixedBitSet::new_empty(body.basic_blocks.len());
