...
impl<Qualifier> TypeQualifiers<Qualifier> where Qualifier: Copy {
    pub fn make_data(&self, crate_data: &CrateData) -> QualifierData<Qualifier> {
        ...
        for &did in &crate_data.fns {
            ...
            for (local, qualifiers) in itertools::izip!(body.local_decls.indices(), fn_results.results()) {
                if let Some(var_name) = user_idents.get(&local) {
                    data.insert(var_name.as_str().to_owned(), SmallVec::from_slice(qualifiers));
                }
            }
        ...
        for &did in &crate_data.structs {
            ...
            for (field_def, qualifiers) in itertools::izip!(adt_def.all_fields(), self.struct_results(&did)) {
                data.insert(field_def.name.as_str().to_owned(), SmallVec::from_slice(qualifiers));
            }
        ...
        QualifierData { fn_data, struct_data }
    }
}
