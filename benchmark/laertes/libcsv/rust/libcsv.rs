...
#[no_mangle]
pub unsafe extern "C" fn csv_set_opts(mut p: *mut csv_parser,
...
#[no_mangle]
pub unsafe extern "C" fn csv_init(mut p: *mut csv_parser,
    ...
    (*p).pstate = 0 as std::os::raw::c_int;
    (*p).quoted = 0 as std::os::raw::c_int;
    (*p).spaces = 0 as std::os::raw::c_int as size_t;
    (*p).entry_pos = 0 as std::os::raw::c_int as size_t;
    (*p).entry_size = 0 as std::os::raw::c_int as size_t;
    (*p).status = 0 as std::os::raw::c_int;
    (*p).options = options;
    (*p).quote_char = 0x22 as std::os::raw::c_int as std::os::raw::c_uchar;
    (*p).delim_char = 0x2c as std::os::raw::c_int as std::os::raw::c_uchar;
    (*p).is_space = None;
    (*p).is_term = None;
    (*p).blk_size = 128 as std::os::raw::c_int as size_t;
    (*p).malloc_func = None;
    (*p).realloc_func =
        Some(realloc as
...
#[no_mangle]
pub unsafe extern "C" fn csv_fini(mut p: *mut csv_parser,
                                  mut cb1:
 ...
 -> std::os::raw::c_int {
    ...
    let mut quoted: std::os::raw::c_int = (*p).quoted;
    let mut pstate: std::os::raw::c_int = (*p).pstate;
    let mut spaces: size_t = (*p).spaces;
    let mut entry_pos: size_t = (*p).entry_pos;
    if pstate == 2 as std::os::raw::c_int && (*p).quoted != 0 &&
           (*p).options as std::os::raw::c_int & 1 as std::os::raw::c_int != 0 &&
           (*p).options as std::os::raw::c_int & 4 as std::os::raw::c_int != 0 {
    ...
    match pstate {
        3 => {
            (*p).entry_pos =
                ((*p).entry_pos as
                     std::os::raw::c_ulong).wrapping_sub((*p).spaces.wrapping_add(1 as
                                                                              std::os::raw::c_int
                                                                              as
                                                                              std::os::raw::c_ulong))
                    as size_t as size_t;
      ...
      */
        {
            ...
            if (*p).options as std::os::raw::c_int & 8 as std::os::raw::c_int != 0 {
                *(*p).entry_buf.offset(entry_pos as isize) =
                    '\u{0}' as i32 as std::os::raw::c_uchar
            }
            if cb1.is_some() &&
                   (*p).options as std::os::raw::c_int & 16 as std::os::raw::c_int != 0 &&
                   quoted == 0 &&
    ...
    (*p).quoted = (*p).entry_pos as std::os::raw::c_int;
    (*p).spaces = (*p).quoted as size_t;
    (*p).pstate = 0 as std::os::raw::c_int;
    return 0 as std::os::raw::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn csv_set_delim(mut p: *mut csv_parser,
                                       mut c: std::os::raw::c_uchar) {
    /* Set the delimiter */
    if !p.is_null() { (*p).delim_char = c };
}
#[no_mangle]
pub unsafe extern "C" fn csv_set_quote(mut p: *mut csv_parser,
                                       mut c: std::os::raw::c_uchar) {
    /* Set the quote character */
    if !p.is_null() { (*p).quote_char = c };
}
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn csv_set_space_func(mut p: *mut csv_parser,
                                            mut f:
                                                Option<unsafe extern "C" fn(_:
                                                                                std::os::raw::c_uchar)
                                                           -> std::os::raw::c_int>) {
    /* Set the space function */
    if !p.is_null() { (*p).is_space = f };
}
#[no_mangle]
pub unsafe extern "C" fn csv_set_term_func(mut p: *mut csv_parser,
                                           mut f:
                                               Option<unsafe extern "C" fn(_:
                                                                               std::os::raw::c_uchar)
                                                          -> std::os::raw::c_int>) {
    /* Set the term function */
    if !p.is_null() { (*p).is_term = f };
}
#[no_mangle]
pub unsafe extern "C" fn csv_set_realloc_func(mut p: *mut csv_parser,
                                              mut f:
                                                  Option<unsafe extern "C" fn(_:
                                                                                  *mut std::os::raw::c_void,
                                                                              _:
    ...
    if !p.is_null() && f.is_some() { (*p).realloc_func = f };
}
#[no_mangle]
pub unsafe extern "C" fn csv_set_free_func(mut p: *mut csv_parser,
                                           mut f:
                                               Option<unsafe extern "C" fn(_:
                                                                               *mut std::os::raw::c_void)
                                                          -> ()>) {
    /* Set the free function used to free the buffer */
    if !p.is_null() && f.is_some() { (*p).free_func = f };
}
#[no_mangle]
pub unsafe extern "C" fn csv_set_blk_size(mut p: *mut csv_parser,
                                          mut size: size_t) {
    /* Set the block size used to increment buffer size */
    if !p.is_null() { (*p).blk_size = size };
}
#[no_mangle]
pub unsafe extern "C" fn csv_get_buffer_size(mut p: *const csv_parser)
 -> size_t {
    /* Get the size of the entry buffer */
    if !p.is_null() { return (*p).entry_size }
...
#[no_mangle]
pub unsafe extern "C" fn csv_parse(mut p: *mut csv_parser,
    ...
    let mut delim: std::os::raw::c_uchar = (*p).delim_char;
    let mut quote: std::os::raw::c_uchar = (*p).quote_char;
    let mut is_space:
    ...
    if (*p).entry_buf.is_null() && pos < len {
        ...
        if csv_increase_buffer(p) != 0 as std::os::raw::c_int {
            (*p).quoted = quoted;
            (*p).pstate = pstate;
            (*p).spaces = spaces;
    ...
    while pos < len {
        ...
        if entry_pos ==
               (if (*p).options as std::os::raw::c_int & 8 as std::os::raw::c_int != 0 {
                    (*p).entry_size.wrapping_sub(1 as std::os::raw::c_int as
                                                     std::os::raw::c_ulong)
                } else { (*p).entry_size }) {
            if csv_increase_buffer(p) != 0 as std::os::raw::c_int {
                (*p).quoted = quoted;
            ...
            0 | 1 => {
                ...
                if if is_term.is_some() {
                    ...
                    if pstate == 1 as std::os::raw::c_int {
                        ...
                        if cb1.is_some() &&
                               (*p).options as std::os::raw::c_int & 16 as std::os::raw::c_int
                                   != 0 && quoted == 0 &&
                               entry_pos == 0 as std::os::raw::c_int as std::os::raw::c_ulong
                           {
                            cb1.expect("non-null function pointer")(0 as
                        ...
                        quoted = spaces as std::os::raw::c_int;
                        entry_pos = quoted as size_t
                    } else if (*p).options as std::os::raw::c_int & 2 as std::os::raw::c_int
                                  != 0 {
                        if cb2.is_some() {
                            cb2.expect("non-null function pointer")(c as
                                                                        std::os::raw::c_int,
                ...
                if c as std::os::raw::c_int == quote as std::os::raw::c_int {
                    ...
                    if quoted != 0 {
                        ...
                        *(*p).entry_buf.offset(fresh2 as isize) = c;
                        ...
                        if (*p).options as std::os::raw::c_int & 1 as std::os::raw::c_int != 0
                           {
                            (*p).status = 1 as std::os::raw::c_int;
                            (*p).quoted = quoted;
                            (*p).pstate = pstate;
                            (*p).spaces = spaces;
                            (*p).entry_pos = entry_pos;
