...
pub unsafe extern "C" fn uncompress2(
    ...
    loop {
        ...
        if stream.avail_in == 0 as libc::c_int as libc::c_uint {
            ...
            len = (len as libc::c_ulong).wrapping_sub(stream.avail_in as libc::c_ulong) as uLong
                as uLong
        }
    ...
    *sourceLen = (*sourceLen as libc::c_ulong)
    ...
    return if err == Z_STREAM_END {
        ...
        Z_DATA_ERROR
    } else if err == Z_BUF_ERROR && left.wrapping_add(stream.avail_out as libc::c_ulong) != 0 {
        Z_DATA_ERROR
    } else {
        err
    };
