...
extern "C" {
    ...
    #[no_mangle]
    fn bb_strtou(arg: *const libc::c_char, endp: *mut *mut libc::c_char,
                 base: libc::c_int) -> libc::c_uint;
    #[no_mangle]
    ...
    #[no_mangle]
    fn concat_subpath_file(path: *const libc::c_char,
                           filename: *const libc::c_char)
     -> *mut libc::c_char;
    #[no_mangle]
    fn get_signum(name: *const libc::c_char) -> libc::c_int;
    #[no_mangle]
    fn index_in_substrings(strings: *const libc::c_char,
                           key: *const libc::c_char) -> libc::c_int;
    #[no_mangle]
    fn bb_makedev(major: libc::c_uint, minor: libc::c_uint)
     -> libc::c_ulonglong;
}
...
unsafe extern "C" fn add_inode(mut st: *const stat) {
    ...
    *curr =
        xzalloc(::std::mem::size_of::<inode_list>() as libc::c_ulong) as
            *mut inode_list;
    (**curr).dev = (*st).st_dev;
...
unsafe extern "C" fn search_dev_inode(mut st: *const stat) -> smallint {
    ...
    while !ilist.is_null() {
        if (*ilist).dev == (*st).st_dev {
            if option_mask32 & OPT_MOUNT as libc::c_int as libc::c_uint != 0 {
                return 1 as libc::c_int as smallint
            }
    ...
    return 0 as libc::c_int as smallint;
}
unsafe extern "C" fn scan_proc_net_or_maps(mut path: *const libc::c_char,
 ...
 -> smallint {
    ...
    if f.is_null() { return 0 as libc::c_int as smallint }
    if (*(bb_common_bufsiz1.as_mut_ptr() as *mut globals)).recursion_depth ==
           PROC_NET as libc::c_int {
        let mut fd: libc::c_int = 0;
        statbuf.st_dev = 0 as libc::c_int as __dev_t;
        fd =
            socket(2 as libc::c_int, SOCK_DGRAM as libc::c_int,
                   0 as libc::c_int);
        if fd >= 0 as libc::c_int { fstat(fd, &mut statbuf); close(fd); }
        fmt =
            b"%*d: %64[0-9A-Fa-f]:%x %*x:%*x %*x %*x:%*x %*x:%*x %*x %*d %*d %llu\x00"
                as *const u8 as *const libc::c_char;
        fag = addr.as_mut_ptr() as *mut libc::c_void;
        sag = &mut tmp_port as *mut libc::c_uint as *mut libc::c_void
    } else {
        fmt =
            b"%*s %*s %*s %x:%x %llu\x00" as *const u8 as *const libc::c_char;
        fag = &mut major as *mut libc::c_int as *mut libc::c_void;
        sag = &mut minor as *mut libc::c_int as *mut libc::c_void
    }
    retval = 0 as libc::c_int as smallint;
    while !fgets_unlocked(line.as_mut_ptr(), 255 as libc::c_int, f).is_null()
          {
        r =
            sscanf(line.as_mut_ptr(), fmt, fag, sag,
                   &mut uint64_inode as *mut libc::c_longlong);
        if r != 3 as libc::c_int { continue ; }
        statbuf.st_ino = uint64_inode as __ino_t;
        if (*(bb_common_bufsiz1.as_mut_ptr() as *mut globals)).recursion_depth
               == PROC_NET as libc::c_int {
            r = strlen(addr.as_mut_ptr()) as libc::c_int;
            if r == 8 as libc::c_int &&
                   option_mask32 & OPT_IP6 as libc::c_int as libc::c_uint != 0
               {
                continue ;
            }
            if r > 8 as libc::c_int &&
                   option_mask32 & OPT_IP4 as libc::c_int as libc::c_uint != 0
               {
                continue ;
            }
            if tmp_port == port { add_inode(&mut statbuf); }
        } else {
            if !(major != 0 as libc::c_int && minor != 0 as libc::c_int &&
                     statbuf.st_ino != 0 as libc::c_int as libc::c_ulong) {
                continue ;
            }
            statbuf.st_dev =
                bb_makedev(major as libc::c_uint, minor as libc::c_uint) as
                    __dev_t;
...
unsafe extern "C" fn scan_recursive(mut path: *const libc::c_char)
 -> smallint {
    ...
    let mut retval: smallint = 0;
    d = opendir(path);
    if d.is_null() { return 0 as libc::c_int as smallint }
    let ref mut fresh0 =
        (*(bb_common_bufsiz1.as_mut_ptr() as *mut globals)).recursion_depth;
    *fresh0 += 1;
    retval = 0 as libc::c_int as smallint;
    stop_scan = 0 as libc::c_int as smallint;
    while stop_scan == 0 && { d_ent = readdir(d); !d_ent.is_null() } {
        let mut statbuf: stat =
            stat{st_dev: 0,
                 st_ino: 0,
                 st_nlink: 0,
        ...
        if subpath.is_null() { continue ; }
        let mut current_block_25: u64;
        match (*(bb_common_bufsiz1.as_mut_ptr() as
                     *mut globals)).recursion_depth {
            1 => {
                pid =
                    bb_strtou((*d_ent).d_name.as_mut_ptr(),
                              0 as *mut *mut libc::c_char, 10 as libc::c_int)
                        as pid_t;
                if *bb_errno != 0 as libc::c_int ||
                       pid ==
                           (*(bb_common_bufsiz1.as_mut_ptr() as
                                  *mut globals)).mypid ||
                       scan_recursive(subpath) as libc::c_int ==
                           0 as libc::c_int {
                    current_block_25 = 17784502470059252271;
                } else {
                    if option_mask32 & OPT_KILL as libc::c_int as libc::c_uint
                           != 0 {
                        if kill(pid,
                                (*(bb_common_bufsiz1.as_mut_ptr() as
                                       *mut globals)).killsig) !=
                               0 as libc::c_int {
                            bb_perror_msg(b"kill pid %s\x00" as *const u8 as
                                              *const libc::c_char,
                                          (*d_ent).d_name.as_mut_ptr());
                            (*(bb_common_bufsiz1.as_mut_ptr() as
                                   *mut globals)).kill_failed =
                                1 as libc::c_int as smallint
                        }
                    }
                    if option_mask32 &
                           OPT_SILENT as libc::c_int as libc::c_uint == 0 {
                        printf(b"%s \x00" as *const u8 as *const libc::c_char,
                               (*d_ent).d_name.as_mut_ptr());
                    }
                    retval = 1 as libc::c_int as smallint;
                    current_block_25 = 17784502470059252271;
                }
            }
            2 => {
                match index_in_substrings(b"cwd\x00exe\x00root\x00fd\x00lib\x00mmap\x00maps\x00\x00"
                                              as *const u8 as
                                              *const libc::c_char,
                                          (*d_ent).d_name.as_mut_ptr()) {
                    0 | 1 | 2 => { current_block_25 = 11935410828529466384; }
                    3 | 4 | 5 => {
                        current_block_25 = 17309584228929823145;
                        match current_block_25 {
                            17309584228929823145 => {
                                stop_scan = scan_recursive(subpath);
                                if stop_scan != 0 { retval = stop_scan }
                            }
                            _ => {
                                stop_scan =
                    ...
                    6 => {
                        current_block_25 = 4202293315256094820;
                        match current_block_25 {
                            17309584228929823145 => {
                                stop_scan = scan_recursive(subpath);
                                if stop_scan != 0 { retval = stop_scan }
                    ...
                    _ => { current_block_25 = 17784502470059252271; }
                }
            }
            3 => { current_block_25 = 11935410828529466384; }
            _ => { current_block_25 = 17784502470059252271; }
        }
        match current_block_25 {
            11935410828529466384 => {
                if !(stat(subpath, &mut statbuf) < 0 as libc::c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn fuser_main(mut argc: libc::c_int,
 ...
 -> libc::c_int {
    ...
    loop  {
        ...
        sig = get_signum(&mut *arg.offset(1 as libc::c_int as isize));
        if sig < 0 as libc::c_int { continue ; }
        (*(bb_common_bufsiz1.as_mut_ptr() as *mut globals)).killsig = sig;
        loop  {
            arg = *pp.offset(1 as libc::c_int as isize);
            let ref mut fresh2 = *pp.offset(0 as libc::c_int as isize);
            *fresh2 = arg;
            pp = pp.offset(1);
    ...
    getopt32(argv, b"^mks64\x00-1\x00" as *const u8 as *const libc::c_char);
    ...
    while !(*pp).is_null() {
        ...
        if sscanf(*pp, b"%u/%4s\x00" as *const u8 as *const libc::c_char,
                  &mut port as *mut libc::c_uint,
                  path.as_mut_ptr().offset(::std::mem::size_of::<[libc::c_char; 11]>()
                                               as libc::c_ulong as
                                               isize).offset(-(1 as
                                                                   libc::c_int
                                                                   as isize)))
               == 2 as libc::c_int &&
               access(path.as_mut_ptr(), 4 as libc::c_int) == 0 as libc::c_int
           {
    ...
    if scan_recursive(b"/proc\x00" as *const u8 as *const libc::c_char) != 0 {
        ...
        return (*(bb_common_bufsiz1.as_mut_ptr() as *mut globals)).kill_failed
                   as libc::c_int
    }
