...
pub unsafe extern "C" fn ht_create() -> *mut ht {
    let mut table: *mut ht =
        malloc(::core::mem::size_of::<ht>() as libc::c_ulong) as *mut ht;
    if table.is_null() {
        return 0 as *mut ht;
    }
    ...
    (*table).entries = calloc(
        (*table).capacity,
        ::core::mem::size_of::<ht_entry>() as libc::c_ulong
    ) as *mut ht_entry;
...
unsafe extern "C" fn hash_key(mut key: *const libc::c_char) -> uint64_t {
    ...
    while *p != 0 {
        ...
        hash = (hash as libc::c_ulong)
            .wrapping_mul(1099511628211 as libc::c_ulong)
...
unsafe extern "C" fn ht_set_entry(
    ...
    let mut index: size_t =
        hash & capacity.wrapping_sub(1 as libc::c_int as libc::c_ulong);
    while !((*entries.offset(index as isize)).key).is_null() {
        if strcmp(key, (*entries.offset(index as isize)).key)
            == 0 as libc::c_int
        {
            let ref mut fresh0 = (*entries.offset(index as isize)).value;
            *fresh0 = value;
    ...
    let ref mut fresh1 = (*entries.offset(index as isize)).key;
    *fresh1 = key as *mut libc::c_char;
    let ref mut fresh2 = (*entries.offset(index as isize)).value;
    *fresh2 = value;
    return key;
}
unsafe extern "C" fn ht_expand(mut table: *mut ht) -> bool {
    let mut new_capacity: size_t =
        ((*table).capacity).wrapping_mul(2 as libc::c_int as libc::c_ulong);
    if new_capacity < (*table).capacity {
        return 0 as libc::c_int != 0;
    }
    let mut new_entries: *mut ht_entry = calloc(
        new_capacity,
        ::core::mem::size_of::<ht_entry>() as libc::c_ulong
    ) as *mut ht_entry;
    ...
    let mut i: size_t = 0 as libc::c_int as size_t;
    while i < (*table).capacity {
        let mut entry: ht_entry = *((*table).entries).offset(i as isize);
...
pub unsafe extern "C" fn ht_next(mut it: *mut hti) -> bool {
    let mut table: *mut ht = (*it)._table;
    while (*it)._index < (*table).capacity {
        let mut i: size_t = (*it)._index;
