...
pub unsafe extern "C" fn zadd_unsigned(
    ...
    let mut addend: *mut zahl_char_t = 0 as *mut zahl_char_t;
    if zzero(b) != 0 {
        zabs(a, c);
        return;
    } else if zzero(c) != 0 {
    ...
    *((*a).chars).offset(size as isize) = 0 as libc::c_int as zahl_char_t;
        ...
        if (*a).used < (*c).used {
            ...
            memset(
                ...
                n.wrapping_sub((*a).used)
                    .wrapping_mul(
                        ::core::mem::size_of::<zahl_char_t>() as libc::c_ulong
                    )
            );
        }
        addend = (*c).chars;
    ...
    while carry[(i & 1 as libc::c_int as libc::c_ulong) as usize] != 0 {
        carry[(!i & 1 as libc::c_int as libc::c_ulong) as usize] =
            (*((*a).chars).offset(i as isize) == 4294967295 as libc::c_uint)
                as libc::c_int as uint32_t;
        let fresh1 = i;
        i = i.wrapping_add(1);
