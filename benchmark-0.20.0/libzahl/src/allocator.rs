...
#[no_mangle]
pub unsafe extern "C" fn libzahl_realloc(
    ...
    if need & (!need).wrapping_add(1 as libc::c_int as libc::c_ulong) != need {
        ...
        i = ::core::mem::size_of::<size_t>() as libc::c_ulong;
    ...
    if libzahl_pool_n[i as usize] != 0 {
        ...
        libzahl_pool_n[i as usize];
        new = *(libzahl_pool[i as usize])
            .offset(libzahl_pool_n[i as usize] as isize);
        memcpy(
            new as *mut libc::c_void,
            (*a).chars as *const libc::c_void,
            ((*a).alloced).wrapping_mul(
