...
pub unsafe extern "C" fn znot(
    ...
    if bits != 0 {
        let ref mut fresh1 = *((*a).chars).offset(
            ((*a).used).wrapping_sub(1 as libc::c_int as libc::c_ulong)
                as isize
        );
        *fresh1 &= ((1 as libc::c_int as zahl_char_t) << bits)
