...
unsafe extern "C" fn split_node_(
    ...
    let mut hw: libc::c_double =
        (*(*node).bounds).width / 2 as libc::c_int as libc::c_double;
    let mut hh: libc::c_double =
        (*(*node).bounds).height / 2 as libc::c_int as libc::c_double;
    ...
    let mut key: *mut libc::c_void = (*node).key;
