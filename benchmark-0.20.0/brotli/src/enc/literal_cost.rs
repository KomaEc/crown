...
unsafe extern "C" fn DecideMultiByteStatsLevel(
    ...
    let mut max_utf8: size_t = 1 as libc::c_int as size_t;
    let mut last_c: size_t = 0 as libc::c_int as size_t;
    let mut i: size_t = 0;
    i = 0 as libc::c_int as size_t;
    while i < len {
        let mut c: size_t =
            *data.offset((pos.wrapping_add(i) & mask) as isize) as size_t;
        counts[UTF8Position(last_c, c, 2 as libc::c_int as size_t) as usize] =
            (counts
                [UTF8Position(last_c, c, 2 as libc::c_int as size_t) as usize])
                .wrapping_add(1);
        counts[UTF8Position(last_c, c, 2 as libc::c_int as size_t) as usize];
        last_c = c;
        i = i.wrapping_add(1);
...
unsafe extern "C" fn EstimateBitCostsForLiteralsUTF8(
    ...
    let max_utf8: size_t = DecideMultiByteStatsLevel(pos, len, mask, data);
    ...
    while i < in_window {
        ...
        histogram[utf8_pos as usize][c as usize];
        in_window_utf8[utf8_pos as usize] =
            (in_window_utf8[utf8_pos as usize]).wrapping_add(1);
        in_window_utf8[utf8_pos as usize];
    ...
    while i < len {
        if i >= window_half {
            let mut c_0: size_t = (if i < window_half
            ...
            let mut last_c_0: size_t = (if i < window_half
            ...
            histogram[utf8_pos2 as usize][*data.offset(
                (pos.wrapping_add(i).wrapping_sub(window_half) & mask) as isize
            ) as usize] = (histogram[utf8_pos2 as usize][*data.offset(
                (pos.wrapping_add(i).wrapping_sub(window_half) & mask) as isize
            )
                as usize])
                .wrapping_sub(1);
        ...
        if i.wrapping_add(window_half) < len {
            ...
            histogram[utf8_pos2_0 as usize][*data.offset(
                (pos.wrapping_add(i).wrapping_add(window_half) & mask) as isize
            ) as usize] = (histogram[utf8_pos2_0 as usize][*data.offset(
                (pos.wrapping_add(i).wrapping_add(window_half) & mask) as isize
            )
                as usize])
                .wrapping_add(1);
        ...
        let mut c_2: size_t = (if i < 1 as libc::c_int as libc::c_ulong {
            0 as libc::c_int
        } else {
            *data.offset(
                (pos.wrapping_add(i)
        ...
        let mut last_c_2: size_t = (if i < 2 as libc::c_int as libc::c_ulong {
            0 as libc::c_int
        } else {
            *data.offset(
                (pos.wrapping_add(i)
        ...
        let mut histo: size_t = histogram[utf8_pos_0 as usize]
        ...
        lit_cost =
            FastLog2(in_window_utf8[utf8_pos_0 as usize]) - FastLog2(histo);
        lit_cost += 0.02905f64;
        if lit_cost < 1.0f64 {
            lit_cost *= 0.5f64;
            lit_cost += 0.5f64;
...
pub unsafe extern "C" fn BrotliEstimateBitCostsForLiterals(
    ...
    if BrotliIsMostlyUTF8(data, pos, mask, len, kMinUTF8Ratio) != 0 {
        ...
        let mut window_half: size_t = 2000 as libc::c_int as size_t;
        let mut in_window: size_t = brotli_min_size_t(window_half, len);
        let mut i: size_t = 0;
        i = 0 as libc::c_int as size_t;
        while i < in_window {
        ...
        while i < len {
            ...
            if i >= window_half {
                histogram[*data.offset(
                    (pos.wrapping_add(i).wrapping_sub(window_half) & mask)
                        as isize
                ) as usize] = (histogram[*data.offset(
                    (pos.wrapping_add(i).wrapping_sub(window_half) & mask)
                        as isize
                ) as usize])
                    .wrapping_sub(1);
            ...
            if i.wrapping_add(window_half) < len {
                histogram[*data.offset(
                    (pos.wrapping_add(i).wrapping_add(window_half) & mask)
                        as isize
                ) as usize] = (histogram[*data.offset(
                    (pos.wrapping_add(i).wrapping_add(window_half) & mask)
                        as isize
            ...
            let mut lit_cost: libc::c_double =
                FastLog2(in_window) - FastLog2(histo);
            lit_cost += 0.029f64;
            if lit_cost < 1.0f64 {
                lit_cost *= 0.5f64;
                lit_cost += 0.5f64;
