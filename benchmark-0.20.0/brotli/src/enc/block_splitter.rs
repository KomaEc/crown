...
extern "C" {
    ...
    fn BrotliPopulationCostLiteral(
        _: *const HistogramLiteral
    ) -> libc::c_double;
    fn BrotliPopulationCostCommand(
        _: *const HistogramCommand
    ) -> libc::c_double;
    fn BrotliPopulationCostDistance(
        _: *const HistogramDistance
    ) -> libc::c_double;
...
#[inline(always)]
unsafe extern "C" fn brotli_max_uint8_t(
    mut a: uint8_t,
    mut b: uint8_t
) -> uint8_t {
...
unsafe extern "C" fn HistogramAddLiteral(
    ...
    (*self_0).data_[val as usize] =
        ((*self_0).data_[val as usize]).wrapping_add(1);
    (*self_0).data_[val as usize];
...
unsafe extern "C" fn HistogramAddVectorLiteral(
    ...
    (*self_0).total_count_ = ((*self_0).total_count_ as libc::c_ulong)
        .wrapping_add(n) as size_t as size_t;
    ...
    loop {
        ...
        let fresh0 = p;
        p = p.offset(1);
        (*self_0).data_[*fresh0 as usize] =
            ((*self_0).data_[*fresh0 as usize]).wrapping_add(1);
        (*self_0).data_[*fresh0 as usize];
    }
}
#[inline(always)]
unsafe extern "C" fn HistogramAddHistogramLiteral(
    ...
    let mut i: size_t = 0;
    (*self_0).total_count_ = ((*self_0).total_count_ as libc::c_ulong)
        .wrapping_add((*v).total_count_) as size_t
        as size_t;
    i = 0 as libc::c_int as size_t;
    while i < 256 as libc::c_int as libc::c_ulong {
        (*self_0).data_[i as usize] = ((*self_0).data_[i as usize]
            as libc::c_uint)
...
unsafe extern "C" fn HistogramAddVectorCommand(
    ...
    loop {
        ...
        let fresh1 = p;
        p = p.offset(1);
        (*self_0).data_[*fresh1 as usize] =
            ((*self_0).data_[*fresh1 as usize]).wrapping_add(1);
        (*self_0).data_[*fresh1 as usize];
    }
}
#[inline(always)]
...
#[inline(always)]
unsafe extern "C" fn HistogramClearDistance(
...
unsafe extern "C" fn HistogramAddVectorDistance(
    ...
    loop {
        ...
        let fresh2 = p;
        p = p.offset(1);
        (*self_0).data_[*fresh2 as usize] =
            ((*self_0).data_[*fresh2 as usize]).wrapping_add(1);
        (*self_0).data_[*fresh2 as usize];
    }
}
#[inline(always)]
...
unsafe extern "C" fn CountLiterals(
    ...
    while i < num_commands {
        total_length = (total_length as libc::c_ulong).wrapping_add(
            (*cmds.offset(i as isize)).insert_len_ as libc::c_ulong
        ) as size_t as size_t;
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn CopyLiteralsToByteArray(
    ...
    i = 0 as libc::c_int as size_t;
    while i < num_commands {
        let mut insert_len: size_t =
            (*cmds.offset(i as isize)).insert_len_ as size_t;
        if from_pos.wrapping_add(insert_len) > mask {
            let mut head_size: size_t = mask
            ...
            pos = (pos as libc::c_ulong).wrapping_add(head_size) as size_t
                as size_t;
            insert_len = (insert_len as libc::c_ulong).wrapping_sub(head_size)
                as size_t as size_t;
        }
        if insert_len > 0 as libc::c_int as libc::c_ulong {
            ...
            pos = (pos as libc::c_ulong).wrapping_add(insert_len) as size_t
                as size_t;
        }
        from_pos =
            from_pos
                ...
                .wrapping_add(
                    CommandCopyLen(&*cmds.offset(i as isize)) as libc::c_ulong
                )
                & mask;
        i = i.wrapping_add(1);
        i;
...
unsafe extern "C" fn MyRand(mut seed: *mut uint32_t) -> uint32_t {
    *seed = (*seed as libc::c_uint).wrapping_mul(16807 as libc::c_uint)
        as uint32_t as uint32_t;
    return *seed;
}
#[inline(always)]
...
unsafe extern "C" fn InitialEntropyCodesLiteral(
    ...
    let mut seed: uint32_t = 7 as libc::c_int as uint32_t;
    ...
    while i < num_histograms {
        ...
        if i != 0 as libc::c_int as libc::c_ulong {
            pos = (pos as libc::c_ulong).wrapping_add(
...
unsafe extern "C" fn RandomSampleLiteral(
    ...
    if stride >= length {
        ...
        pos = (MyRand(seed) as libc::c_ulong).wrapping_rem(
            length
                .wrapping_sub(stride)
                .wrapping_add(1 as libc::c_int as libc::c_ulong)
        );
...
unsafe extern "C" fn RefineEntropyCodesLiteral(
    ...
    while iter < iters {
        let mut sample: HistogramLiteral = HistogramLiteral {
            data_: [0; 256],
            total_count_: 0,
            bit_cost_: 0.
        };
...
unsafe extern "C" fn FindBlocksLiteral(
    ...
    let data_size: size_t = HistogramDataSizeLiteral();
    let bitmaplen: size_t = num_histograms
        .wrapping_add(7 as libc::c_int as libc::c_ulong)
        >> 3 as libc::c_int;
    let mut num_blocks: size_t = 1 as libc::c_int as size_t;
    let mut i: size_t = 0;
    let mut j: size_t = 0;
    ...
    while i != 0 as libc::c_int as libc::c_ulong {
        ...
        while j < num_histograms {
            *insert_cost.offset(
                ...
                - BitCost(
                    (*histograms.offset(j as isize)).data_[i as usize]
                        as size_t
                );
            j = j.wrapping_add(1);
            j;
    ...
    i = 0 as libc::c_int as size_t;
    while i < length {
        let byte_ix: size_t = i;
        let mut ix: size_t = byte_ix.wrapping_mul(bitmaplen);
        let mut insert_cost_ix: size_t = (*data.offset(byte_ix as isize)
            as libc::c_ulong)
            .wrapping_mul(num_histograms);
        let mut min_cost: libc::c_double = 1e99f64;
        let mut block_switch_cost: libc::c_double = block_switch_bitcost;
        let mut k: size_t = 0;
        ...
        while k < num_histograms {
            *cost.offset(k as isize) -= min_cost;
            if *cost.offset(k as isize) >= block_switch_cost {
                let mask: uint8_t = ((1 as libc::c_uint)
                    << (k & 7 as libc::c_int as libc::c_ulong))
                    as uint8_t;
                *cost.offset(k as isize) = block_switch_cost;
                let ref mut fresh3 = *switch_signal
                    .offset(ix.wrapping_add(k >> 3 as libc::c_int) as isize);
                *fresh3 =
                    (*fresh3 as libc::c_int | mask as libc::c_int) as uint8_t;
            }
            k = k.wrapping_add(1);
            k;
        }
    ...
    let mut byte_ix_0: size_t =
        length.wrapping_sub(1 as libc::c_int as libc::c_ulong);
    let mut ix_0: size_t = byte_ix_0.wrapping_mul(bitmaplen);
    let mut cur_id: uint8_t = *block_id.offset(byte_ix_0 as isize);
    while byte_ix_0 > 0 as libc::c_int as libc::c_ulong {
        let mask_0: uint8_t = ((1 as libc::c_uint)
            << (cur_id as libc::c_int & 7 as libc::c_int))
            as uint8_t;
        byte_ix_0 = byte_ix_0.wrapping_sub(1);
        byte_ix_0;
        ix_0 =
            (ix_0 as libc::c_ulong).wrapping_sub(bitmaplen) as size_t as size_t;
...
unsafe extern "C" fn RemapBlockIdsLiteral(
    ...
    let mut next_id: uint16_t = 0 as libc::c_int as uint16_t;
    let mut i: size_t = 0;
    i = 0 as libc::c_int as size_t;
    while i < num_histograms {
    ...
    while i < length {
            ...
            == kInvalidId as libc::c_int
        {
            ...
            *new_id.offset(*block_ids.offset(i as isize) as isize) = fresh4;
        }
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn ClusterBlocksLiteral(
    ...
    let mut histogram_symbols: *mut uint32_t =
        if num_blocks > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
                num_blocks.wrapping_mul(
    ...
    let mut block_lengths: *mut uint32_t =
        if num_blocks > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
                num_blocks.wrapping_mul(
    ...
    let expected_num_clusters: size_t = (16 as libc::c_int as libc::c_ulong)
        .wrapping_mul(
            num_blocks
    ...
    let mut all_histograms_size: size_t = 0 as libc::c_int as size_t;
    let mut all_histograms_capacity: size_t = expected_num_clusters;
    let mut all_histograms: *mut HistogramLiteral =
        if all_histograms_capacity > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
                all_histograms_capacity
                    .wrapping_mul(::core::mem::size_of::<HistogramLiteral>()
                        as libc::c_ulong)
            ) as *mut HistogramLiteral
        } else {
            0 as *mut HistogramLiteral
        };
    let mut cluster_size_size: size_t = 0 as libc::c_int as size_t;
    let mut cluster_size_capacity: size_t = expected_num_clusters;
    let mut cluster_size: *mut uint32_t = if cluster_size_capacity
        > 0 as libc::c_int as libc::c_ulong
    {
        BrotliAllocate(
            m,
            cluster_size_capacity.wrapping_mul(
                ::core::mem::size_of::<uint32_t>() as libc::c_ulong
            )
        ) as *mut uint32_t
    } else {
    ...
    let mut num_clusters: size_t = 0 as libc::c_int as size_t;
    let mut histograms: *mut HistogramLiteral =
        if brotli_min_size_t(num_blocks, 64 as libc::c_int as size_t)
            > 0 as libc::c_int as libc::c_ulong
        {
            BrotliAllocate(
                m,
    ...
    let mut max_num_pairs: size_t =
        (64 as libc::c_int * 64 as libc::c_int / 2 as libc::c_int) as size_t;
    let mut pairs_capacity: size_t =
        max_num_pairs.wrapping_add(1 as libc::c_int as libc::c_ulong);
    let mut pairs: *mut HistogramPair =
        if pairs_capacity > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
                pairs_capacity
                    .wrapping_mul(::core::mem::size_of::<HistogramPair>()
                        as libc::c_ulong)
    ...
    let mut block_idx: size_t = 0 as libc::c_int as size_t;
    i = 0 as libc::c_int as size_t;
    while i < length {
        let ref mut fresh5 = *block_lengths.offset(block_idx as isize);
    ...
    i = 0 as libc::c_int as size_t;
    while i < num_blocks {
        let num_to_combine: size_t = brotli_min_size_t(
        ...
        while j < num_to_combine {
            ...
            k = 0 as libc::c_int as size_t;
            while k < *block_lengths.offset(i.wrapping_add(j) as isize)
                as libc::c_ulong
            {
                let fresh6 = pos;
                ...
                HistogramAddLiteral(
                    ...
                    *data.offset(fresh6 as isize) as size_t
                );
                k = k.wrapping_add(1);
                k;
            }
        ...
        if all_histograms_capacity
            < all_histograms_size.wrapping_add(num_new_clusters)
        {
            let mut _new_size: size_t = if all_histograms_capacity
                == 0 as libc::c_int as libc::c_ulong
            {
                all_histograms_size.wrapping_add(num_new_clusters)
            } else {
            ...
            new_array = if _new_size > 0 as libc::c_int as libc::c_ulong {
                BrotliAllocate(
                    m,
                    _new_size.wrapping_mul(::core::mem::size_of::<
                        HistogramLiteral
                    >()
                        as libc::c_ulong)
                ) as *mut HistogramLiteral
                ...
                && all_histograms_capacity != 0 as libc::c_int as libc::c_ulong
            {
                memcpy(
                    ...
                    all_histograms_capacity.wrapping_mul(
                        ::core::mem::size_of::<HistogramLiteral>()
                            as libc::c_ulong
                    )
                );
            }
        ...
        if cluster_size_capacity
            < cluster_size_size.wrapping_add(num_new_clusters)
        {
            let mut _new_size_0: size_t =
                if cluster_size_capacity == 0 as libc::c_int as libc::c_ulong {
                    cluster_size_size.wrapping_add(num_new_clusters)
                } else {
        ...
        while j < num_new_clusters {
            let fresh7 = all_histograms_size;
            all_histograms_size = all_histograms_size.wrapping_add(1);
            *all_histograms.offset(fresh7 as isize) =
                *histograms.offset(new_clusters[j as usize] as isize);
            let fresh8 = cluster_size_size;
            cluster_size_size = cluster_size_size.wrapping_add(1);
            *cluster_size.offset(fresh8 as isize) =
                sizes[new_clusters[j as usize] as usize];
        ...
        j = 0 as libc::c_int as size_t;
        while j < num_to_combine {
            *histogram_symbols.offset(i.wrapping_add(j) as isize) =
                (num_clusters as uint32_t)
                    .wrapping_add(remap[symbols[j as usize] as usize]);
            j = j.wrapping_add(1);
            j;
        }
        num_clusters = (num_clusters as libc::c_ulong)
            .wrapping_add(num_new_clusters) as size_t
            as size_t;
        i = (i as libc::c_ulong)
            .wrapping_add(64 as libc::c_int as libc::c_ulong)
            as size_t as size_t;
    ...
    i = 0 as libc::c_int as size_t;
    while i < num_blocks {
        let mut histo: HistogramLiteral = HistogramLiteral {
            data_: [0; 256],
            total_count_: 0,
            bit_cost_: 0.
        };
        ...
        while j_0 < *block_lengths.offset(i as isize) as libc::c_ulong {
            ...
            HistogramAddLiteral(
                &mut histo,
                *data.offset(fresh9 as isize) as size_t
            );
        ...
        while j_0 < num_final_clusters {
            let cur_bits: libc::c_double =
                BrotliHistogramBitCostDistanceLiteral(
                    &mut histo,
                    &mut *all_histograms
                        .offset(*clusters.offset(j_0 as isize) as isize)
                );
        ...
        *histogram_symbols.offset(i as isize) = best_out;
        if *new_index.offset(best_out as isize) == kInvalidIndex {
            let fresh10 = next_index;
            next_index = next_index.wrapping_add(1);
            *new_index.offset(best_out as isize) = fresh10;
        }
        i = i.wrapping_add(1);
        i;
    }
    ...
    if (*split).types_alloc_size < num_blocks {
        let mut _new_size_1: size_t =
            if (*split).types_alloc_size == 0 as libc::c_int as libc::c_ulong {
                num_blocks
            } else {
                (*split).types_alloc_size
            };
        let mut new_array_1: *mut uint8_t = 0 as *mut uint8_t;
    ...
    if (*split).lengths_alloc_size < num_blocks {
        let mut _new_size_2: size_t = if (*split).lengths_alloc_size
            == 0 as libc::c_int as libc::c_ulong
        {
            num_blocks
        } else {
        ...
        let mut new_array_2: *mut uint32_t = 0 as *mut uint32_t;
    ...
    let mut cur_length: uint32_t = 0 as libc::c_int as uint32_t;
    let mut block_idx_0: size_t = 0 as libc::c_int as size_t;
    let mut max_type: uint8_t = 0 as libc::c_int as uint8_t;
    i = 0 as libc::c_int as size_t;
    while i < num_blocks {
        cur_length = (cur_length as libc::c_uint)
            .wrapping_add(*block_lengths.offset(i as isize))
            as uint32_t as uint32_t;
            ...
            || *histogram_symbols.offset(i as isize)
                != *histogram_symbols
                    .offset(i.wrapping_add(1 as libc::c_int as libc::c_ulong)
                        as isize)
        {
            let id: uint8_t = *new_index
                .offset(*histogram_symbols.offset(i as isize) as isize)
                as uint8_t;
    ...
    (*split).num_types =
        (max_type as size_t).wrapping_add(1 as libc::c_int as libc::c_ulong);
...
unsafe extern "C" fn SplitByteVectorLiteral(
    ...
    let mut num_histograms: size_t = length
        ...
        .wrapping_add(1 as libc::c_int as libc::c_ulong);
    let mut histograms: *mut HistogramLiteral = 0 as *mut HistogramLiteral;
    ...
    if length == 0 as libc::c_int as libc::c_ulong {
        ...
        (*split).num_types = 1 as libc::c_int as size_t;
        *((*split).types).offset((*split).num_blocks as isize) =
            0 as libc::c_int as uint8_t;
        *((*split).lengths).offset((*split).num_blocks as isize) =
            length as uint32_t;
    ...
    let mut num_blocks: size_t = 0 as libc::c_int as size_t;
    let bitmaplen: size_t = num_histograms
        .wrapping_add(7 as libc::c_int as libc::c_ulong)
        >> 3 as libc::c_int;
    let mut insert_cost: *mut libc::c_double = if data_size
        .wrapping_mul(num_histograms)
        > 0 as libc::c_int as libc::c_ulong
    {
        BrotliAllocate(
    ...
    let mut cost: *mut libc::c_double =
        if num_histograms > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
                num_histograms
                    .wrapping_mul(::core::mem::size_of::<libc::c_double>()
                        as libc::c_ulong)
            ) as *mut libc::c_double
        } else {
            0 as *mut libc::c_double
        };
    let mut switch_signal: *mut uint8_t =
        if length.wrapping_mul(bitmaplen) > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
    ...
    let iters: size_t = (if (*params).quality < 11 as libc::c_int {
        3 as libc::c_int
    } else {
        10 as libc::c_int
    }) as size_t;
    ...
    while i < iters {
        ...
        num_histograms =
            RemapBlockIdsLiteral(block_ids, length, new_id, num_histograms);
...
unsafe extern "C" fn RefineEntropyCodesCommand(
    ...
    iter = 0 as libc::c_int as size_t;
    while iter < iters {
        let mut sample: HistogramCommand = HistogramCommand {
            data_: [0; 704],
            total_count_: 0,
            bit_cost_: 0.
        };
...
unsafe extern "C" fn FindBlocksCommand(
    ...
    let data_size: size_t = HistogramDataSizeCommand();
    let bitmaplen: size_t = num_histograms
        .wrapping_add(7 as libc::c_int as libc::c_ulong)
        >> 3 as libc::c_int;
    let mut num_blocks: size_t = 1 as libc::c_int as size_t;
            ...
            *cost.offset(k as isize) -= min_cost;
            if *cost.offset(k as isize) >= block_switch_cost {
                let mask: uint8_t = ((1 as libc::c_uint)
                    ...
                    as uint8_t;
                *cost.offset(k as isize) = block_switch_cost;
                let ref mut fresh11 = *switch_signal
                    .offset(ix.wrapping_add(k >> 3 as libc::c_int) as isize);
                *fresh11 =
                    (*fresh11 as libc::c_int | mask as libc::c_int) as uint8_t;
            }
            k = k.wrapping_add(1);
            k;
        }
...
unsafe extern "C" fn RemapBlockIdsCommand(
    ...
    while i < length {
            ...
            == kInvalidId as libc::c_int
        {
            let fresh12 = next_id;
            next_id = next_id.wrapping_add(1);
            *new_id.offset(*block_ids.offset(i as isize) as isize) = fresh12;
        }
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn ClusterBlocksCommand(
    ...
    let mut all_histograms_size: size_t = 0 as libc::c_int as size_t;
    let mut all_histograms_capacity: size_t = expected_num_clusters;
    let mut all_histograms: *mut HistogramCommand =
        if all_histograms_capacity > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
                all_histograms_capacity
                    .wrapping_mul(::core::mem::size_of::<HistogramCommand>()
                        as libc::c_ulong)
    ...
    i = 0 as libc::c_int as size_t;
    while i < length {
        let ref mut fresh13 = *block_lengths.offset(block_idx as isize);
    ...
    while i < num_blocks {
        ...
        while j < num_to_combine {
                ...
                as libc::c_ulong
            {
                ...
                HistogramAddCommand(
                    &mut *histograms.offset(j as isize),
                    *data.offset(fresh14 as isize) as size_t
                );
                k = k.wrapping_add(1);
                k;
            }
            ...
            < all_histograms_size.wrapping_add(num_new_clusters)
        {
                ...
                && all_histograms_capacity != 0 as libc::c_int as libc::c_ulong
            {
                memcpy(
                    ...
                    all_histograms as *const libc::c_void,
                    all_histograms_capacity.wrapping_mul(
                        ::core::mem::size_of::<HistogramCommand>()
                            as libc::c_ulong
                    )
                );
            }
        ...
        j = 0 as libc::c_int as size_t;
        while j < num_new_clusters {
            let fresh15 = all_histograms_size;
            all_histograms_size = all_histograms_size.wrapping_add(1);
            *all_histograms.offset(fresh15 as isize) =
                *histograms.offset(new_clusters[j as usize] as isize);
            let fresh16 = cluster_size_size;
            cluster_size_size = cluster_size_size.wrapping_add(1);
            *cluster_size.offset(fresh16 as isize) =
                sizes[new_clusters[j as usize] as usize];
            remap[new_clusters[j as usize] as usize] = j as uint32_t;
            j = j.wrapping_add(1);
            j;
    ...
    i = 0 as libc::c_int as size_t;
    while i < num_blocks {
        let mut histo: HistogramCommand = HistogramCommand {
            data_: [0; 704],
            total_count_: 0,
            bit_cost_: 0.
        };
        ...
        while j_0 < *block_lengths.offset(i as isize) as libc::c_ulong {
            ...
            HistogramAddCommand(
                &mut histo,
                *data.offset(fresh17 as isize) as size_t
            );
            j_0 = j_0.wrapping_add(1);
            j_0;
        }
        ...
        *histogram_symbols.offset(i as isize) = best_out;
        if *new_index.offset(best_out as isize) == kInvalidIndex {
            let fresh18 = next_index;
            next_index = next_index.wrapping_add(1);
            *new_index.offset(best_out as isize) = fresh18;
        }
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn SplitByteVectorCommand(
    ...
    let mut num_histograms: size_t = length
        .wrapping_div(literals_per_histogram)
        .wrapping_add(1 as libc::c_int as libc::c_ulong);
    let mut histograms: *mut HistogramCommand = 0 as *mut HistogramCommand;
    if num_histograms > max_histograms {
        num_histograms = max_histograms;
    }
    if length == 0 as libc::c_int as libc::c_ulong {
...
unsafe extern "C" fn RefineEntropyCodesDistance(
    ...
    iter = 0 as libc::c_int as size_t;
    while iter < iters {
        let mut sample: HistogramDistance = HistogramDistance {
            data_: [0; 544],
            total_count_: 0,
            bit_cost_: 0.
        };
...
unsafe extern "C" fn FindBlocksDistance(
    ...
    let data_size: size_t = HistogramDataSizeDistance();
    let bitmaplen: size_t = num_histograms
        .wrapping_add(7 as libc::c_int as libc::c_ulong)
        >> 3 as libc::c_int;
    let mut num_blocks: size_t = 1 as libc::c_int as size_t;
            ...
            *cost.offset(k as isize) -= min_cost;
            if *cost.offset(k as isize) >= block_switch_cost {
                let mask: uint8_t = ((1 as libc::c_uint)
                    ...
                    as uint8_t;
                *cost.offset(k as isize) = block_switch_cost;
                let ref mut fresh19 = *switch_signal
                    .offset(ix.wrapping_add(k >> 3 as libc::c_int) as isize);
                *fresh19 =
                    (*fresh19 as libc::c_int | mask as libc::c_int) as uint8_t;
            }
            k = k.wrapping_add(1);
            k;
        }
...
unsafe extern "C" fn RemapBlockIdsDistance(
    ...
    while i < length {
            ...
            == kInvalidId as libc::c_int
        {
            let fresh20 = next_id;
            next_id = next_id.wrapping_add(1);
            *new_id.offset(*block_ids.offset(i as isize) as isize) = fresh20;
        }
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn ClusterBlocksDistance(
    ...
    let mut all_histograms_size: size_t = 0 as libc::c_int as size_t;
    let mut all_histograms_capacity: size_t = expected_num_clusters;
    let mut all_histograms: *mut HistogramDistance =
        if all_histograms_capacity > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
                all_histograms_capacity
                    .wrapping_mul(::core::mem::size_of::<HistogramDistance>()
                        as libc::c_ulong)
    ...
    i = 0 as libc::c_int as size_t;
    while i < length {
        let ref mut fresh21 = *block_lengths.offset(block_idx as isize);
    ...
    while i < num_blocks {
        ...
        while j < num_to_combine {
                ...
                as libc::c_ulong
            {
                ...
                HistogramAddDistance(
                    &mut *histograms.offset(j as isize),
                    *data.offset(fresh22 as isize) as size_t
                );
                k = k.wrapping_add(1);
                k;
            }
            ...
            < all_histograms_size.wrapping_add(num_new_clusters)
        {
                ...
                && all_histograms_capacity != 0 as libc::c_int as libc::c_ulong
            {
                memcpy(
                    ...
                    all_histograms as *const libc::c_void,
                    all_histograms_capacity.wrapping_mul(
                        ::core::mem::size_of::<HistogramDistance>()
                            as libc::c_ulong
                    )
                );
            }
        ...
        j = 0 as libc::c_int as size_t;
        while j < num_new_clusters {
            let fresh23 = all_histograms_size;
            all_histograms_size = all_histograms_size.wrapping_add(1);
            *all_histograms.offset(fresh23 as isize) =
                *histograms.offset(new_clusters[j as usize] as isize);
            let fresh24 = cluster_size_size;
            cluster_size_size = cluster_size_size.wrapping_add(1);
            *cluster_size.offset(fresh24 as isize) =
                sizes[new_clusters[j as usize] as usize];
            remap[new_clusters[j as usize] as usize] = j as uint32_t;
            j = j.wrapping_add(1);
            j;
    ...
    while i < num_blocks {
        ...
        while j_0 < *block_lengths.offset(i as isize) as libc::c_ulong {
            ...
            HistogramAddDistance(
                &mut histo,
                *data.offset(fresh25 as isize) as size_t
            );
            j_0 = j_0.wrapping_add(1);
            j_0;
        }
        ...
        *histogram_symbols.offset(i as isize) = best_out;
        if *new_index.offset(best_out as isize) == kInvalidIndex {
            let fresh26 = next_index;
            next_index = next_index.wrapping_add(1);
            *new_index.offset(best_out as isize) = fresh26;
        }
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn SplitByteVectorDistance(
    ...
    let mut num_histograms: size_t = length
        .wrapping_div(literals_per_histogram)
        .wrapping_add(1 as libc::c_int as libc::c_ulong);
    let mut histograms: *mut HistogramDistance = 0 as *mut HistogramDistance;
    if num_histograms > max_histograms {
        num_histograms = max_histograms;
    }
    if length == 0 as libc::c_int as libc::c_ulong {
    ...
    while i < iters {
        ...
        num_histograms =
            RemapBlockIdsDistance(block_ids, length, new_id, num_histograms);
...
pub unsafe extern "C" fn BrotliSplitBlock(
    ...
    let mut literals: *mut uint8_t =
        if literals_count > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
                literals_count.wrapping_mul(
                    ::core::mem::size_of::<uint8_t>() as libc::c_ulong
                )
            ) as *mut uint8_t
        } else {
    ...
    let mut insert_and_copy_codes: *mut uint16_t =
        if num_commands > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
                num_commands.wrapping_mul(
                    ::core::mem::size_of::<uint16_t>() as libc::c_ulong
                )
            ) as *mut uint16_t
        } else {
    ...
    i = 0 as libc::c_int as size_t;
    while i < num_commands {
        *insert_and_copy_codes.offset(i as isize) =
            (*cmds.offset(i as isize)).cmd_prefix_;
        i = i.wrapping_add(1);
        i;
    }
    ...
    let mut distance_prefixes: *mut uint16_t =
        if num_commands > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
                num_commands.wrapping_mul(
    ...
    while i_0 < num_commands {
        let mut cmd: *const Command =
            &*cmds.offset(i_0 as isize) as *const Command;
        if CommandCopyLen(cmd) != 0
            && (*cmd).cmd_prefix_ as libc::c_int >= 128 as libc::c_int
        {
            let fresh27 = j;
            j = j.wrapping_add(1);
            *distance_prefixes.offset(fresh27 as isize) =
