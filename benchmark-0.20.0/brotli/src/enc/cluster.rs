...
pub unsafe extern "C" fn BrotliCompareAndPushToQueueLiteral(
    ...
    let mut is_good_pair: libc::c_int = 0 as libc::c_int;
    ...
    if (*out.offset(idx1 as isize)).total_count_
        ...
        == 0 as libc::c_int as libc::c_ulong
    {
        ...
        let mut threshold: libc::c_double =
...
pub unsafe extern "C" fn BrotliHistogramCombineLiteral(
    ...
    let mut cost_diff_threshold: libc::c_double = 0.0f64;
    let mut min_cluster_size: size_t = 1 as libc::c_int as size_t;
    let mut num_pairs: size_t = 0 as libc::c_int as size_t;
    ...
    while num_clusters > min_cluster_size {
        ...
        let mut i: size_t = 0;
        if (*pairs.offset(0 as libc::c_int as isize)).cost_diff
            >= cost_diff_threshold
        {
            ...
            (*out.offset(best_idx1 as isize)).bit_cost_ =
                (*pairs.offset(0 as libc::c_int as isize)).cost_combo;
            let ref mut fresh0 = *cluster_size.offset(best_idx1 as isize);
            *fresh0 = (*fresh0 as libc::c_uint)
                .wrapping_add(*cluster_size.offset(best_idx2 as isize))
                as uint32_t as uint32_t;
            i = 0 as libc::c_int as size_t;
            ...
            num_clusters;
            let mut copy_to_idx: size_t = 0 as libc::c_int as size_t;
            i = 0 as libc::c_int as size_t;
            while i < num_pairs {
...
pub unsafe extern "C" fn BrotliHistogramRemapLiteral(
    ...
    i = 0 as libc::c_int as size_t;
    while i < in_size {
        let mut best_out: uint32_t = if i == 0 as libc::c_int as libc::c_ulong {
        ...
        let mut best_bits: libc::c_double =
    ...
    while i < num_clusters {
        HistogramClearLiteral(
            &mut *out.offset(*clusters.offset(i as isize) as isize)
        );
        i = i.wrapping_add(1);
        i;
    }
...
pub unsafe extern "C" fn BrotliHistogramReindexLiteral(
    ...
    let mut new_index: *mut uint32_t =
    ...
    let mut tmp: *mut HistogramLiteral = 0 as *mut HistogramLiteral;
    let mut i: size_t = 0;
    ...
    i = 0 as libc::c_int as size_t;
    while i < length {
        if *new_index.offset(*symbols.offset(i as isize) as isize)
            == kInvalidIndex
        {
...
pub unsafe extern "C" fn BrotliClusterHistogramsLiteral(
    ...
    let mut cluster_size: *mut uint32_t =
        if in_size > 0 as libc::c_int as libc::c_ulong {
            BrotliAllocate(
                m,
                in_size.wrapping_mul(
                    ::core::mem::size_of::<uint32_t>() as libc::c_ulong
                )
            ) as *mut uint32_t
        } else {
    ...
    let max_input_histograms: size_t = 64 as libc::c_int as size_t;
    ...
    let mut pairs: *mut HistogramPair =
        if pairs_capacity.wrapping_add(1 as libc::c_int as libc::c_ulong)
            > 0 as libc::c_int as libc::c_ulong
        {
            BrotliAllocate(
                m,
    ...
    while i < in_size {
        ...
        while j < num_to_combine {
            *clusters.offset(num_clusters.wrapping_add(j) as isize) =
        ...
        num_clusters = (num_clusters as libc::c_ulong)
            ...
            as size_t;
        i = (i as libc::c_ulong).wrapping_add(max_input_histograms) as size_t
            as size_t;
    }
        ...
        < max_num_pairs.wrapping_add(1 as libc::c_int as libc::c_ulong)
    {
        ...
        let mut new_array: *mut HistogramPair = 0 as *mut HistogramPair;
        while _new_size
            < max_num_pairs.wrapping_add(1 as libc::c_int as libc::c_ulong)
        {
    ...
    *out_size =
        BrotliHistogramReindexLiteral(m, out, histogram_symbols, in_size);
    if 0 as libc::c_int != 0 {
        return;
    }
}
...
pub unsafe extern "C" fn BrotliHistogramReindexCommand(
    ...
    let mut next_index: uint32_t = 0;
    let mut tmp: *mut HistogramCommand = 0 as *mut HistogramCommand;
    let mut i: size_t = 0;
    if 0 as libc::c_int != 0 || 0 as libc::c_int != 0 {
        return 0 as libc::c_int as size_t;
    }
...
pub unsafe extern "C" fn BrotliClusterHistogramsCommand(
    ...
    clusters = 0 as *mut uint32_t;
    *out_size =
        BrotliHistogramReindexCommand(m, out, histogram_symbols, in_size);
    if 0 as libc::c_int != 0 {
        return;
    }
}
...
pub unsafe extern "C" fn BrotliHistogramReindexDistance(
    ...
    let mut next_index: uint32_t = 0;
    let mut tmp: *mut HistogramDistance = 0 as *mut HistogramDistance;
    let mut i: size_t = 0;
    if 0 as libc::c_int != 0 || 0 as libc::c_int != 0 {
        return 0 as libc::c_int as size_t;
    }
...
pub unsafe extern "C" fn BrotliClusterHistogramsDistance(
    ...
    clusters = 0 as *mut uint32_t;
    *out_size =
        BrotliHistogramReindexDistance(m, out, histogram_symbols, in_size);
    if 0 as libc::c_int != 0 {
        return;
    }
}
