...
unsafe extern "C" fn BlockSplitIteratorNext(
    mut self_0: *mut BlockSplitIterator
) {
    if (*self_0).length_ == 0 as libc::c_int as libc::c_ulong {
        (*self_0).idx_ = ((*self_0).idx_).wrapping_add(1);
        (*self_0).idx_;
        (*self_0).type_ = *((*(*self_0).split_).types)
            .offset((*self_0).idx_ as isize)
            as size_t;
        (*self_0).length_ = *((*(*self_0).split_).lengths)
            .offset((*self_0).idx_ as isize)
            as size_t;
    }
...
pub unsafe extern "C" fn BrotliBuildHistogramsWithContext(
    ...
    while i < num_commands {
        ...
        HistogramAddCommand(
            &mut *insert_and_copy_histograms
                .offset(insert_and_copy_it.type_ as isize),
        ...
        while j != 0 as libc::c_int as libc::c_ulong {
            ...
            if !context_modes.is_null() {
                let mut lut: ContextLut =
                    &*_kBrotliContextLookupTable.as_ptr().offset(
                        ((*context_modes.offset(context as isize)
                            as libc::c_uint)
                            << 9 as libc::c_int)
                            as isize
                    ) as *const uint8_t;
                context = (context << 6 as libc::c_int).wrapping_add(
