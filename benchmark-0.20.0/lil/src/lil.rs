...
unsafe extern "C" fn alloc_value(mut str: *const libc::c_char) -> lil_value_t {
    ...
    if !str.is_null() {
        (*val).l = strlen(str);
        (*val).d =
            malloc(((*val).l).wrapping_add(1 as libc::c_int as libc::c_ulong))
                as *mut libc::c_char;
        if ((*val).d).is_null() {
...
pub unsafe extern "C" fn lil_append_string(
    ...
    (*val).l =
        ((*val).l as libc::c_ulong).wrapping_add(len) as size_t as size_t;
    (*val).d = new;
    return 1 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn lil_append_val(
    ...
    (*val).l =
        ((*val).l as libc::c_ulong).wrapping_add((*v).l) as size_t as size_t;
    (*val).d = new;
    return 1 as libc::c_int;
}
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn lil_list_append(
    mut list: lil_list_t,
    mut val: lil_value_t
) {
    let mut nv: *mut lil_value_t = realloc(
        (*list).v as *mut libc::c_void,
        (::core::mem::size_of::<lil_value_t>() as libc::c_ulong).wrapping_mul(
            ((*list).c).wrapping_add(1 as libc::c_int as libc::c_ulong)
        )
    ) as *mut lil_value_t;
    if nv.is_null() {
        return;
    ...
    let ref mut fresh2 = *nv.offset(fresh1 as isize);
    *fresh2 = val;
}
#[no_mangle]
...
unsafe extern "C" fn lil_find_local_var(
    ...
    if (*env).vars > 0 as libc::c_int as libc::c_ulong {
        let mut i: size_t =
            ((*env).vars).wrapping_sub(1 as libc::c_int as libc::c_ulong);
        loop {
...
unsafe extern "C" fn find_cmd(
    ...
    if (*lil).cmds > 0 as libc::c_int as libc::c_ulong {
        let mut i: size_t =
            ((*lil).cmds).wrapping_sub(1 as libc::c_int as libc::c_ulong);
        loop {
...
unsafe extern "C" fn add_func(
    ...
    ncmd = realloc(
        ...
        (::core::mem::size_of::<lil_func_t>() as libc::c_ulong).wrapping_mul(
            ((*lil).cmds).wrapping_add(1 as libc::c_int as libc::c_ulong)
        )
    ) as *mut lil_func_t;
    if ncmd.is_null() {
    ...
    let ref mut fresh4 = *ncmd.offset(fresh3 as isize);
    *fresh4 = cmd;
    return cmd;
}
#[no_mangle]
...
pub unsafe extern "C" fn lil_set_var(
    ...
    if local != 2 as libc::c_int {
            ...
            && ((*lil).callback[6 as libc::c_int as usize]).is_some()
        {
            ...
            let mut r: libc::c_int = proc_0.expect("non-null function pointer")(
                lil,
                name,
                &mut newval
            );
    ...
    nvar = realloc(
        ...
        (::core::mem::size_of::<lil_var_t>() as libc::c_ulong).wrapping_mul(
            ((*env).vars).wrapping_add(1 as libc::c_int as libc::c_ulong)
        )
    ) as *mut lil_var_t;
    if nvar.is_null() {
        return 0 as lil_var_t;
    ...
    (*env).var = nvar;
    let ref mut fresh5 = *nvar.offset((*env).vars as isize);
    *fresh5 = calloc(
        1 as libc::c_int as libc::c_ulong,
        ::core::mem::size_of::<_lil_var_t>() as libc::c_ulong
    ) as lil_var_t;
    let ref mut fresh6 = (**nvar.offset((*env).vars as isize)).n;
    *fresh6 = strclone(name);
    let ref mut fresh7 = (**nvar.offset((*env).vars as isize)).env;
    *fresh7 = env;
    let ref mut fresh8 = (**nvar.offset((*env).vars as isize)).v;
    *fresh8 = if freeval != 0 {
        val
    } else {
        lil_clone_value(val)
    ...
    return *nvar.offset(fresh9 as isize);
}
#[no_mangle]
...
pub unsafe extern "C" fn lil_get_var_or(
        ...
        && (var.is_null() || (*var).env == (*lil).rootenv)
    {
        ...
        if proc_0.expect("non-null function pointer")(lil, name, &mut newretval)
            != 0
        {
            retval = newretval;
        }
    ...
    while (*lil).head < (*lil).clen
        && (*((*lil).code).offset((*lil).head as isize) as libc::c_int
            ...
            || *((*lil).code).offset((*lil).head as isize) as libc::c_int
                == '#' as i32
            || *(*__ctype_b_loc())
                .offset(*((*lil).code).offset((*lil).head as isize)
...
unsafe extern "C" fn get_bracketpart(mut lil: lil_t) -> lil_value_t {
    ...
    while (*lil).head < (*lil).clen {
            ...
            == ']' as i32
        {
            ...
            (*lil).head = ((*lil).head).wrapping_add(1);
            lil_append_char(cmd, *((*lil).code).offset(fresh10 as isize));
        }
    }
...
unsafe extern "C" fn next_word(mut lil: lil_t) -> lil_value_t {
        ...
        == '[' as i32
    {
        ...
        || *((*lil).code).offset((*lil).head as isize) as libc::c_int
            == '\'' as i32
    {
        let fresh12 = (*lil).head;
        (*lil).head = ((*lil).head).wrapping_add(1);
        let mut sc: libc::c_char = *((*lil).code).offset(fresh12 as isize);
        val = alloc_value(0 as *const libc::c_char);
        while (*lil).head < (*lil).clen {
            if *((*lil).code).offset((*lil).head as isize) as libc::c_int
                == '[' as i32
                || *((*lil).code).offset((*lil).head as isize) as libc::c_int
                    == '$' as i32
            {
                let mut tmp: lil_value_t = if *((*lil).code)
                    .offset((*lil).head as isize)
                    as libc::c_int
                    == '$' as i32
                {
                    get_dollarpart(lil)
                ...
                if ((*cmd).proc_0).is_some() {
                        ...
                        ) == 0
                    {
                        let mut args_0: lil_value_t =
                            lil_list_to_value(words, 1 as libc::c_int);
                        lil_set_var(
                            lil,
                            b"args\0" as *const u8 as *const libc::c_char,
                            args_0,
                        ...
                        while i < (*(*cmd).argnames).c {
                            lil_set_var(
                                lil,
                                lil_to_string(
                                    *((*(*cmd).argnames).v).offset(i as isize)
                                ),
                                if i < ((*words).c).wrapping_sub(
                                    1 as libc::c_int as libc::c_ulong
                                ) {
                                    *((*words).v).offset(i.wrapping_add(
                                        1 as libc::c_int as libc::c_ulong
                                    )
                                        as isize)
                                } else {
...
unsafe extern "C" fn ee_skip_spaces(mut ee: *mut expreval_t) {
    while (*ee).head < (*ee).len
        && *(*__ctype_b_loc())
            .offset(*((*ee).code).offset((*ee).head as isize) as libc::c_int
...
unsafe extern "C" fn ee_numeric_element(mut ee: *mut expreval_t) {
    ...
    while (*ee).head < (*ee).len {
        if *((*ee).code).offset((*ee).head as isize) as libc::c_int
            == '.' as i32
        {
        ...
        if (*ee).type_0 == 0 as libc::c_int {
            (*ee).ival = (*ee).ival * 10 as libc::c_int as libc::c_long
                + (*((*ee).code).offset((*ee).head as isize) as libc::c_int
    ...
    if (*ee).type_0 == 1 as libc::c_int {
        (*ee).dval = (*ee).ival as libc::c_double
...
unsafe extern "C" fn ee_element(mut ee: *mut expreval_t) {
    if *(*__ctype_b_loc()).offset(
        *((*ee).code).offset((*ee).head as isize) as libc::c_int as isize
    ) as libc::c_int
        & _ISdigit as libc::c_int as libc::c_ushort as libc::c_int
        != 0
    {
...
unsafe extern "C" fn ee_unary(mut ee: *mut expreval_t) {
                ...
                == '!' as i32)
    {
        let fresh14 = (*ee).head;
        (*ee).head = ((*ee).head).wrapping_add(1);
        let mut op: libc::c_char = *((*ee).code).offset(fresh14 as isize);
        ee_unary(ee);
        if (*ee).error != 0 {
            return;
        }
...
unsafe extern "C" fn ee_muldiv(mut ee: *mut expreval_t) {
    ...
    while (*ee).head < (*ee).len
        && (*ee).error == 0
        && *(*__ctype_b_loc()).offset(*((*ee).code).offset(
                    ...
                    match (*ee).type_0 {
                        ...
                        0 => {
                            if (*ee).ival == 0 as libc::c_int as libc::c_long {
                                ...
                                (*ee).dval =
                                    fmod(odval, (*ee).ival as libc::c_double);
                            }
                            (*ee).type_0 = 1 as libc::c_int;
                        }
                        _ => {
                    ...
                    match (*ee).type_0 {
                        1 => {
                            if (*ee).dval == 0.0f64 {
                                ...
                                (*ee).dval =
                                    fmod(oival as libc::c_double, (*ee).dval);
                            }
                        }
                        0 => {
                            if (*ee).ival == 0 as libc::c_int as libc::c_long {
                    ...
                    match (*ee).type_0 {
                        ...
                        0 => {
                            if (*ee).ival == 0 as libc::c_int as libc::c_long {
                                ...
                                (*ee).dval =
                                    odval / (*ee).ival as libc::c_double;
                            }
                            (*ee).type_0 = 1 as libc::c_int;
                        }
                        _ => {
                    ...
                    match (*ee).type_0 {
                        1 => {
                            if (*ee).dval == 0.0f64 {
                                ...
                                (*ee).dval =
                                    oival as libc::c_double / (*ee).dval;
                            }
                        }
                        0 => {
                            if (*ee).ival == 0 as libc::c_int as libc::c_long {
                                ...
                                (*ee).dval = oival as libc::c_double
                    ...
                    match (*ee).type_0 {
                        1 => {
                            if (*ee).dval == 0.0f64 {
                                ...
                                (*ee).ival = (oival as libc::c_double
                                    / (*ee).dval)
                                    as lilint_t;
                            }
                            (*ee).type_0 = 0 as libc::c_int;
...
unsafe extern "C" fn ee_compare(mut ee: *mut expreval_t) {
                    ...
                    == '=' as i32)
    {
        ...
        {
            op = 1 as libc::c_int;
        } else if *((*ee).code).offset((*ee).head as isize) as libc::c_int
            == '>' as i32
            && *(*__ctype_b_loc()).offset(*((*ee).code).offset(
                ((*ee).head).wrapping_add(1 as libc::c_int as libc::c_ulong)
                    as isize
        ...
        (*ee).head = ((*ee).head as libc::c_ulong).wrapping_add(
            ...
            1 => match (*ee).type_0 {
                1 => {
                    ...
                    match (*ee).type_0 {
                        ...
                        0 => {
                            (*ee).ival =
                                (if odval < (*ee).ival as libc::c_double {
                                    1 as libc::c_int
                                } else {
                                    0 as libc::c_int
                                }) as lilint_t;
                ...
                0 => {
                    ...
                    match (*ee).type_0 {
                        1 => {
                            (*ee).ival =
                                (if (oival as libc::c_double) < (*ee).dval {
                                    1 as libc::c_int
                                } else {
                                    0 as libc::c_int
                                }) as lilint_t;
...
pub unsafe extern "C" fn lil_unused_name(
    ...
    let mut name: *mut libc::c_char =
        malloc((strlen(part)).wrapping_add(64 as libc::c_int as libc::c_ulong))
            as *mut libc::c_char;
    let mut val: lil_value_t = 0 as *mut _lil_value_t;
    let mut i: size_t = 0;
    i = 0 as libc::c_int as size_t;
...
unsafe extern "C" fn fnc_reflect(
    ...
    if strcmp(type_0, b"globals\0" as *const u8 as *const libc::c_char) == 0 {
        ...
        while i < (*(*lil).rootenv).vars {
            lil_list_append(
                vars_0,
                lil_alloc_string(
                    (**((*(*lil).rootenv).var).offset(i as isize)).n
                )
            );
            i = i.wrapping_add(1);
            i;
    ...
    if strcmp(type_0, b"has-var\0" as *const u8 as *const libc::c_char) == 0 {
        ...
        while !env_0.is_null() {
            i = 0 as libc::c_int as size_t;
            while i < (*env_0).vars {
                if strcmp(target_0, (**((*env_0).var).offset(i as isize)).n)
                    == 0
                {
                    return lil_alloc_string(
                        b"1\0" as *const u8 as *const libc::c_char
    ...
    ) == 0
    {
        ...
        free((*lil).dollarprefix as *mut libc::c_void);
        (*lil).dollarprefix =
            strclone(lil_to_string(*argv.offset(1 as libc::c_int as isize)));
        return r_0;
    }
...
unsafe extern "C" fn fnc_func(
    ...
    if argc == 3 as libc::c_int as libc::c_ulong {
        ...
        (*cmd).argnames =
            lil_subst_to_list(lil, *argv.offset(1 as libc::c_int as isize));
        ...
        if argc < 2 as libc::c_int as libc::c_ulong {
            let mut tmp: lil_value_t =
                lil_alloc_string(b"args\0" as *const u8 as *const libc::c_char);
            ...
            (*cmd).argnames =
                lil_subst_to_list(lil, *argv.offset(0 as libc::c_int as isize));
            (*cmd).code =
                lil_clone_value(*argv.offset(1 as libc::c_int as isize));
        }
    }
    return name;
}
...
unsafe extern "C" fn fnc_enveval(
    ...
    if argc == 1 as libc::c_int as libc::c_ulong {
        ...
        while i < lil_list_size(invars) {
            ...
            *fresh15 = lil_clone_value(lil_get_var(
                lil,
    ...
    if !invars.is_null() || !outvars.is_null() {
        if !outvars.is_null() {
            ...
            while i < lil_list_size(outvars) {
                ...
                *fresh16 = lil_clone_value(lil_get_var(
                    lil,
            ...
            while i < lil_list_size(invars) {
                ...
                *fresh17 = lil_clone_value(lil_get_var(
                    lil,
                    lil_to_string(lil_list_get(invars, i))
                ));
                i = i.wrapping_add(1);
...
unsafe extern "C" fn real_inc(
    ...
    if fmod(dv, 1 as libc::c_int as libc::c_double) != 0. {
        ...
        pv = lil_alloc_integer(
            (lil_to_integer(pv) as libc::c_float + v) as lilint_t
        );
    }
...
unsafe extern "C" fn fnc_if(
    ...
    if v != 0 {
        ...
        > (base as size_t).wrapping_add(2 as libc::c_int as libc::c_ulong)
    {
        r = lil_parse_value(
            lil,
...
unsafe extern "C" fn fnc_substr(
    ...
    slen = strlen(str);
    start =
        atoll(lil_to_string(*argv.offset(1 as libc::c_int as isize))) as size_t;
...
unsafe extern "C" fn fnc_strpos(
    ...
    if argc > 2 as libc::c_int as libc::c_ulong {
        min = atoll(lil_to_string(*argv.offset(2 as libc::c_int as isize)))
            as size_t;
        if min >= strlen(hay) {
            return lil_alloc_integer(-(1 as libc::c_int) as lilint_t);
        }
...
unsafe extern "C" fn fnc_length(
    ...
    while i < argc {
        ...
        total = (total as libc::c_ulong)
            .wrapping_add(strlen(lil_to_string(*argv.offset(i as isize))))
            as size_t as size_t;
        i = i.wrapping_add(1);
        i;
    }
...
unsafe extern "C" fn real_trim(
    ...
    if left != 0 {
        while *str.offset(base as isize) as libc::c_int != 0
            && !(strchr(chars, *str.offset(base as isize) as libc::c_int))
                .is_null()
        {
            base += 1;
            base;
    ...
    if right != 0 {
        ...
        while len != 0
            && !(strchr(
                chars,
                *s.offset(len.wrapping_sub(1 as libc::c_int as libc::c_ulong)
                    as isize) as libc::c_int
            ))
            .is_null()
        {
...
unsafe extern "C" fn fnc_catcher(
    ...
    if argc == 0 as libc::c_int as libc::c_ulong {
        ...
        let mut catcher: *const libc::c_char =
            lil_to_string(*argv.offset(0 as libc::c_int as isize));
        free((*lil).catcher as *mut libc::c_void);
        (*lil).catcher =
