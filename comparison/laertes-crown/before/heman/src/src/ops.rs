...
pub unsafe extern "C" fn heman_ops_step(
    ...
    if (*hmap).nbands == 1 as std::os::raw::c_int {} else {
        __assert_fail(
            ...
            (*::std::mem::transmute::<
                &[u8; 50],
                &[std::os::raw::c_char; 50],
            >(b"heman_image *heman_ops_step(heman_image *, float)\0"))
                .as_ptr(),
        );
    }
    let mut result = heman_image_create((*hmap).width, (*hmap).height, 1 as std::os::raw::c_int);
...
pub unsafe extern "C" fn heman_ops_sweep(
    ...
    let mut dst = (*result).data;
    let mut src: *const std::os::raw::c_float = (*hmap).data;
    let mut invw = 1.0f32 / (*hmap).width as std::os::raw::c_float;
...
pub unsafe extern "C" fn heman_ops_stitch_vertical(
    ...
    while tile < count {
        memcpy(
            ...
            (**images.offset(tile as isize)).data as *const std::os::raw::c_void,
...
pub unsafe extern "C" fn heman_ops_sobel(
    ...
    if (*img).nbands == 3 as std::os::raw::c_int {} else {
        __assert_fail(
...
pub unsafe extern "C" fn heman_ops_replace_color(
    ...
    if (*texture).nbands == 3 as std::os::raw::c_int {} else {
        __assert_fail(
    ...
    while y < height {
        ...
        let mut src = ((*source).data).offset((y * width * 3 as std::os::raw::c_int) as isize);
        let mut tex = ((*texture).data).offset((y * width * 3 as std::os::raw::c_int) as isize);
        let mut x = 0 as std::os::raw::c_int;
        while x < width {
...
unsafe extern "C" fn _match(
    ...
    let mut mcolor = ((*mask).data).offset((pixel_index * 3 as std::os::raw::c_int) as isize);
...
pub unsafe extern "C" fn heman_ops_percentiles(
    ...
    if mask.is_null() || (*mask).nbands == 3 as std::os::raw::c_int {} else {
        __assert_fail(
...
pub unsafe extern "C" fn heman_ops_merge_political(
    ...
    if (*cmap).nbands == 3 as std::os::raw::c_int {} else {
        __assert_fail(
    ...
    let mut result = heman_image_create((*hmap).width, (*hmap).height, 4 as std::os::raw::c_int);
