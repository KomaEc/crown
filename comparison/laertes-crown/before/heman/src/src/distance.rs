...
#[no_mangle]
pub static mut INF: std::os::raw::c_float = 1E20f64 as std::os::raw::c_float;
unsafe extern "C" fn edt(
    ...
    let mut s: std::os::raw::c_float = 0.;
...
unsafe extern "C" fn transform_to_distance(mut sdf: *mut heman_image) {
    ...
    let mut y_1: std::os::raw::c_int = 0;
...
pub unsafe extern "C" fn heman_distance_from_cpcf(
    ...
    if (*cf).nbands == 2 as std::os::raw::c_int
    ...
    let mut udf = heman_image_create((*cf).width, (*cf).height, 1 as std::os::raw::c_int);
    let mut dptr = (*udf).data;
    let mut sptr = (*cf).data;
    let mut scale = (1.0f32 as std::os::raw::c_double
        / sqrt(
            ((*cf).width * (*cf).width + (*cf).height * (*cf).height) as std::os::raw::c_double,
        )) as std::os::raw::c_float;
    let mut y = 0 as std::os::raw::c_int;
    while y < (*cf).height {
        let mut x = 0 as std::os::raw::c_int;
