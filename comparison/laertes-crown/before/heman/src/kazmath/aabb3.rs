...
pub unsafe extern "C" fn kmAABB3Initialize(
    ...
    (*pBox).min.x = (*point).x - width / 2 as std::os::raw::c_int as std::os::raw::c_float;
    (*pBox).min.y = (*point).y - height / 2 as std::os::raw::c_int as std::os::raw::c_float;
    (*pBox).min.z = (*point).z - depth / 2 as std::os::raw::c_int as std::os::raw::c_float;
    (*pBox).max.x = (*point).x + width / 2 as std::os::raw::c_int as std::os::raw::c_float;
    (*pBox).max.y = (*point).y + height / 2 as std::os::raw::c_int as std::os::raw::c_float;
    (*pBox).max.z = (*point).z + depth / 2 as std::os::raw::c_int as std::os::raw::c_float;
    return pBox;
}
#[no_mangle]
...
pub unsafe extern "C" fn kmAABB3IntersectsAABB(
    ...
    return (kmAABB3ContainsAABB(box_0, other) != 0 as std::os::raw::c_int as std::os::raw::c_uint)
...
pub unsafe extern "C" fn kmAABB3ContainsAABB(
    ...
    kmVec3Fill(
        &mut *corners.as_mut_ptr().offset(0 as std::os::raw::c_int as isize),
        (*to_check).min.x,
        (*to_check).min.y,
        (*to_check).min.z,
    );
    kmVec3Fill(
        &mut *corners.as_mut_ptr().offset(1 as std::os::raw::c_int as isize),
        (*to_check).max.x,
        (*to_check).min.y,
        (*to_check).min.z,
    );
    kmVec3Fill(
        &mut *corners.as_mut_ptr().offset(2 as std::os::raw::c_int as isize),
        (*to_check).max.x,
        (*to_check).max.y,
        (*to_check).min.z,
    );
    kmVec3Fill(
        &mut *corners.as_mut_ptr().offset(3 as std::os::raw::c_int as isize),
        (*to_check).min.x,
        (*to_check).max.y,
        (*to_check).min.z,
    );
    kmVec3Fill(
        &mut *corners.as_mut_ptr().offset(4 as std::os::raw::c_int as isize),
        (*to_check).min.x,
        (*to_check).min.y,
        (*to_check).max.z,
    );
    kmVec3Fill(
        &mut *corners.as_mut_ptr().offset(5 as std::os::raw::c_int as isize),
        (*to_check).max.x,
        (*to_check).min.y,
        (*to_check).max.z,
    );
    kmVec3Fill(
        &mut *corners.as_mut_ptr().offset(6 as std::os::raw::c_int as isize),
        (*to_check).max.x,
        (*to_check).max.y,
        (*to_check).max.z,
    );
    kmVec3Fill(
        &mut *corners.as_mut_ptr().offset(7 as std::os::raw::c_int as isize),
        (*to_check).min.x,
        (*to_check).max.y,
        (*to_check).max.z,
    );
...
#[no_mangle]
pub unsafe extern "C" fn kmAABB3DiameterX(mut aabb: *const kmAABB3) -> std::os::raw::c_float {
    return fabs(((*aabb).max.x - (*aabb).min.x) as std::os::raw::c_double) as std::os::raw::c_float;
}
#[no_mangle]
pub unsafe extern "C" fn kmAABB3DiameterY(mut aabb: *const kmAABB3) -> std::os::raw::c_float {
    return fabs(((*aabb).max.y - (*aabb).min.y) as std::os::raw::c_double) as std::os::raw::c_float;
}
#[no_mangle]
pub unsafe extern "C" fn kmAABB3DiameterZ(mut aabb: *const kmAABB3) -> std::os::raw::c_float {
    return fabs(((*aabb).max.z - (*aabb).min.z) as std::os::raw::c_double) as std::os::raw::c_float;
}
#[no_mangle]
