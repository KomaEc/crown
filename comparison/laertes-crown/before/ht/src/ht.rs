...
unsafe extern "C" fn hash_key(mut key: *const std::os::raw::c_char) -> uint64_t {
...
pub unsafe extern "C" fn ht_get(
    ...
    let mut index = hash
        & ((*table).capacity).wrapping_sub(1 as std::os::raw::c_int as std::os::raw::c_ulong);
...
pub unsafe extern "C" fn ht_set(
    ...
    if (*table).length
        >= ((*table).capacity).wrapping_div(2 as std::os::raw::c_int as std::os::raw::c_ulong)
    {
        if !ht_expand(table) {
            return 0 as *const std::os::raw::c_char;
        }
...
pub unsafe extern "C" fn ht_iterator(mut table: *mut ht) -> hti {
    let mut it = hti {
        ...
        value: 0 as *mut std::os::raw::c_void,
        _table: 0 as *mut ht,
        _index: 0,
    };
    it._table = table;
