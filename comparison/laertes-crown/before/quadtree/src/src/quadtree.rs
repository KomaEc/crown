...
extern "C" {
    ...
    fn quadtree_node_reset(
        ...
        key_free: Option::<unsafe extern "C" fn(*mut std::os::raw::c_void) -> ()>,
    );
    ...
    fn quadtree_node_free(
        node: *mut quadtree_node_t,
        value_free: Option::<unsafe extern "C" fn(*mut std::os::raw::c_void) -> ()>,
    );
...
unsafe extern "C" fn elision_(mut key: *mut std::os::raw::c_void) {}
unsafe extern "C" fn reset_node_(
    ...
    if ((*tree).key_free).is_some() {
        ...
        quadtree_node_reset(
            node,
            Some(elision_ as unsafe extern "C" fn(*mut std::os::raw::c_void) -> ()),
        );
    };
}
