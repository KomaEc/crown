...
extern "C" {
    ...
    fn fopen(__filename: *const std::os::raw::c_char, __modes: *const std::os::raw::c_char) -> *mut FILE;
    fn fputs(__s: *const std::os::raw::c_char, __stream: *mut FILE) -> std::os::raw::c_int;
    fn _IO_getc(__fp: *mut _IO_FILE) -> std::os::raw::c_int;
    fn _IO_putc(__c: std::os::raw::c_int, __fp: *mut _IO_FILE) -> std::os::raw::c_int;
    ...
    fn glob(
        ...
        __errfunc: Option::<
            unsafe extern "C" fn(*const std::os::raw::c_char, std::os::raw::c_int) -> std::os::raw::c_int,
        >,
        __pglob: *mut glob_t,
    ) -> std::os::raw::c_int;
    ...
    fn uname(__name: *mut utsname) -> std::os::raw::c_int;
...
pub struct glob_t {
    ...
    pub gl_closedir: Option::<unsafe extern "C" fn(*mut std::os::raw::c_void) -> ()>,
    ...
    >,
    pub gl_lstat: Option::<
        unsafe extern "C" fn(*const std::os::raw::c_char, *mut std::os::raw::c_void) -> std::os::raw::c_int,
    >,
    pub gl_stat: Option::<
        unsafe extern "C" fn(*const std::os::raw::c_char, *mut std::os::raw::c_void) -> std::os::raw::c_int,
    >,
...
#[no_mangle]
pub static mut exclude_list: [*const std::os::raw::c_char; 14] = [
...
#[inline]
unsafe extern "C" fn utoa(mut str: *mut std::os::raw::c_char, mut v: size_t) {
    let mut p = str;
    loop {
        let fresh0 = p;
        p = p.offset(1);
...
unsafe extern "C" fn small_vec_u64_free(mut v: *mut small_vec_u64_t) {
    ...
    free((*v).p as *mut std::os::raw::c_void);
...
unsafe extern "C" fn string_table_maybe_grow(mut t: *mut string_table_t, mut n: size_t) {
    ...
    let mut arr = realloc(
        (*t).arr as *mut std::os::raw::c_void,
        ((*t).capacity)
...
unsafe extern "C" fn string_table_store(
    ...
    memcpy(
        ((*t).arr).offset((*t).n as isize) as *mut std::os::raw::c_void,
...
unsafe extern "C" fn string_table_copy_from_file(
    ...
    loop {
        ...
        string_table_maybe_grow(t, 1 as std::os::raw::c_int as size_t);
        let ref mut fresh12 = (*t).n;
        let fresh13 = *fresh12;
        *fresh12 = (*fresh12).wrapping_add(1);
        *((*t).arr).offset(fresh13 as isize) = c as std::os::raw::c_char;
    }
    string_table_maybe_grow(t, 1 as std::os::raw::c_int as size_t);
    let ref mut fresh14 = (*t).n;
    let fresh15 = *fresh14;
    *fresh14 = (*fresh14).wrapping_add(1);
    *((*t).arr).offset(fresh15 as isize) = '\0' as i32 as std::os::raw::c_char;
}
    ...
    while i < *needed_not_found {
        ...
        ) != 0
        {
            ...
            *((*needed_buf_offsets).p)
                ...
                .offset(
                    (*needed_not_found).wrapping_sub(1 as std::os::raw::c_int as std::os::raw::c_ulong)
                        as isize,
                );
...
unsafe extern "C" fn check_absolute_paths(
    ...
    while i < *needed_not_found {
            ...
            .is_null()
        {
            ...
            let mut path: [std::os::raw::c_char; 4096] = [0; 4096];
            let mut len = strlen(
                ((*st).arr)
                    .offset(*((*needed_buf_offsets).p).offset(i as isize) as isize),
            );
            ...
            (*s)
                ...
                as usize] = (*needed_not_found <= 1 as std::os::raw::c_int as std::os::raw::c_ulong)
                as std::os::raw::c_int as std::os::raw::c_char;
            let mut err = 0 as *mut std::os::raw::c_char;
...
unsafe extern "C" fn check_search_paths(
    ...
    let mut st: *const string_table_t = &mut (*s).string_table;
    while *((*st).arr).offset(offset as isize) as std::os::raw::c_int != '\0' as i32 {
        while *((*st).arr).offset(offset as isize) as std::os::raw::c_int == ':' as i32
            && *((*st).arr).offset(offset as isize) as std::os::raw::c_int != '\0' as i32
        {
            offset = offset.wrapping_add(1);
        }
        if *((*st).arr).offset(offset as isize) as std::os::raw::c_int == '\0' as i32 {
            return exit_code;
        }
        let mut dest = path.as_mut_ptr();
        while *((*st).arr).offset(offset as isize) as std::os::raw::c_int != '\0' as i32
            && *((*st).arr).offset(offset as isize) as std::os::raw::c_int != ':' as i32
            && dest != path_end
        {
            let fresh16 = offset;
            offset = offset.wrapping_add(1);
...
unsafe extern "C" fn interpolate_variables(
    ...
    loop {
        ...
        curr_src = dollar.offset_from((*st).arr) as std::os::raw::c_long as size_t;
        ...
        if *((*st).arr).offset(curr_src as isize) as std::os::raw::c_int == '{' as i32 {
        ...
        memcpy(
            &mut *((*st).arr).offset((*s).string_table.n as isize) as *mut std::os::raw::c_char
                as *mut std::os::raw::c_void,
            &mut *((*st).arr).offset(prev_src as isize) as *mut std::os::raw::c_char
                as *const std::os::raw::c_void,
            bytes_to_dollar,
        );
    ...
    if prev_src != src {
        ...
        memcpy(
            ((*st).arr).offset((*st).n as isize) as *mut std::os::raw::c_void,
            ((*st).arr).offset(prev_src as isize) as *const std::os::raw::c_void,
            n,
        );
...
unsafe extern "C" fn print_line(
    ...
    let mut conf_name: *mut std::os::raw::c_char = 0 as *mut std::os::raw::c_char;
    match reason.how as std::os::raw::c_uint {
        2 => {
            if (reason.depth).wrapping_add(1 as std::os::raw::c_int as std::os::raw::c_ulong) >= depth {
                ...
                let mut num: [std::os::raw::c_char; 8] = [0; 8];
                utoa(
                    num.as_mut_ptr(),
...
unsafe extern "C" fn visited_files_contains(
    ...
    let mut i = 0 as std::os::raw::c_int as size_t;
    while i < (*files).n {
        let mut f: *mut visited_file_t = &mut *((*files).arr).offset(i as isize)
...
unsafe extern "C" fn recurse(
    ...
    let mut e_ident: [std::os::raw::c_char; 16] = [0; 16];
    if fread(
    ...
    let mut origin: [std::os::raw::c_char; 4096] = [0; 4096];
...
unsafe extern "C" fn parse_ld_config_file(
    ...
    let mut line: [std::os::raw::c_char; 4096] = [0; 4096];
    let mut tmp: [std::os::raw::c_char; 4096] = [0; 4096];
    while c != -(1 as std::os::raw::c_int) {
        ...
        *end.offset(1 as std::os::raw::c_int as isize) = '\0' as i32 as std::os::raw::c_char;
                ...
                != 0
        {
            ...
            *((*st).arr)
                .offset(
                    ((*st).n).wrapping_sub(1 as std::os::raw::c_int as std::os::raw::c_ulong) as isize,
...
unsafe extern "C" fn parse_ld_so_conf(mut s: *mut libtree_state_t) {
    ...
    if (*st).n > (*s).ld_so_conf_offset {
        *((*st).arr)
        ...
        string_table_store(st, b"\0" as *const u8 as *const std::os::raw::c_char);
    };
}
