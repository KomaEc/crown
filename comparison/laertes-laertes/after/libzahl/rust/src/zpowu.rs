...
#[no_mangle]
pub unsafe extern "C" fn zpowu(mut a: *mut zahl, mut b: *mut zahl,
                               mut c: std::os::raw::c_ulonglong) {
    let mut neg: std::os::raw::c_int = 0;
    if (c == 0) as std::os::raw::c_int as std::os::raw::c_long != 0 {
    ...
    neg =
        (zsignum(b) < 0 as std::os::raw::c_int &&
             c & 1 as std::os::raw::c_int as std::os::raw::c_ulonglong != 0) as std::os::raw::c_int;
    zabs(libzahl_tmp_pow_b.as_mut_ptr(), b);
    if c & 1 as std::os::raw::c_int as std::os::raw::c_ulonglong != 0 {
        zset(a, libzahl_tmp_pow_b.as_mut_ptr());
    } else { zsetu(a, 1 as std::os::raw::c_int as uint64_t); }
    loop  {
