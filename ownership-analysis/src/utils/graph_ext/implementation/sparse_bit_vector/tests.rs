...
fn test_rev1() {
    ...
    let graph: SparseBitVectorGraph<usize> =
        SparseBitVectorGraph::new(4, edges.clone().into_iter());
    let graph = graph.reverse();
    edges
        .iter_mut()
        .for_each(|(src, dest)| std::mem::swap(src, dest));
    edges.sort_by(cmp);
...
fn test_rev3() {
    ...
    let graph: SparseBitVectorGraph<usize> =
        SparseBitVectorGraph::new(5, edges.clone().into_iter());
    let graph = graph.reverse();
    edges
        .iter_mut()
        .for_each(|(src, dest)| std::mem::swap(src, dest));
