...
fn test1() {
    ...
    orc_common::test::run_compiler_with(PROGRAM.into(), |tcx, functions, structs| {
        ...
        let f = steensgaard.struct_fields.get_content(s, 0);
        let g = steensgaard.struct_fields.get_content(s, 1);
...
fn test2() {
    ...
    orc_common::test::run_compiler_with(PROGRAM.into(), |tcx, functions, structs| {
        ...
        let pts = &steensgaard.pts;
        let f = input.1[0];
        let local = steensgaard.function_locals.get_content(f, 1);
        let p = steensgaard.function_locals.get_content(f, 2);
        assert_ne!(local, p);
