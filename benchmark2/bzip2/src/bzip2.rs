...
unsafe extern "C" fn mkCell() -> *mut Cell {
    ...
    let ref mut fresh0 = (*c).name;
    *fresh0 = 0 as *mut Char;
    let ref mut fresh1 = (*c).link;
    *fresh1 = 0 as *mut zzzz;
    return c;
}
unsafe extern "C" fn snocString(mut root: *mut Cell, mut name: *mut Char) -> *mut Cell {
    if root.is_null() {
        let mut tmp = mkCell();
        let ref mut fresh2 = (*tmp).name;
        *fresh2 = myMalloc(
        ...
        let ref mut fresh3 = (*tmp_0).link;
        *fresh3 = snocString((*tmp_0).link, name);
        return root;
    };
}
