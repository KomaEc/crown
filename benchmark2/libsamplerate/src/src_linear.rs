    ...
    *fresh0 = 0 as libc::c_int as libc::c_long;
    ...
    while input_index < 1.0f64 && (*priv_0).out_gen < (*priv_0).out_count {
        if (*priv_0).in_used as libc::c_double
            + (*state).channels as libc::c_double * (1.0f64 + input_index)
        ...
        while ch < (*state).channels {
            *((*data).data_out)
                ...
                + input_index
                    * (*((*data).data_in).offset(ch as isize) as libc::c_double
                        - *((*priv_0).last_value).offset(ch as isize) as libc::c_double))
                as libc::c_float;
...
unsafe extern "C" fn linear_data_new(mut channels: libc::c_int) -> *mut LINEAR_DATA {
    ...
    if !priv_0.is_null() {
        (*priv_0)
            .linear_magic_marker = 'l' as i32 + (('i' as i32) << 4 as libc::c_int)
...
pub unsafe extern "C" fn linear_state_new(
    ...
    *fresh4 = linear_data_new((*state).channels) as *mut libc::c_void;
    if ((*state).private_data).is_null() {
    ...
    if !state.is_null() {
        ...
        if !linear.is_null() {
            if !((*linear).last_value).is_null() {
                ...
                let ref mut fresh8 = (*linear).last_value;
