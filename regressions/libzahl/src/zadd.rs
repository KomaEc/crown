...
impl Default for OrcGeneratedXXX1 {
    fn default() -> Self {
        Self {}
    }
}
...
pub unsafe extern "C" fn zadd_unsigned(
    ...
    let mut n: size_t = 0;
    let mut carry: [uint32_t; 2] = [0 as libc::c_int as uint32_t, 0 as libc::c_int as uint32_t];
    ...
    *(*a.as_deref().unwrap()).chars.offset(size as isize) = 0 as libc::c_int as zahl_char_t;
        ...
        if (*a.as_deref().unwrap()).used < (*c).used {
            ...
            memset(
                (*a.as_deref().unwrap())
                    ...
                    as *mut libc::c_void,
                0 as libc::c_int,
                n.wrapping_sub((*a.as_deref().unwrap()).used)
    ...
    while i < n {
        ...
        *(*a.as_deref().unwrap()).chars.offset(i as isize) =
            (*(*a.as_deref().unwrap()).chars.offset(i as isize) as libc::c_uint).wrapping_add(
                (*addend.offset(i as isize))
                    .wrapping_add(carry[(i & 1 as libc::c_int as libc::c_ulong) as usize]),
            ) as zahl_char_t as zahl_char_t;
        i = i.wrapping_add(1);
    }
    while carry[(i & 1 as libc::c_int as libc::c_ulong) as usize] != 0 {
        carry[(!i & 1 as libc::c_int as libc::c_ulong) as usize] =
            (*(*a.as_deref().unwrap()).chars.offset(i as isize) == 4294967295 as libc::c_uint)
                as libc::c_int as uint32_t;
        let fresh1 = i;
        i = i.wrapping_add(1);
        *(*a.as_deref().unwrap()).chars.offset(fresh1 as isize) =
            (*(*a.as_deref().unwrap()).chars.offset(fresh1 as isize) as libc::c_uint)
