...
impl Default for ErasedByPreprocessor53 {
    fn default() -> Self {
        Self {}
    }
}
...
impl Default for ErasedByPreprocessor54 {
    fn default() -> Self {
        Self {}
    }
}
...
impl Default for ErasedByPreprocessor55 {
    fn default() -> Self {
        Self {}
    }
}
...
pub unsafe extern "C" fn zstr(
    ...
    if b.is_null() {
        ();
        b = malloc(n.wrapping_add(1 as libc::c_int as libc::c_ulong)) as *mut libc::c_char;
        if b.is_null() {
            ();
            libzahl_error = *__errno_location();
            longjmp(libzahl_jmp_buf.as_mut_ptr(), 1 as libc::c_int);
        ...
        if zzero(libzahl_tmp_str_num.as_mut_ptr()) == 0 {
            sprintf(
                ...
                if zzero(libzahl_tmp_str_rem.as_mut_ptr()) != 0 {
                    ...
                    *((*libzahl_tmp_str_rem.as_mut_ptr()).chars).offset(0 as libc::c_int as isize)
                        as libc::c_ulong
                },
            );
            *b.offset(n.wrapping_add(9 as libc::c_int as libc::c_ulong) as isize) = overridden;
