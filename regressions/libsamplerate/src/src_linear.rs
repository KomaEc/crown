...
impl Default for LINEAR_DATA {fn default() -> Self {Self {
linear_magic_marker: Default::default(),
...
impl LINEAR_DATA {pub fn take(&mut self) -> Self {core::mem::take(self)}}
...
unsafe extern "C" fn linear_vari_process(
    ...
    let mut priv_0 = 0 as *mut LINEAR_DATA;
    ...
    priv_0= (*state.as_deref().unwrap()).private_data as *mut LINEAR_DATA;
    if !(*priv_0).dirty {
        ch= 0 as libc::c_int;
    ...
    while input_index < 1.0f64 && (*priv_0).out_gen < (*priv_0).out_count {
        if (*priv_0).in_used as libc::c_double
            + (*state.as_deref().unwrap()).channels as libc::c_double * (1.0f64 + input_index)
        ...
        while ch < (*state.as_deref().unwrap()).channels {
            *(*data.as_deref().unwrap()).data_out
                ...
                ) = (*(*priv_0).last_value.offset(ch as isize) as libc::c_double
                + input_index
                    * (*(*data.as_deref().unwrap()).data_in.offset(ch as isize) as libc::c_double
                        - *(*priv_0).last_value.offset(ch as isize) as libc::c_double))
                as libc::c_float;
            (*priv_0).out_gen+= 1;
            ch+= 1;
...
pub unsafe extern "C" fn linear_get_name(
    ...
    if src_enum == SRC_LINEAR as libc::c_int {
        return b"Linear Interpolator\0" as *const u8 as *const libc::c_char;
    }
...
unsafe extern "C" fn linear_data_new(mut channels: libc::c_int) -> *mut /* owning */ LINEAR_DATA {
    let mut priv_0 = calloc(
        1 as libc::c_int as libc::c_ulong,
        ::std::mem::size_of::<LINEAR_DATA>() as libc::c_ulong,
    ) as *mut LINEAR_DATA;
    if !priv_0.is_null() {
        (*priv_0).linear_magic_marker= 'l' as i32 + (('i' as i32) << 4 as libc::c_int)
            + (('n' as i32) << 8 as libc::c_int) + (('e' as i32) << 12 as libc::c_int)
            + (('a' as i32) << 16 as libc::c_int) + (('r' as i32) << 20 as libc::c_int);
...
pub unsafe extern "C" fn linear_state_new(
    ...
    (*state).private_data= linear_data_new((*state).channels) as *mut libc::c_void;
    if (*state).private_data.is_null() {();
        free(state as *mut libc::c_void);
        *error.as_deref_mut().unwrap()= SRC_ERR_MALLOC_FAILED;
        return 0 as *mut SRC_STATE;
    }
    (*state).vt= core::ptr::addr_of_mut!(crate::src::src_linear::linear_state_vt);
    linear_reset(state.as_mut());
    *error.as_deref_mut().unwrap()= SRC_ERR_NO_ERROR;
    return state;
}
unsafe extern "C" fn linear_reset(mut state: Option<&mut SRC_STATE>) {
    let mut priv_0 = 0 as *mut LINEAR_DATA;
    priv_0= (*state.as_deref().unwrap()).private_data as *mut LINEAR_DATA;
    if priv_0.is_null() {();
        return;
...
unsafe extern "C" fn linear_copy(mut state: *mut SRC_STATE) -> *mut /* owning */ SRC_STATE {
    if (*state).private_data.is_null() {();
        return 0 as *mut SRC_STATE;
    }
    let mut to = calloc(
    ...
    let mut from_priv = (*state).private_data as *mut LINEAR_DATA;
...
unsafe extern "C" fn linear_close(mut state: *mut /* owning */ SRC_STATE) {
    if !state.is_null() {
        let mut linear = (*state).private_data as *mut LINEAR_DATA;
        if !linear.is_null() {
            if !(*linear).last_value.is_null() {
                free((*linear).last_value as *mut libc::c_void);
                (*linear).last_value= 0 as *mut libc::c_float;
            }else { (); }
            free(linear as *mut libc::c_void);
