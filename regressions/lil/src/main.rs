...
unsafe extern "C" fn do_exit(mut lil: lil_t, mut val: lil_value_t) {
    ...
    exit_code = crate::src::lil::lil_to_integer(
        val.as_deref_mut()
            .map(|r| r as *mut _)
...
unsafe extern "C" fn do_system(
    ...
    while i < argc {
        ...
        cmdlen = (cmdlen as libc::c_ulong).wrapping_add(len) as libc::c_int as libc::c_int;
        i = i.wrapping_add(1);
    }
    cmd = realloc(
        cmd as *mut libc::c_void,
    ...
    *cmd.offset(cmdlen as isize) = 0 as libc::c_int as libc::c_char;
    ...
    if !p.is_null() {
        ...
        loop {
            ...
            size = (size as libc::c_ulong).wrapping_add(bytes as libc::c_ulong) as size_t as size_t;
        }
        retval = realloc(
            retval as *mut libc::c_void,
...
unsafe extern "C" fn fnc_readline(
    ...
    loop {
        ...
        if len < size {
            size = (size as libc::c_ulong).wrapping_add(64 as libc::c_int as libc::c_ulong)
                as size_t as size_t;
...
unsafe extern "C" fn repl() -> libc::c_int {
    ...
    while running != 0 {
        ...
        strres = crate::src::lil::lil_to_string(
            result
                .as_deref_mut()
                .map(|r| r as *mut _)
                .unwrap_or(std::ptr::null_mut()),
        ...
        if crate::src::lil::lil_error(lil.as_deref_mut(), Some(&mut err_msg), Some(&mut pos)) != 0 {
            printf(
                b"error at %i: %s\n\0" as *const u8 as *const libc::c_char,
                pos as libc::c_int,
                err_msg,
...
unsafe extern "C" fn nonint(
    ...
    i = 2 as libc::c_int;
    while i < argc {
        crate::src::lil::lil_list_append(
            arglist
                .as_deref_mut()
                .map(|r| r as *mut _)
                .unwrap_or(std::ptr::null_mut()),
            crate::src::lil::lil_alloc_string(*argv.offset(i as isize)),
        );
        i += 1;
    }
    args = crate::src::lil::lil_list_to_value(
        arglist
            .as_deref_mut()
            .map(|r| r as *mut _)
            .unwrap_or(std::ptr::null_mut()),
    ...
    tmpcode = malloc((strlen(filename)).wrapping_add(256 as libc::c_int as libc::c_ulong))
        as *mut libc::c_char;
    sprintf(
        tmpcode,
