...
unsafe fn main_0(mut argc: std::os::raw::c_int, mut argv: *mut *mut std::os::raw::c_char)
 -> std::os::raw::c_int {
    let mut info: *const crate::example2::ti_indicator_info = crate::sample::ti_indicators.as_mut_ptr();
    ...
    info= crate::indicators_index::ti_find_indicator(*argv.offset(1 as std::os::raw::c_int as isize));
    if info.is_null() {();
    ...
    let mut outputs: [*mut std::os::raw::c_double; 5] =
        [crate::sample::out[0 as std::os::raw::c_int as usize].as_mut_ptr(),
         crate::sample::out[1 as std::os::raw::c_int as usize].as_mut_ptr(),
         crate::sample::out[2 as std::os::raw::c_int as usize].as_mut_ptr(),
         crate::sample::out[3 as std::os::raw::c_int as usize].as_mut_ptr(),
         crate::sample::out[4 as std::os::raw::c_int as usize].as_mut_ptr()];
    ...
    while !(*alt).is_null() {
        if strcmp((*alt), (*info).name) == 0 as std::os::raw::c_int {
            ...
            j= 0 as std::os::raw::c_int;
            while j < (*info).inputs {
                inputs[j as usize]= crate::sample::alternative.as_mut_ptr();
                j+= 1
            }
            break ;
    ...
    if ret == 0 as std::os::raw::c_int {
        ...
        while i_1 < 15 as std::os::raw::c_int {
            printf(b"%s\x00" as *const u8 as *const std::os::raw::c_char,
                   crate::sample::datet[i_1 as usize]);
            if o != 0 {
                printf(b" %8.2f\x00" as *const u8 as *const std::os::raw::c_char,
                       crate::sample::open[i_1 as usize]);
            }
            if h != 0 {
                printf(b" %8.2f\x00" as *const u8 as *const std::os::raw::c_char,
                       crate::sample::high[i_1 as usize]);
            }
            if l != 0 {
                printf(b" %8.2f\x00" as *const u8 as *const std::os::raw::c_char,
                       crate::sample::low[i_1 as usize]);
            }
            if c != 0 {
                printf(b" %8.2f\x00" as *const u8 as *const std::os::raw::c_char,
                       crate::sample::close[i_1 as usize]);
            }
            if v != 0 {
                printf(b" %8.0f\x00" as *const u8 as *const std::os::raw::c_char,
                       crate::sample::volume[i_1 as usize]);
            }
            if r != 0 {
                printf(b" %8.2f\x00" as *const u8 as *const std::os::raw::c_char,
                       crate::sample::close[i_1 as usize]);
            ...
            if a != 0 {
                printf(b" %8.2f\x00" as *const u8 as *const std::os::raw::c_char,
                       crate::sample::alternative[i_1 as usize]);
            }
            if i_1 >= start {
                k= 0 as std::os::raw::c_int;
                while k < (*info).outputs {
                    if crate::sample::out[k as usize][(i_1 - start) as usize] !=
                           crate::sample::out[k as usize][(i_1 - start) as usize] {
