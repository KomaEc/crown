                ...
                1 => {
                    if op > max * 1.5f64 + 2 as std::os::raw::c_int as std::os::raw::c_double
                        ...
                        if (0 as std::os::raw::c_int == 0) as std::os::raw::c_int as
                               std::os::raw::c_long != 0 {
                            __assert_rtn(b"check_output\x00" as *const u8 as *const i8,
...
pub unsafe extern "C" fn stress(mut info: *const crate::example2::ti_indicator_info) {
    ...
    while i < 10 as std::os::raw::c_int {
        inputs[i as usize]=
            if i < (*info).inputs {
                crate::fuzzer::dummy_in.as_mut_ptr()
            } else { 0 as *mut std::os::raw::c_double };
        inputs0[i as usize]=
            if i < (*info).inputs {
                crate::fuzzer::dummy_in0.as_mut_ptr()
            } else { 0 as *mut std::os::raw::c_double };
        outputs[i as usize]=
            if i < (*info).outputs {
                crate::fuzzer::dummy_ot.as_mut_ptr()
            } else { 0 as *mut std::os::raw::c_double };
        i+= 1
    }
    ...
    loop  {
        ...
        while j < opt_count {
            options[j as usize]=
                crate::fuzzer::optionsd[options_index[j as usize] as usize];
    ...
    if !(0.1f64 >
           ...
           std::os::raw::c_long != 0 {
        __assert_rtn(b"stress\x00" as *const u8 as *const i8,
 ...
 -> std::os::raw::c_int {
    ...
    if argc > 1 as std::os::raw::c_int {
        let mut info: *const crate::example2::ti_indicator_info =
            crate::indicators_index::ti_find_indicator(*argv.offset(1 as std::os::raw::c_int as isize));
        if !info.is_null() { stress(info); }else { (); }
    } else {
        let mut info_0: *const crate::example2::ti_indicator_info = crate::fuzzer::ti_indicators.as_mut_ptr();
        loop  {
            stress(info_0);
