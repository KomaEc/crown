    ...
    while 1 as std::os::raw::c_int as Bool != 0 {
        ...
        while nHeap > 1 as std::os::raw::c_int {
            ...
            while 1 as std::os::raw::c_int as Bool != 0 {
                ...
                if yy_0 < nHeap
                    && weight[heap[(yy_0 + 1 as std::os::raw::c_int) as usize] as usize]
            ...
            weight[nNodes as usize] = ((weight[n1 as usize] as std::os::raw::c_uint
                & 0xffffff00 as std::os::raw::c_uint)
                .wrapping_add(
                    weight[n2 as usize] as std::os::raw::c_uint
                        & 0xffffff00 as std::os::raw::c_uint,
                )
                | (1 as std::os::raw::c_int
            ...
            while weight[tmp_2 as usize]
                < weight[heap[(zz_2 >> 1 as std::os::raw::c_int) as usize] as usize]
            {
                heap[zz_2 as usize] = heap[(zz_2 >> 1 as std::os::raw::c_int) as usize];
        ...
        while i <= alphaSize {
            ...
            *len.offset((i - 1 as std::os::raw::c_int) as isize) = j as UChar;
            if j > maxLen {
                tooLong = 1 as std::os::raw::c_int as Bool
            }
            i += 1
        }
        if tooLong == 0 {
...
pub unsafe extern "C" fn BZ2_hbCreateDecodeTables(
    ...
    while i < alphaSize {
        *base.offset(
            (*length.offset(i as isize) as std::os::raw::c_int + 1 as std::os::raw::c_int) as isize,
        ) += 1;
        i += 1
    }
    i = 1 as std::os::raw::c_int;
    ...
    i = minLen;
    while i <= maxLen {
        vec += *base.offset((i + 1 as std::os::raw::c_int) as isize) - *base.offset(i as isize);
    ...
    while i <= maxLen {
        *base.offset(i as isize) = ((*limit.offset((i - 1 as std::os::raw::c_int) as isize)
            + 1 as std::os::raw::c_int)
            << 1 as std::os::raw::c_int)
