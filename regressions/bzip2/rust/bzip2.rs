...
unsafe extern "C" fn isascii(mut _c: std::os::raw::c_int) -> std::os::raw::c_int {
    return (_c & !(0x7f as std::os::raw::c_int) == 0 as std::os::raw::c_int)
        as std::os::raw::c_int;
}
#[inline]
unsafe extern "C" fn __istype(
    ...
    return if isascii(_c) != 0 {
        ...
        (__maskrune(_c, _f) != 0) as std::os::raw::c_int
    };
}
#[no_mangle]
#[inline]
...
unsafe extern "C" fn compressStream(mut stream: *mut FILE, mut zStream: *mut FILE) {
    ...
    if !(ferror(stream) != 0) {
        if !(ferror(zStream) != 0) {
            bzf = crate::bzlib::BZ2_bzWriteOpen(
                &mut bzerr,
                zStream,
                blockSize100k,
                verbosity,
            ...
            if bzerr != 0 as std::os::raw::c_int {
                ...
                match current_block {
                    ...
                    660242869387099075 => {}
                    _ => {
                        crate::bzlib::BZ2_bzWriteClose64(
                            &mut bzerr,
                            bzf,
    ...
    if !(ferror(stream) != 0) {
        if !(ferror(zStream) != 0) {
            's_51: loop {
                ...
                bzf = crate::bzlib::BZ2_bzReadOpen(
                    &mut bzerr,
                    zStream,
                    verbosity,
                ...
                crate::bzlib::BZ2_bzReadGetUnused(&mut bzerr, bzf, &mut unusedTmpV, &mut nUnused);
                if bzerr != 0 as std::os::raw::c_int {
                    panic(
                                ...
                                match bzerr {
                                    -9 => {
                                        ...
                                        match current_block {
                                            6455255476181645667 => {
                                                panic(
                                                    b"decompress:unexpected error\x00" as *const u8
                                                        as *const std::os::raw::c_char,
                                                );
                                            }
                                            3642457097893642164 => {
...
unsafe extern "C" fn testStream(mut zStream: *mut FILE) -> Bool {
    ...
    if !(ferror(zStream) != 0) {
        's_41: loop {
            ...
            crate::bzlib::BZ2_bzReadGetUnused(&mut bzerr, bzf, &mut unusedTmpV, &mut nUnused);
            if bzerr != 0 as std::os::raw::c_int {
                panic(b"test:bzReadGetUnused\x00" as *const u8 as *const std::os::raw::c_char);
            }
            unusedTmp = unusedTmpV as *mut UChar;
            i = 0 as std::os::raw::c_int;
            while i < nUnused {
            ...
            if nUnused == 0 as std::os::raw::c_int && myfeof(zStream) as std::os::raw::c_int != 0 {
                current_block = 15125582407903384992;
                break;
            }
        }
                ...
                match bzerr {
                    -9 => {
                        ...
                        match current_block {
                            6101827300316655396 => {
                                panic(
                                    b"test:unexpected error\x00" as *const u8
                                        as *const std::os::raw::c_char,
                                );
                            }
...
unsafe extern "C" fn cleanUpAndFail(mut ec: Int32) -> ! {
    ...
    if noisy as std::os::raw::c_int != 0
...
unsafe extern "C" fn mySIGSEGVorSIGBUScatcher(mut n: IntNative) {
    ...
    write(
        2 as std::os::raw::c_int,
        b"\n\x00" as *const u8 as *const std::os::raw::c_char as *const std::os::raw::c_void,
        1 as std::os::raw::c_int as size_t,
    );
    write(
        2 as std::os::raw::c_int,
        progName as *const std::os::raw::c_void,
        strlen(progName),
    );
    write(
        2 as std::os::raw::c_int,
...
unsafe extern "C" fn fileExists(mut name: *mut Char) -> Bool {
    let mut tmp: *mut FILE = fopen(name, b"rb\x00" as *const u8 as *const std::os::raw::c_char);
    let mut exists: Bool =
...
unsafe extern "C" fn hasSuffix(mut s: *const Char, mut suffix: *const Char) -> Bool {
    ...
    if strcmp(s.offset(ns as isize).offset(-(nx as isize)), suffix) == 0 as std::os::raw::c_int {
        return 1 as std::os::raw::c_int as Bool;
    }
    return 0 as std::os::raw::c_int as Bool;
}
...
unsafe extern "C" fn compress(mut name: *mut Char) {
    ...
    match srcMode {
        1 => {
            copyFileName(
                inName.as_mut_ptr(),
                b"(stdin)\x00" as *const u8 as *const std::os::raw::c_char as *mut Char,
            );
            copyFileName(
                outName.as_mut_ptr(),
                b"(stdout)\x00" as *const u8 as *const std::os::raw::c_char as *mut Char,
            );
        }
        3 => {
    ...
    match srcMode {
        ...
        _ => {
            panic(b"compress: bad srcMode\x00" as *const u8 as *const std::os::raw::c_char);
        }
    }
    if verbosity >= 1 as std::os::raw::c_int {
        fprintf(
...
unsafe extern "C" fn uncompress(mut name: *mut Char) {
    ...
    match srcMode {
        ...
        _ => {
            panic(b"uncompress: bad srcMode\x00" as *const u8 as *const std::os::raw::c_char);
        }
    }
    if verbosity >= 1 as std::os::raw::c_int {
        fprintf(
    ...
    if magicNumberOK != 0 {
        ...
        if srcMode == 3 as std::os::raw::c_int {
            ...
            if retVal_0 != 0 as std::os::raw::c_int {
                ioError();
            }
        }
    }
...
unsafe extern "C" fn testf(mut name: *mut Char) {
    ...
    copyFileName(
        outName.as_mut_ptr(),
        b"(none)\x00" as *const u8 as *const std::os::raw::c_char as *mut Char,
    );
    match srcMode {
        1 => {
            copyFileName(
    ...
    match srcMode {
        ...
        _ => {
            panic(b"testf: bad srcMode\x00" as *const u8 as *const std::os::raw::c_char);
        }
    }
    if verbosity >= 1 as std::os::raw::c_int {
        fprintf(
    ...
    if allOK == 0 {
        testFailsExist = 1 as std::os::raw::c_int as Bool
    };
}
/*---------------------------------------------*/
...
unsafe fn main_0(mut argc: IntNative, mut argv: *mut *mut Char) -> IntNative {
    ...
    /*-- Initialise --*/
    outputHandleJustInCase = 0 as *mut FILE; /* avoid bogus warning from egcs-1.1.X */
    ...
    signal(
        ...
        Some(mySIGSEGVorSIGBUScatcher as unsafe extern "C" fn(_: IntNative) -> ()),
    );
    signal(
    ...
    addFlagsFromEnvVar(
        &mut argList,
        b"BZIP\x00" as *const u8 as *const std::os::raw::c_char as *mut Char,
    );
    i = 1 as std::os::raw::c_int;
    ...
    if !strstr(
        progName,
        b"unzip\x00" as *const u8 as *const std::os::raw::c_char,
    )
    .is_null()
        || !strstr(
            progName,
    ...
    if !strstr(
        progName,
        b"z2cat\x00" as *const u8 as *const std::os::raw::c_char,
    )
    .is_null()
        || !strstr(
            progName,
        ...
        || !strstr(
            progName,
            b"zcat\x00" as *const u8 as *const std::os::raw::c_char,
        )
        .is_null()
        || !strstr(
            progName,
    ...
    {
        opMode = 2 as std::os::raw::c_int;
        srcMode = if numFileNames == 0 as std::os::raw::c_int {
            1 as std::os::raw::c_int
        } else {
            2 as std::os::raw::c_int
        }
    ...
    if opMode == 1 as std::os::raw::c_int
        && smallMode as std::os::raw::c_int != 0
    ...
    if opMode != 1 as std::os::raw::c_int {
        blockSize100k = 0 as std::os::raw::c_int
    }
    if srcMode == 3 as std::os::raw::c_int {
        signal(
            2 as std::os::raw::c_int,
            Some(mySignalCatcher as unsafe extern "C" fn(_: IntNative) -> ()),
        );
        signal(
