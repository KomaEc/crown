...
impl Default for osn_context {fn default() -> Self {Self {
perm: std::ptr::null_mut(),
permGradIndex3D: std::ptr::null_mut(),
}}}
impl osn_context {pub fn take(&mut self) -> Self {core::mem::take(self)}}
...
#[no_mangle]
pub unsafe extern "C" fn open_simplex_noise_free(mut ctx: *mut /* owning */ osn_context) {
    if ctx.is_null() {();
        return;
    }
    if !(*ctx).perm.is_null() {
