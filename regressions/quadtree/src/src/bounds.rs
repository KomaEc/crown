...
impl Default for quadtree_point {
    fn default() -> Self {
        Self {
            x: Default::default(),
            y: Default::default(),
...
impl Default for quadtree_bounds {
    fn default() -> Self {
        Self {
            nw: None,
            se: None,
...
pub unsafe extern "C" fn quadtree_bounds_extend(
    ...
    (*bounds).width =
        fabs((*(*bounds).nw.as_deref().unwrap()).x - (*(*bounds).se.as_deref().unwrap()).x);
    (*bounds).height =
        fabs((*(*bounds).nw.as_deref().unwrap()).y - (*(*bounds).se.as_deref().unwrap()).y);
}
#[no_mangle]
...
pub unsafe extern "C" fn quadtree_bounds_new() -> Option<Box<quadtree_bounds_t>> {
    let mut bounds = None;
    bounds = Some(Box::new(
        <crate::src::src::bounds::quadtree_bounds as Default>::default(),
    ));
