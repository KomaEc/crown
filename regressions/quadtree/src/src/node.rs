...
impl Default for quadtree_node {
    fn default() -> Self {
        Self {
            ne: std::ptr::null_mut(),
            nw: std::ptr::null_mut(),
...
#[no_mangle]
pub unsafe extern "C" fn quadtree_node_ispointer(mut node: *const quadtree_node_t) -> libc::c_int {
...
#[no_mangle]
pub unsafe extern "C" fn quadtree_node_isempty(mut node: *const quadtree_node_t) -> libc::c_int {
    return ((*node).nw.is_null()
        && (*node).ne.is_null()
        && (*node).sw.is_null()
        && (*node).se.is_null()
...
#[no_mangle]
pub unsafe extern "C" fn quadtree_node_isleaf(mut node: *const quadtree_node_t) -> libc::c_int {
    return ((*node).point != 0 as *mut libc::c_void as *mut quadtree_point_t) as libc::c_int;
}
#[no_mangle]
...
pub unsafe extern "C" fn quadtree_node_new() -> Option<Box<quadtree_node_t>> {
    let mut node = None;
    node = Some(Box::new(
        <crate::src::src::node::quadtree_node as Default>::default(),
    ));
...
pub unsafe extern "C" fn quadtree_node_free(
    ...
    if !(*node.as_deref().unwrap()).nw.is_null() {
        quadtree_node_free(
            Some(Box::from_raw((*node.as_deref_mut().unwrap()).nw)),
            key_free,
        );
    } else {
        ();
    ...
    if !(*node.as_deref().unwrap()).ne.is_null() {
        quadtree_node_free(
            Some(Box::from_raw((*node.as_deref_mut().unwrap()).ne)),
            key_free,
        );
    } else {
        ();
    ...
    if !(*node.as_deref().unwrap()).sw.is_null() {
        quadtree_node_free(
            Some(Box::from_raw((*node.as_deref_mut().unwrap()).sw)),
            key_free,
        );
    } else {
        ();
    ...
    if !(*node.as_deref().unwrap()).se.is_null() {
        quadtree_node_free(
            Some(Box::from_raw((*node.as_deref_mut().unwrap()).se)),
            key_free,
        );
    } else {
        ();
    }
    crate::src::src::bounds::quadtree_bounds_free(Some(Box::from_raw(
        (*node.as_deref_mut().unwrap()).bounds,
    )));
    quadtree_node_reset(
        node.as_deref_mut()
            .map(|r| r as *mut _)
