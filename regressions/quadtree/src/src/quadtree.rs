...
unsafe extern "C" fn node_contains_(
    mut outer: *const quadtree_node_t,
    mut it: *const quadtree_point_t,
) -> libc::c_int {
    return (!(*outer).bounds.is_null()
        && (*(*(*outer).bounds).nw.as_deref().unwrap()).x < (*it).x
        && (*(*(*outer).bounds).nw.as_deref().unwrap()).y > (*it).y
        && (*(*(*outer).bounds).se.as_deref().unwrap()).x > (*it).x
        && (*(*(*outer).bounds).se.as_deref().unwrap()).y < (*it).y) as libc::c_int;
}
unsafe extern "C" fn elision_(mut key: *const libc::c_void) {}
...
unsafe extern "C" fn get_quadrant_(
    ...
    mut point: *const quadtree_point_t,
) -> *mut quadtree_node_t {
    if node_contains_(
        (*root).nw as *const crate::src::src::node::quadtree_node,
        point,
    ) != 0
    {
        return (*root).nw;
    ...
    if node_contains_(
        (*root).ne as *const crate::src::src::node::quadtree_node,
        point,
    ) != 0
    {
        return (*root).ne;
    ...
    if node_contains_(
        (*root).sw as *const crate::src::src::node::quadtree_node,
        point,
    ) != 0
    {
        return (*root).sw;
    ...
    if node_contains_(
        (*root).se as *const crate::src::src::node::quadtree_node,
        point,
    ) != 0
    {
        return (*root).se;
...
unsafe extern "C" fn find_(
    ...
    mut x: libc::c_double,
    mut y: libc::c_double,
) -> *const quadtree_point_t {
    if crate::src::src::node::quadtree_node_isleaf(
        node.as_deref()
            .map(|r| r as *const _)
            .unwrap_or(std::ptr::null()),
    ) != 0
    {
        if (*(*node.as_deref().unwrap()).point).x == x
            && (*(*node.as_deref().unwrap()).point).y == y
        {
...
pub unsafe extern "C" fn quadtree_new(
    ...
    tree = malloc(::std::mem::size_of::<quadtree_t>() as libc::c_ulong) as *mut quadtree_t;
    if tree.is_null() {
        ();
        return 0 as *mut quadtree_t;
    }
    (*tree).root = crate::src::src::node::quadtree_node_with_bounds(minx, miny, maxx, maxy);
    if (*tree).root.as_deref().is_none() {
        ();
        return 0 as *mut quadtree_t;
    }
    (*tree).key_free = None;
...
pub unsafe extern "C" fn quadtree_insert(
    ...
    point = crate::src::src::point::quadtree_point_new(x, y);
    if point.is_null() {
        ();
        return 0 as libc::c_int;
    }
