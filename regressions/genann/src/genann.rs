...
#[no_mangle]
pub unsafe extern "C" fn genann_act_sigmoid_cached(mut a: libc::c_double) -> libc::c_double {
    let min = -15.0f64;
    let max = 15.0f64;
    ...
    if initialized == 0 {
        ...
        while i < 4096 as libc::c_int {
            lookup[i as usize] = genann_act_sigmoid(min + interval * i as libc::c_double);
            i += 1;
        }
...
pub unsafe extern "C" fn genann_init(
    ...
    let size = (::std::mem::size_of::<crate::src::example1::genann>() as libc::c_ulong)
        .wrapping_add(
            (::std::mem::size_of::<libc::c_double>() as libc::c_ulong).wrapping_mul(
                (total_weights + total_neurons + (total_neurons - inputs)) as libc::c_ulong,
            ),
        ) as libc::c_int;
    ...
    (*ret).activation_hidden =
        Some(genann_act_sigmoid_cached as unsafe extern "C" fn(libc::c_double) -> libc::c_double);
    (*ret).activation_output =
        Some(genann_act_sigmoid_cached as unsafe extern "C" fn(libc::c_double) -> libc::c_double);
    return ret;
}
#[no_mangle]
pub unsafe extern "C" fn genann_read(
    mut in_0: *mut FILE,
) -> Option<Box<crate::src::example1::genann>> {
...
pub unsafe extern "C" fn genann_randomize(mut ann: Option<&mut crate::src::example1::genann>) {
    ...
    i = 0 as libc::c_int;
    while i < (*ann.as_deref().unwrap()).total_weights {
        let mut r = rand() as libc::c_double / 2147483647 as libc::c_int as libc::c_double;
        *(*ann.as_deref().unwrap()).weight.offset(i as isize) = r - 0.5f64;
        i += 1;
    }
}
#[no_mangle]
pub unsafe extern "C" fn genann_free(mut ann: Option<Box<crate::src::example1::genann>>) {
    ();
}
#[no_mangle]
...
pub unsafe extern "C" fn genann_train(
    ...
    let mut j: libc::c_int = 0;
    let mut k: libc::c_int = 0;
    let mut o: *const libc::c_double = (*ann)
        .output
    ...
    if (*ann).activation_output
        == Some(genann_act_linear as unsafe extern "C" fn(libc::c_double) -> libc::c_double)
    {
        j = 0 as libc::c_int;
        while j < (*ann).outputs {
            let fresh14 = t;
    ...
    while h >= 0 as libc::c_int {
        ...
        while j < (*ann).hidden {
                ...
                })
            {
                ...
                let windex = k * ((*ann).hidden + 1 as libc::c_int) + (j + 1 as libc::c_int);
    ...
    let mut d_1: *const libc::c_double = (*ann)
        .delta
        .offset(((*ann).hidden * (*ann).hidden_layers) as isize);
    ...
    if w.offset_from((*ann).weight) as libc::c_long == (*ann).total_weights as libc::c_long {
    } else {
        __assert_fail(
            b"w - ann->weight == ann->total_weights\0" as *const u8 as *const libc::c_char,
            b"genann.c\0" as *const u8 as *const libc::c_char,
    ...
    h = (*ann).hidden_layers - 1 as libc::c_int;
    while h >= 0 as libc::c_int {
        let mut d_2: *const libc::c_double = (*ann).delta.offset((h * (*ann).hidden) as isize);
        ...
        let mut w_0 = (*ann).weight.offset(
            (if h != 0 {
                ((*ann).inputs + 1 as libc::c_int) * (*ann).hidden
                    + ((*ann).hidden + 1 as libc::c_int) * (*ann).hidden * (h - 1 as libc::c_int)
            } else {
                0 as libc::c_int
            }) as isize,
        );
                ...
                }) + 1 as libc::c_int
            {
                if k == 0 as libc::c_int {
                    ...
                    *fresh20 += (*d_2) * learning_rate * -1.0f64;
                    ...
                    w_0 = w_0.offset(1);
                    *fresh21 +=
                        (*d_2) * learning_rate * *i_0.offset((k - 1 as libc::c_int) as isize);
                }
                k += 1;
            }
            d_2 = d_2.offset(1);
