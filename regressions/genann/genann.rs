...
pub unsafe extern "C" fn genann_act_sigmoid_cached(
    ...
    if initialized == 0 {
        ...
        while i < 4096 as std::os::raw::c_int {
            lookup[i as usize] = genann_act_sigmoid(min + interval * i as std::os::raw::c_double);
            i += 1
        }
    ...
    if i_0 <= 0 as std::os::raw::c_int {
        return lookup[0 as std::os::raw::c_int as usize];
    }
...
pub unsafe extern "C" fn genann_act_threshold(
    ...
    return (a > 0 as std::os::raw::c_int as std::os::raw::c_double) as std::os::raw::c_int
...
pub unsafe extern "C" fn genann_init(
    ...
    let size: std::os::raw::c_int =
        (::std::mem::size_of::<crate::example1::genann>() as std::os::raw::c_ulong).wrapping_add(
    ...
    let mut ret: *mut crate::example1::genann =
        malloc(size as std::os::raw::c_ulong) as *mut crate::example1::genann;
    if ret.is_null() {
        ();
        ();
        return 0 as *mut crate::example1::genann;
    ...
    /* Set pointers. */
    (*ret).weight = (ret as *mut std::os::raw::c_char)
        .offset(::std::mem::size_of::<crate::example1::genann>() as std::os::raw::c_ulong as isize)
    ...
    (*ret).activation_hidden = Some(
        genann_act_sigmoid_cached
            as unsafe extern "C" fn(_: std::os::raw::c_double) -> std::os::raw::c_double,
    );
...
pub unsafe extern "C" fn genann_read(mut in_0: *mut FILE) -> Option<Box<crate::example1::genann>> {
    ...
    *__error() = 0 as std::os::raw::c_int;
    rc = fscanf(
        in_0,
        b"%d %d %d %d\x00" as *const u8 as *const std::os::raw::c_char,
    ...
    let mut ann: *mut crate::example1::genann = genann_init(inputs, hidden_layers, hidden, outputs);
    let mut i: std::os::raw::c_int = 0;
    i = 0 as std::os::raw::c_int;
...
#[no_mangle]
pub unsafe extern "C" fn genann_copy(
    mut ann: *const crate::example1::genann,
) -> *mut crate::example1::genann {
    let size: std::os::raw::c_int =
        (::std::mem::size_of::<crate::example1::genann>() as std::os::raw::c_ulong).wrapping_add(
            (::std::mem::size_of::<std::os::raw::c_double>() as std::os::raw::c_ulong)
    ...
    memcpy(
        ...
        ann as *const std::os::raw::c_void,
...
pub unsafe extern "C" fn genann_randomize(mut ann: Option<&mut crate::example1::genann>) {
    ...
    i = 0 as std::os::raw::c_int;
    while i < (*ann.as_deref().unwrap()).total_weights {
        let mut r: std::os::raw::c_double = rand() as std::os::raw::c_double
...
pub unsafe extern "C" fn genann_run(
    mut ann: Option<&mut crate::example1::genann>,
    ...
    let mut o: *mut std::os::raw::c_double = (*ann.as_deref().unwrap())
        .output
        .offset((*ann.as_deref().unwrap()).inputs as isize);
    let mut i: *const std::os::raw::c_double = (*ann.as_deref().unwrap()).output as *const f64;
    /* Copy the inputs to the scratch area, where we also store each neuron's
     * output, for consistency. This way the first layer isn't a special case. */
    memcpy(
        (*ann.as_deref().unwrap()).output as *mut std::os::raw::c_void,
        inputs as *const std::os::raw::c_void,
        (::std::mem::size_of::<std::os::raw::c_double>() as std::os::raw::c_ulong)
            .wrapping_mul((*ann.as_deref().unwrap()).inputs as std::os::raw::c_ulong),
    );
    ...
    while h < (*ann.as_deref().unwrap()).hidden_layers {
        ...
        i = i.offset(if h == 0 as std::os::raw::c_int {
            (*ann.as_deref().unwrap()).inputs
        } else {
            (*ann.as_deref().unwrap()).hidden
        } as isize);
    ...
    if !(w.offset_from((*ann.as_deref().unwrap()).weight as *const f64) as std::os::raw::c_long
        == (*ann.as_deref().unwrap()).total_weights as std::os::raw::c_long)
        as std::os::raw::c_int as std::os::raw::c_long
        != 0
    {
        __assert_rtn(
            b"genann_run\x00" as *const u8 as *const i8,
    ...
    if !(o.offset_from((*ann.as_deref().unwrap()).output as *const f64) as std::os::raw::c_long
        == (*ann.as_deref().unwrap()).total_neurons as std::os::raw::c_long)
        as std::os::raw::c_int as std::os::raw::c_long
        != 0
    {
        __assert_rtn(
...
pub unsafe extern "C" fn genann_train(
    mut ann: *const crate::example1::genann,
    mut inputs: *const std::os::raw::c_double,
    mut desired_outputs: *const std::os::raw::c_double,
    ...
    let mut o: *const std::os::raw::c_double = (*ann)
        .output
        .offset((*ann).inputs as isize)
        .offset(((*ann).hidden * (*ann).hidden_layers) as isize); /* First output. */
    let mut d: *mut std::os::raw::c_double = (*ann)
        .delta
        .offset(((*ann).hidden * (*ann).hidden_layers) as isize); /* First delta. */
    let mut t: *const std::os::raw::c_double = desired_outputs; /* First desired output. */
    ...
    while h >= 0 as std::os::raw::c_int {
        ...
        let mut o_0: *const std::os::raw::c_double = (*ann)
            .output
            .offset((*ann).inputs as isize)
        ...
        let dd: *const std::os::raw::c_double = (*ann)
            .delta
            .offset(((h + 1 as std::os::raw::c_int) * (*ann).hidden) as isize);
        /* Find first weight in following layer (which may be hidden or output). */
        let ww: *const std::os::raw::c_double = (*ann)
            .weight
            .offset((((*ann).inputs + 1 as std::os::raw::c_int) * (*ann).hidden) as isize)
            .offset((((*ann).hidden + 1 as std::os::raw::c_int) * (*ann).hidden * h) as isize);
        j = 0 as std::os::raw::c_int;
        while j < (*ann).hidden {
                ...
                })
            {
                ...
                let windex: std::os::raw::c_int =
                    k * ((*ann).hidden + 1 as std::os::raw::c_int) + (j + 1 as std::os::raw::c_int);
                let forward_weight: std::os::raw::c_double = *ww.offset(windex as isize);
    ...
    let mut d_1: *const std::os::raw::c_double = (*ann)
        .delta
        .offset(((*ann).hidden * (*ann).hidden_layers) as isize); /* First output delta. */
    let mut w: *mut std::os::raw::c_double = (*ann).weight.offset(
        (if (*ann).hidden_layers != 0 {
            (((*ann).inputs + 1 as std::os::raw::c_int) * (*ann).hidden)
                + ((*ann).hidden + 1 as std::os::raw::c_int)
                    * (*ann).hidden
                    * ((*ann).hidden_layers - 1 as std::os::raw::c_int)
        } else {
            0 as std::os::raw::c_int
        }) as isize,
    );
    ...
    while j < (*ann).outputs {
            ...
            }) + 1 as std::os::raw::c_int
        {
            if k == 0 as std::os::raw::c_int {
                ...
                w = w.offset(1);
                (*fresh11) +=
                    (*d_1) * learning_rate * *i.offset((k - 1 as std::os::raw::c_int) as isize)
            }
            k += 1
        }
        d_1 = d_1.offset(1);
    ...
    if !(w.offset_from((*ann).weight as *const f64) as std::os::raw::c_long
        == (*ann).total_weights as std::os::raw::c_long) as std::os::raw::c_int
        as std::os::raw::c_long
        != 0
    {
        __assert_rtn(
            b"genann_train\x00" as *const u8 as *const i8,
            b"genann.c\x00" as *const u8 as *const std::os::raw::c_char,
    ...
    while h >= 0 as std::os::raw::c_int {
        ...
        let mut w_0: *mut std::os::raw::c_double = (*ann).weight.offset(
            (if h != 0 {
                (((*ann).inputs + 1 as std::os::raw::c_int) * (*ann).hidden)
                    + ((*ann).hidden + 1 as std::os::raw::c_int)
                        * (*ann).hidden
...
pub unsafe extern "C" fn genann_write(
    ...
    while i < (*ann).total_weights {
        fprintf(
            out,
            b" %.20e\x00" as *const u8 as *const std::os::raw::c_char,
