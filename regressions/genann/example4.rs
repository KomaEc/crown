...
pub unsafe extern "C" fn load_data() {
    ...
    while feof(in_0) == 0 && !fgets(line.as_mut_ptr(), 1024 as std::os::raw::c_int, in_0).is_null()
    {
        samples += 1
    }
    fseek(
    ...
    input = malloc(
        (::std::mem::size_of::<std::os::raw::c_double>() as std::os::raw::c_ulong)
            .wrapping_mul(samples as std::os::raw::c_ulong)
    ...
    while i < samples {
        ...
        *c.offset(2 as std::os::raw::c_int as isize) = 0.0f64;
        *c.offset(1 as std::os::raw::c_int as isize) = *c.offset(2 as std::os::raw::c_int as isize);
        *c.offset(0 as std::os::raw::c_int as isize) = *c.offset(1 as std::os::raw::c_int as isize);
        ...
        let mut split: *mut std::os::raw::c_char = strtok(
            line.as_mut_ptr(),
        ...
        *split.offset(
            strlen(split).wrapping_sub(1 as std::os::raw::c_int as std::os::raw::c_ulong) as isize,
        ) = 0 as std::os::raw::c_int as std::os::raw::c_char;
        if strcmp(split, class_names[0 as std::os::raw::c_int as usize]) == 0 as std::os::raw::c_int
        {
...
unsafe fn main_0(
    ...
    while i < loops {
        ...
        while j < samples {
            crate::genann::genann_train(
                ann,
                input.offset((j * 4 as std::os::raw::c_int) as isize),
    ...
    j = 0 as std::os::raw::c_int;
    while j < samples {
        let mut guess: *const std::os::raw::c_double = crate::genann::genann_run(
            ann.as_mut(),
            input.offset((j * 4 as std::os::raw::c_int) as isize),
        );
        ...
        } else if *class.offset((j * 3 as std::os::raw::c_int + 1 as std::os::raw::c_int) as isize)
            == 1.0f64
        {
        ...
        } else if *class.offset((j * 3 as std::os::raw::c_int + 2 as std::os::raw::c_int) as isize)
            == 1.0f64
        {
