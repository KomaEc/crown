...
pub struct csv_parser {
    ...
    pub blk_size: size_t,
    pub malloc_func: Option<unsafe extern "C" fn(_: size_t) -> *mut std::os::raw::c_void>,
    pub realloc_func: Option<
        unsafe extern "C" fn(_: *mut std::os::raw::c_void, _: size_t) -> *mut std::os::raw::c_void,
    >,
...
static mut csv_errors: [*const std::os::raw::c_char; 5] = [
    ...
    b"error parsing data while strict checking enabled\x00" as *const u8
        as *const std::os::raw::c_char,
    b"memory exhausted while increasing buffer size\x00" as *const u8
        as *const std::os::raw::c_char,
...
pub unsafe extern "C" fn csv_error(mut p: *const csv_parser) -> std::os::raw::c_int {
    if !(!p.is_null()
        && !(b"received null csv_parser\x00" as *const u8 as *const std::os::raw::c_char).is_null())
        as std::os::raw::c_int as std::os::raw::c_long
        != 0
    {
        __assert_rtn(
            b"csv_error\x00" as *const u8 as *const i8,
            b"libcsv.c\x00" as *const u8 as *const std::os::raw::c_char,
            82 as std::os::raw::c_int,
            b"p && \"received null csv_parser\"\x00" as *const u8 as *const std::os::raw::c_char,
        );
    } else {
    };
...
#[no_mangle]
pub unsafe extern "C" fn csv_get_opts(mut p: *const csv_parser) -> std::os::raw::c_int {
...
pub unsafe extern "C" fn csv_free(mut p: *mut csv_parser) {
    ...
    if !(*p).entry_buf.is_null() && (*p).free_func.is_some() {
        (*p).free_func.expect("non-null function pointer")(
            (*p).entry_buf as *mut std::os::raw::c_void,
        );
    }
...
pub unsafe extern "C" fn csv_fini(
    ...
    >,
    mut cb2: Option<
        unsafe extern "C" fn(_: std::os::raw::c_int, _: *mut std::os::raw::c_void) -> (),
    >,
    ...
    if pstate == 2 as std::os::raw::c_int
        ...
        && (*p.as_deref().unwrap()).options as std::os::raw::c_int & 4 as std::os::raw::c_int != 0
    {
        ...
        (*p.as_deref_mut().unwrap()).status = 1 as std::os::raw::c_int; /* get rid of spaces and original quote */
        return -(1 as std::os::raw::c_int);
    }
    ...
    match pstate {
        3 => {
            (*p.as_deref_mut().unwrap()).entry_pos =
                ((*p.as_deref().unwrap()).entry_pos as std::os::raw::c_ulong).wrapping_sub(
                    (*p.as_deref().unwrap())
                        .spaces
        ...
        */
        {
            if quoted == 0 {
                entry_pos =
                    (entry_pos as std::os::raw::c_ulong).wrapping_sub(spaces) as size_t as size_t
            }
            if (*p.as_deref().unwrap()).options as std::os::raw::c_int & 8 as std::os::raw::c_int
                != 0
            {
                *(*p.as_deref().unwrap())
                    .entry_buf
                    .offset(entry_pos as isize) = '\u{0}' as i32 as std::os::raw::c_uchar
            }
            if cb1.is_some()
                && (*p.as_deref().unwrap()).options as std::os::raw::c_int
            ...
            if cb2.is_some() {
                cb2.expect("non-null function pointer")(-(1 as std::os::raw::c_int), data);
            }
...
#[no_mangle]
pub unsafe extern "C" fn csv_get_delim(mut p: *const csv_parser) -> std::os::raw::c_uchar {
    if !(!p.is_null()
        && !(b"received null csv_parser\x00" as *const u8 as *const std::os::raw::c_char).is_null())
        as std::os::raw::c_int as std::os::raw::c_long
        != 0
    {
        __assert_rtn(
            b"csv_get_delim\x00" as *const u8 as *const i8,
            b"libcsv.c\x00" as *const u8 as *const std::os::raw::c_char,
...
#[no_mangle]
pub unsafe extern "C" fn csv_get_quote(mut p: *const csv_parser) -> std::os::raw::c_uchar {
    if !(!p.is_null()
        && !(b"received null csv_parser\x00" as *const u8 as *const std::os::raw::c_char).is_null())
        as std::os::raw::c_int as std::os::raw::c_long
        != 0
    {
        __assert_rtn(
            b"csv_get_quote\x00" as *const u8 as *const i8,
            b"libcsv.c\x00" as *const u8 as *const std::os::raw::c_char,
...
pub unsafe extern "C" fn csv_set_space_func(
    mut p: Option<&mut csv_parser>,
    mut f: Option<unsafe extern "C" fn(_: std::os::raw::c_uchar) -> std::os::raw::c_int>,
) {
...
pub unsafe extern "C" fn csv_set_free_func(
    mut p: Option<&mut csv_parser>,
    mut f: Option<unsafe extern "C" fn(_: *mut std::os::raw::c_void) -> ()>,
) {
...
unsafe extern "C" fn csv_increase_buffer(mut p: *mut csv_parser) -> std::os::raw::c_int {
    if p.is_null() {
        ();
        return 0 as std::os::raw::c_int;
    }
    ...
    if (*p).entry_size >= (18446744073709551615 as std::os::raw::c_ulong).wrapping_sub(to_add) {
        to_add = (18446744073709551615 as std::os::raw::c_ulong).wrapping_sub((*p).entry_size)
    }
    if to_add == 0 {
    ...
    loop {
        ...
        to_add = (to_add as std::os::raw::c_ulong)
    ...
    (*p).entry_buf = vp as *mut std::os::raw::c_uchar; /* Access input data as array of unsigned char */
    (*p).entry_size =
        ((*p).entry_size as std::os::raw::c_ulong).wrapping_add(to_add) as size_t as size_t; /* The character we are currently processing */
    return 0 as std::os::raw::c_int; /* The number of characters we have processed in this call */
}
#[no_mangle]
pub unsafe extern "C" fn csv_parse(
    ...
    {
        __assert_rtn(
            b"csv_parse\x00" as *const u8 as *const i8,
            b"libcsv.c\x00" as *const u8 as *const std::os::raw::c_char,
    ...
    let mut is_space: Option<
        unsafe extern "C" fn(_: std::os::raw::c_uchar) -> std::os::raw::c_int,
    > = (*p.as_deref().unwrap()).is_space;
    let mut is_term: Option<unsafe extern "C" fn(_: std::os::raw::c_uchar) -> std::os::raw::c_int> =
    ...
    while pos < len {
        ...
        if entry_pos
            == (if (*p.as_deref().unwrap()).options as std::os::raw::c_int
        ...
        match pstate {
            0 | 1 => {
                if (if is_space.is_some() {
                    ...
                    && c as std::os::raw::c_int != delim as std::os::raw::c_int
                {
                    continue;
                }
                if if is_term.is_some() {
                    ...
                    (c as std::os::raw::c_int == 0xd as std::os::raw::c_int
                        || c as std::os::raw::c_int == 0xa as std::os::raw::c_int)
                        as std::os::raw::c_int
                } != 0
                {
                    ...
                    if pstate == 1 as std::os::raw::c_int {
                        if quoted == 0 {
                            entry_pos = (entry_pos as std::os::raw::c_ulong).wrapping_sub(spaces)
                                as size_t as size_t
                        } /* ROW_NOT_BEGUN */
                        if (*p.as_deref().unwrap()).options as std::os::raw::c_int
                            & 8 as std::os::raw::c_int
                        ...
                        entry_pos = quoted as size_t;
                        if cb2.is_some() {
                            cb2.expect("non-null function pointer")(c as std::os::raw::c_int, data);
                        }
                        pstate = 0 as std::os::raw::c_int;
                        spaces = 0 as std::os::raw::c_int as size_t;
                        quoted = spaces as std::os::raw::c_int;
                        entry_pos = quoted as size_t
                    } else if (*p.as_deref().unwrap()).options as std::os::raw::c_int
            ...
            3 => {
                ...
                if c as std::os::raw::c_int == delim as std::os::raw::c_int {
                    /* Comma */
                    entry_pos = (entry_pos as std::os::raw::c_ulong).wrapping_sub(
                        spaces.wrapping_add(1 as std::os::raw::c_int as std::os::raw::c_ulong),
                    ) as size_t as size_t; /* get rid of spaces and original quote */
                    if quoted == 0 {
                        entry_pos = (entry_pos as std::os::raw::c_ulong).wrapping_sub(spaces)
                            as size_t as size_t
                    }
                ...
                } != 0
                {
                    ...
                    *(*p.as_deref().unwrap()).entry_buf.offset(fresh8 as isize) = c;
                        ...
                        != 0
                    {
                        ...
                        (*p.as_deref_mut().unwrap()).spaces = spaces;
                        (*p.as_deref_mut().unwrap()).entry_pos = entry_pos;
                        return pos.wrapping_sub(1 as std::os::raw::c_int as std::os::raw::c_ulong);
                    }
                    pstate = 2 as std::os::raw::c_int;
                    spaces = 0 as std::os::raw::c_int as size_t;
...
pub unsafe extern "C" fn csv_write2(
    ...
    if dest.is_null() {
        ();
        dest_size = 0 as std::os::raw::c_int as size_t
    }
