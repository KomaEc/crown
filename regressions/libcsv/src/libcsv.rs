...
pub unsafe extern "C" fn csv_init(
    ...
    (*p.as_deref_mut().unwrap()).free_func= Some(free as unsafe extern "C" fn(*mut libc::c_void) -> ());
    return 0 as libc::c_int;
}
#[no_mangle]
...
pub unsafe extern "C" fn csv_fini(
    ...
    if pstate == 2 as libc::c_int && (*p.as_deref().unwrap()).quoted != 0
    ...
    match pstate {
        3 => {
            (*p.as_deref_mut().unwrap()).entry_pos= ((*p.as_deref().unwrap()).entry_pos as libc::c_ulong)
                .wrapping_sub(
                    (*p.as_deref().unwrap()).spaces.wrapping_add(1 as libc::c_int as libc::c_ulong),
                ) as size_t as size_t;
    ...
    match current_block_26 {
        5901124769980541461 => {
            ...
            if (*p.as_deref().unwrap()).options as libc::c_int & 8 as libc::c_int != 0 {
                *(*p.as_deref().unwrap()).entry_buf
                    .offset(entry_pos as isize) = '\0' as i32 as libc::c_uchar;
            }
            if cb1.is_some() && (*p.as_deref().unwrap()).options as libc::c_int & 16 as libc::c_int != 0
                && quoted == 0 && entry_pos == 0 as libc::c_int as libc::c_ulong
            {
                ...
                cb1
                    .expect(
                        "non-null function pointer",
                    )((*p.as_deref().unwrap()).entry_buf as *mut libc::c_void, entry_pos, data);
            }
    ...
    (*p.as_deref_mut().unwrap()).status= 0 as libc::c_int; (*p.as_deref_mut().unwrap()).entry_pos= (*p.as_deref().unwrap()).status as size_t; (*p.as_deref_mut().unwrap()).quoted= (*p.as_deref().unwrap()).entry_pos as libc::c_int; (*p.as_deref_mut().unwrap()).spaces= (*p.as_deref().unwrap()).quoted as size_t;
    (*p.as_deref_mut().unwrap()).pstate= 0 as libc::c_int;
    return 0 as libc::c_int;
}
#[no_mangle]
...
pub unsafe extern "C" fn csv_parse(
    ...
    let mut is_space: Option::<unsafe extern "C" fn(libc::c_uchar) -> libc::c_int> = (*p.as_deref().unwrap()).is_space;
    let mut is_term: Option::<unsafe extern "C" fn(libc::c_uchar) -> libc::c_int> = (*p.as_deref().unwrap()).is_term;
    let mut quoted = (*p.as_deref().unwrap()).quoted;
    let mut pstate = (*p.as_deref().unwrap()).pstate;
    let mut spaces = (*p.as_deref().unwrap()).spaces;
    let mut entry_pos = (*p.as_deref().unwrap()).entry_pos;
    ...
    while pos < len {
        if entry_pos
            == (if (*p.as_deref().unwrap()).options as libc::c_int & 8 as libc::c_int != 0 {
                (*p.as_deref().unwrap()).entry_size.wrapping_sub(1 as libc::c_int as libc::c_ulong)
            } else {
            ...
            0 | 1 => {
                ...
                } != 0
                {
                    if pstate == 1 as libc::c_int {
                        ...
                        quoted= spaces as libc::c_int;
                        entry_pos= quoted as size_t;
                    } else if (*p.as_deref().unwrap()).options as libc::c_int & 2 as libc::c_int != 0 {
                        if cb2.is_some() {
                            cb2
                                .expect(
                                    "non-null function pointer",
...
pub unsafe extern "C" fn csv_fwrite(
    ...
    return csv_fwrite2(fp.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), src, src_size, 0x22 as libc::c_int as libc::c_uchar);
}
#[no_mangle]
