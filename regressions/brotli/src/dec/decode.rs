...
unsafe extern "C" fn BitMask(mut n: uint32_t) -> uint32_t {
    if 0 != 0 || 0 as libc::c_int != 0 {
        ...
        return crate::src::dec::decode::kBrotliBitMask[n as usize]
    };
}
#[inline(always)]
unsafe extern "C" fn BrotliBitReaderSaveState(
    mut from: *mut crate::src::dec::bit_reader::BrotliBitReader,
...
#[inline(always)]
unsafe extern "C" fn BrotliGetBitsUnmasked(mut br: *mut crate::src::dec::bit_reader::BrotliBitReader) -> uint64_t {
...
unsafe extern "C" fn DecodeSymbol(
    ...
    if (*table).bits as libc::c_uint > 8 as libc::c_uint {
        ...
        BrotliDropBits(br.as_mut(), 8 as libc::c_uint);
        table= table
            .offset(
    ...
    BrotliDropBits(br.as_mut(), (*table).bits as uint32_t);
    return (*table).value as uint32_t;
}
#[inline(always)]
unsafe extern "C" fn ReadSymbol(
    mut table: *const HuffmanCode,
    mut br: *mut /* owning */ crate::src::dec::bit_reader::BrotliBitReader,
) -> uint32_t {
    return DecodeSymbol(BrotliGet16BitsUnmasked(br.as_mut()), table, br);
}
#[inline(never)]
unsafe extern "C" fn SafeDecodeSymbol(
    ...
    BrotliDropBits(br.as_mut(), (8 as libc::c_uint).wrapping_add((*table).bits as libc::c_uint));
    *result= (*table).value as uint32_t;
    return 1 as libc::c_int;
}
#[inline(always)]
unsafe extern "C" fn SafeReadSymbol(
    ...
    let mut val: uint32_t = 0;
    if (BrotliSafeGetBits(br.as_mut(), 15 as libc::c_int as uint32_t, Some(&mut val)) != 0)
        as libc::c_int as libc::c_long != 0
    {
        *result.as_deref_mut().unwrap()= DecodeSymbol(val, table, br);
        return 1 as libc::c_int;
    }
    return SafeDecodeSymbol(table, br, result.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()));
}
#[inline(always)]
unsafe extern "C" fn PreloadSymbol(
    ...
    table= table.offset(BrotliGetBits(br.as_mut(), 8 as libc::c_uint) as isize);
...
unsafe extern "C" fn ReadPreloadedSymbol(
    ...
    if ((*bits.as_deref().unwrap()) > 8 as libc::c_uint) as libc::c_int as libc::c_long != 0 {
        let mut val = BrotliGet16BitsUnmasked(br.as_mut());
        let mut ext = table
        ...
        BrotliDropBits(br.as_mut(), (*ext).bits as uint32_t);
        result= (*ext).value as uint32_t;
    } else {
        BrotliDropBits(br.as_mut(), (*bits.as_deref().unwrap()));
    }
    PreloadSymbol(0 as libc::c_int, table, br, bits.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()), value.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()));
    return result;
}
#[inline(always)]
...
unsafe extern "C" fn ReadCodeLengthCodeLengths(
    ...
    while i < (17 as libc::c_int + 1 as libc::c_int) as libc::c_uint {
        let code_len_idx = crate::src::dec::decode::kCodeLengthCodeOrder[i as usize];
            ...
            as libc::c_int as libc::c_long != 0
        {
            ...
            if crate::src::dec::decode::kCodeLengthPrefixLength[ix as usize] as libc::c_uint > available_bits {
                (*h).sub_loop_counter= i;
                (*h).repeat= num_codes;
                (*h).space= space;
        ...
        v= crate::src::dec::decode::kCodeLengthPrefixValue[ix as usize] as uint32_t;
        BrotliDropBits(br.as_mut(), crate::src::dec::decode::kCodeLengthPrefixLength[ix as usize] as uint32_t);
...
unsafe extern "C" fn ReadBlockLength(
    ...
    nbits= crate::src::dec::decode::_kBrotliPrefixCodeRanges[code as usize].nbits as uint32_t;
    return (crate::src::dec::decode::_kBrotliPrefixCodeRanges[code as usize].offset as libc::c_uint)
        .wrapping_add(BrotliReadBits24(br.as_mut(), nbits));
}
#[inline(always)]
unsafe extern "C" fn SafeReadBlockLength(
    ...
    {
        if SafeReadSymbol(table, br, Some(&mut index)) == 0 {
            return 0 as libc::c_int;
        }
    } else {
    ...
    let mut bits: uint32_t = 0;
    let mut nbits = crate::src::dec::decode::_kBrotliPrefixCodeRanges[index as usize].nbits as uint32_t;
    let mut offset = crate::src::dec::decode::_kBrotliPrefixCodeRanges[index as usize].offset as uint32_t;
...
unsafe extern "C" fn DecodeBlockTypeAndLength(
    ...
    if safe == 0 {
        ...
        if SafeReadSymbol(type_tree, br, Some(&mut block_type)) == 0 {
            return 0 as libc::c_int;
        }
...
unsafe extern "C" fn BrotliEnsureRingBuffer(
    ...
    (*s).ringbuffer= (*s).alloc_func
        ...
        ((*s).new_ringbuffer_size as size_t)
            .wrapping_add(crate::src::dec::decode::kRingBufferWriteAheadSlack as libc::c_ulong),
    ) as *mut uint8_t;
...
unsafe extern "C" fn ReadDistanceInternal(
    ...
    if safe == 0 {
        ...
        BrotliBitReaderSaveState(br, Some(&mut memento));
        if SafeReadSymbol(distance_tree, br, Some(&mut code)) == 0 {
            return 0 as libc::c_int;
        }
    }
...
unsafe extern "C" fn ReadCommandInternal(
    ...
    if safe == 0 {
        ...
        BrotliBitReaderSaveState(br, Some(&mut memento));
        if SafeReadSymbol((*s).htree_command, br, Some(&mut cmd_code)) == 0 {
            return 0 as libc::c_int;
        }
    }
    v= crate::src::dec::decode::kCmdLut[cmd_code as usize];
                                ...
                                == 0
                            {
                                ...
                                if safe == 0 {
                                    *(*s).ringbuffer
                                    ...
                                    if SafeReadSymbol((*s).literal_htree, br, Some(&mut literal)) == 0
                                    {
                                        result= BROTLI_DECODER_NEEDS_MORE_INPUT;
                                        break 'c_1900;
                                    } else {
                        ...
                        loop {
                                ...
                                == 0
                            {
                                ...
                                if safe == 0 {
                                    ...
                                    if SafeReadSymbol(hc, br, Some(&mut literal_0)) == 0 {
                                        result= BROTLI_DECODER_NEEDS_MORE_INPUT;
                                        break 'c_1900;
                                    } else {
