...
unsafe extern "C" fn PrintHelp(mut name: *const libc::c_char, mut error: libc::c_int) {
    let mut media = if error != 0 { crate::src::tools::brotli::stderr } else { crate::src::tools::brotli::stdout };
    fprintf(
        media,
...
unsafe extern "C" fn InitializeBuffers(mut context: Option<&mut Context>) {
    ...
    (*context.as_deref_mut().unwrap()).available_out= crate::src::tools::brotli::kFileBufferSize;
...
unsafe extern "C" fn ProvideInput(mut context: Option<&mut Context>) -> libc::c_int {
    (*context.as_deref_mut().unwrap()).available_in= fread(
        ...
        crate::src::tools::brotli::kFileBufferSize,
...
unsafe extern "C" fn PrintFileProcessingProgress(mut context: *mut Context) {
    ...
    fprintf(crate::src::tools::brotli::stderr, b" -> \0" as *const u8 as *const libc::c_char);
    ...
    loop {
            ...
            == BROTLI_DECODER_RESULT_SUCCESS as libc::c_int as libc::c_uint
        {
            ...
            if (*context.as_deref().unwrap()).verbosity > 0 as libc::c_int {
                fprintf(crate::src::tools::brotli::stderr, b"Decompressed \0" as *const u8 as *const libc::c_char);
                PrintFileProcessingProgress(context.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()));
                fprintf(crate::src::tools::brotli::stderr, b"\n\0" as *const u8 as *const libc::c_char);
            }
            return 1 as libc::c_int;
        } else {
            fprintf(
        ...
        result= crate::src::dec::decode::BrotliDecoderDecompressStream(
            s,
            core::ptr::addr_of_mut!((*context.as_deref_mut().unwrap()).available_in),
            core::ptr::addr_of_mut!((*context.as_deref_mut().unwrap()).next_in),
            core::ptr::addr_of_mut!((*context.as_deref_mut().unwrap()).available_out),
            core::ptr::addr_of_mut!((*context.as_deref_mut().unwrap()).next_out),
            0 as *mut size_t,
        );
    };
}
unsafe extern "C" fn DecompressFiles(mut context: Option<&mut Context>) -> libc::c_int {
    while NextFile(context.as_deref_mut()) != 0 {
        let mut is_ok = 1 as libc::c_int;
        let mut s = crate::src::dec::decode::BrotliDecoderCreateInstance(None, None, 0 as *mut libc::c_void);
        if s.is_null() {();
            fprintf(crate::src::tools::brotli::stderr, b"out of memory\n\0" as *const u8 as *const libc::c_char);
            return 0 as libc::c_int;
        }
        ...
        is_ok= OpenFiles(context.as_deref_mut());
        if is_ok != 0 && (*context.as_deref().unwrap()).current_input_path.is_null()
            && (*context.as_deref().unwrap()).force_overwrite == 0 && isatty(0 as libc::c_int) != 0
        {
            fprintf(
                crate::src::tools::brotli::stderr,
        ...
        if is_ok != 0 {
            is_ok= DecompressFile(context.as_deref_mut(), s);
        }
        crate::src::dec::decode::BrotliDecoderDestroyInstance(s);
        if CloseFiles(context.as_deref_mut(), is_ok) == 0 {
            is_ok= 0 as libc::c_int;
        }
        if is_ok == 0 {
            return 0 as libc::c_int;
...
unsafe extern "C" fn CompressFile(
    ...
    loop {
        ...
        if crate::src::enc::encode::BrotliEncoderIsFinished(s.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut())) != 0 {
            ...
            if (*context.as_deref().unwrap()).verbosity > 0 as libc::c_int {
                fprintf(crate::src::tools::brotli::stderr, b"Compressed \0" as *const u8 as *const libc::c_char);
                PrintFileProcessingProgress(context.as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut()));
                fprintf(crate::src::tools::brotli::stderr, b"\n\0" as *const u8 as *const libc::c_char);
            }
            return 1 as libc::c_int;
...
unsafe extern "C" fn CompressFiles(mut context: Option<&mut Context>) -> libc::c_int {
    while NextFile(context.as_deref_mut()) != 0 {
        let mut is_ok = 1 as libc::c_int;
        ...
        crate::src::enc::encode::BrotliEncoderSetParameter(
            ...
            (*context.as_deref().unwrap()).quality as uint32_t,
        );
        if (*context.as_deref().unwrap()).lgwin > 0 as libc::c_int {
            if (*context.as_deref().unwrap()).lgwin > 24 as libc::c_int {
                crate::src::enc::encode::BrotliEncoderSetParameter(
                    s.as_mut(),
            ...
            crate::src::enc::encode::BrotliEncoderSetParameter(
                s.as_mut(),
                BROTLI_PARAM_LGWIN,
                (*context.as_deref().unwrap()).lgwin as uint32_t,
            );
        } else {
            let mut lgwin = 24 as libc::c_int as uint32_t;
            if (*context.as_deref().unwrap()).input_file_length >= 0 as libc::c_int as libc::c_long {
                lgwin= 10 as libc::c_int as uint32_t;
                while ((1 as libc::c_int as size_t) << lgwin)
                    .wrapping_sub(16 as libc::c_int as libc::c_ulong)
                    < (*context.as_deref().unwrap()).input_file_length as uint64_t
                {
        ...
        if (*context.as_deref().unwrap()).input_file_length > 0 as libc::c_int as libc::c_long {
            let mut size_hint = if (*context.as_deref().unwrap()).input_file_length
                < ((1 as libc::c_int) << 30 as libc::c_int) as libc::c_long
            {
                (*context.as_deref().unwrap()).input_file_length as uint32_t
            } else {
        ...
        is_ok= OpenFiles(context.as_deref_mut());
        if is_ok != 0 && (*context.as_deref().unwrap()).current_output_path.is_null()
            && (*context.as_deref().unwrap()).force_overwrite == 0 && isatty(1 as libc::c_int) != 0
        {
            fprintf(
                crate::src::tools::brotli::stderr,
        ...
        if is_ok != 0 {
            is_ok= CompressFile(context.as_deref_mut(), s.as_mut());
        }
...
unsafe fn main_0(
            ...
            == COMMAND_TEST_INTEGRITY as libc::c_int as libc::c_uint
    {
        if is_ok != 0 {
            ...
            context.buffer= malloc(
                crate::src::tools::brotli::kFileBufferSize.wrapping_mul(2 as libc::c_int as libc::c_ulong),
            ) as *mut uint8_t;
            if context.modified_path.is_null() || context.buffer.is_null() {
                ...
                context.output= context.buffer.offset(crate::src::tools::brotli::kFileBufferSize as isize);
            }
        }
    }
    if is_ok == 0 {
    ...
    match  command as libc::c_uint {
        ...
        0 => {
            is_ok= CompressFiles(Some(&mut context));
        }
        1 | 4 => {
            is_ok= DecompressFiles(Some(&mut context));
        }
        2 | 3 | _ => {
