...
pub unsafe extern "C" fn quadtree_node_with_bounds(
    ...
    node= quadtree_node_new().as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut());
    if node.is_null() {();
        return 0 as *mut quadtree_node_t;
    }
    (*node).bounds= crate::src::src::bounds::quadtree_bounds_new().as_deref_mut().map(|r| r as *mut _).unwrap_or(std::ptr::null_mut());
