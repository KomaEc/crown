 ...
 **/
unsafe extern "C" fn pnm_fget_char(mut stream: *mut FILE) -> std::os::raw::c_int {
    let mut ch: std::os::raw::c_int = getc(stream);
    /* skip the comments */
    if ch == '#' as i32 {
        loop  {
            ch = getc(stream);
 ...
 **/
unsafe extern "C" fn pnm_fscan_uint(mut stream: *mut FILE,
                                    mut value: *mut std::os::raw::c_uint)
 -> std::os::raw::c_int {
    ...
    /* read the value */
    *value = 0 as std::os::raw::c_int as std::os::raw::c_uint;
    loop  {
        tmp =
            (*value).wrapping_mul(10 as std::os::raw::c_int as
...
pub unsafe extern "C" fn pnm_fget_values(mut pnm_ptr: *const pnm_struct,
 ...
 -> std::os::raw::c_int {
    ...
    let mut ch: std::os::raw::c_int = 0;
    let mut ch8: std::os::raw::c_int = 0;
    let mut ch16: std::os::raw::c_int = 0;
    let mut ch24: std::os::raw::c_int = 0;
        ...
        2 | 3 => {
            ...
            while i < num_samples {
                if pnm_fscan_uint(stream,
                                  &mut *sample_values.offset(i as isize)) !=
                       1 as std::os::raw::c_int {
                    break ;
                }
                i = i.wrapping_add(1)
