 ...
 */
#[no_mangle]
pub unsafe extern "C" fn initGenericErrorDefaultFunc(mut handler:
        ...
        *fresh0 =
            Some(xmlGenericErrorDefaultFunc as
                     unsafe extern "C" fn(_: *mut std::os::raw::c_void,
                         ...
                         -> ())
    } else { let ref mut fresh1 = *__xmlGenericError(); *fresh1 = *handler };
}
/* *
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmlSetGenericErrorFunc(mut ctx: *mut std::os::raw::c_void,
                                                mut handler:
                                                    xmlGenericErrorFunc) {
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmlSetStructuredErrorFunc(mut ctx: *mut std::os::raw::c_void,
                                                   mut handler:
                                                       xmlStructuredErrorFunc) {
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmlParserPrintFileInfo(mut input:
                                                    xmlParserInputPtr) {
    if !input.is_null() {
...
unsafe extern "C" fn xmlParserPrintFileContextInternal(mut input:
    ...
    if input.is_null() || (*input).cur.is_null() { return }
    cur = (*input).cur;
    base = (*input).base;
    ...
    loop 
        ...
        let fresh7 = n;
        n = n.wrapping_add(1);
        if !((fresh7 as std::os::raw::c_ulong) <
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmlParserPrintFileContext(mut input:
                                                       xmlParserInputPtr) {
    xmlParserPrintFileContextInternal(input, *__xmlGenericError(),
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmlReportError(mut err: xmlErrorPtr,
    ...
    if !ctxt.is_null() {
        ...
        channel.expect("non-null function pointer")(data,
                  ...
                  (domain == XML_FROM_PARSER as std::os::raw::c_int ||
                       domain == XML_FROM_SCHEMASV as std::os::raw::c_int ||
                       domain == XML_FROM_SCHEMASP as std::os::raw::c_int ||
                       domain == XML_FROM_DTD as std::os::raw::c_int ||
                       domain == XML_FROM_RELAXNGP as std::os::raw::c_int ||
                       domain == XML_FROM_RELAXNGV as std::os::raw::c_int) {
        channel.expect("non-null function pointer")(data,
                                                    b"Entity: line %d: \x00"
                                                        as *const u8 as
                                                        *const std::os::raw::c_char,
    ...
    match domain {
        ...
        17 => {
            channel.expect("non-null function pointer")(data,
                                                        b"Schemas validity \x00"
                                                            as *const u8 as
                                                            *const std::os::raw::c_char);
        }
        16 => {
            channel.expect("non-null function pointer")(data,
                                                        b"Schemas parser \x00"
                                                            as *const u8 as
                                                            *const std::os::raw::c_char);
        }
        18 => {
            channel.expect("non-null function pointer")(data,
                                                        b"Relax-NG parser \x00"
                                                            as *const u8 as
                                                            *const std::os::raw::c_char);
        }
        19 => {
            channel.expect("non-null function pointer")(data,
                                                        b"Relax-NG validity \x00"
                                                            as *const u8 as
                                                            *const std::os::raw::c_char);
        }
        20 => {
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn __xmlSimpleError(mut domain: std::os::raw::c_int,
                                          mut code: std::os::raw::c_int,
                                          mut node: xmlNodePtr,
                                          mut msg: *const std::os::raw::c_char,
                                          mut extra: *const std::os::raw::c_char) {
    if code == XML_ERR_NO_MEMORY as std::os::raw::c_int {
        if !extra.is_null() {
            __xmlRaiseError(None, None, 0 as *mut std::os::raw::c_void,
                            0 as *mut std::os::raw::c_void, node as *mut std::os::raw::c_void,
                            domain, XML_ERR_NO_MEMORY as std::os::raw::c_int,
                            XML_ERR_FATAL, 0 as *const std::os::raw::c_char,
                            0 as std::os::raw::c_int, extra, 0 as *const std::os::raw::c_char,
                            0 as *const std::os::raw::c_char, 0 as std::os::raw::c_int,
                            0 as std::os::raw::c_int,
                            b"Memory allocation failed : %s\n\x00" as
                                *const u8 as *const std::os::raw::c_char, extra);
        } else {
            __xmlRaiseError(None, None, 0 as *mut std::os::raw::c_void,
                            0 as *mut std::os::raw::c_void, node as *mut std::os::raw::c_void,
                            domain, XML_ERR_NO_MEMORY as std::os::raw::c_int,
        ...
        __xmlRaiseError(None, None, 0 as *mut std::os::raw::c_void,
                        0 as *mut std::os::raw::c_void, node as *mut std::os::raw::c_void,
                        domain, code, XML_ERR_ERROR, 0 as *const std::os::raw::c_char,
                        0 as std::os::raw::c_int, extra, 0 as *const std::os::raw::c_char,
                        0 as *const std::os::raw::c_char, 0 as std::os::raw::c_int,
...
pub unsafe extern "C" fn xmlResetError(mut err: xmlErrorPtr) {
    if err.is_null() { return }
    if (*err).code == XML_ERR_OK as std::os::raw::c_int { return }
    if !(*err).message.is_null() {
        xmlFree.expect("non-null function pointer")((*err).message as
                                                        *mut std::os::raw::c_void);
    }
    if !(*err).file.is_null() {
        xmlFree.expect("non-null function pointer")((*err).file as
                                                        *mut std::os::raw::c_void);
    }
    if !(*err).str1.is_null() {
        xmlFree.expect("non-null function pointer")((*err).str1 as
                                                        *mut std::os::raw::c_void);
    }
    if !(*err).str2.is_null() {
        xmlFree.expect("non-null function pointer")((*err).str2 as
                                                        *mut std::os::raw::c_void);
    }
    if !(*err).str3.is_null() {
        xmlFree.expect("non-null function pointer")((*err).str3 as
                                                        *mut std::os::raw::c_void);
    }
    memset(err as *mut std::os::raw::c_void, 0 as std::os::raw::c_int,
           ::std::mem::size_of::<xmlError>() as std::os::raw::c_ulong);
    (*err).code = XML_ERR_OK as std::os::raw::c_int;
}
/* *
 * xmlResetLastError:
 *
...
pub unsafe extern "C" fn xmlResetLastError() {
    if (*__xmlLastError()).code == XML_ERR_OK as std::os::raw::c_int { return }
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmlCtxtGetLastError(mut ctx: *mut std::os::raw::c_void)
 -> xmlErrorPtr {
    let mut ctxt: xmlParserCtxtPtr = ctx as xmlParserCtxtPtr;
    if ctxt.is_null() { return 0 as xmlErrorPtr }
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmlCtxtResetLastError(mut ctx: *mut std::os::raw::c_void) {
    let mut ctxt: xmlParserCtxtPtr = ctx as xmlParserCtxtPtr;
    if ctxt.is_null() { return }
    (*ctxt).errNo = XML_ERR_OK as std::os::raw::c_int;
    if (*ctxt).lastError.code == XML_ERR_OK as std::os::raw::c_int { return }
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmlCopyError(mut from: xmlErrorPtr,
                                      mut to: xmlErrorPtr) -> std::os::raw::c_int {
    let mut message: *mut std::os::raw::c_char = 0 as *mut std::os::raw::c_char;
    let mut file: *mut std::os::raw::c_char = 0 as *mut std::os::raw::c_char;
    let mut str1: *mut std::os::raw::c_char = 0 as *mut std::os::raw::c_char;
    let mut str2: *mut std::os::raw::c_char = 0 as *mut std::os::raw::c_char;
    let mut str3: *mut std::os::raw::c_char = 0 as *mut std::os::raw::c_char;
    if from.is_null() || to.is_null() { return -(1 as std::os::raw::c_int) }
    message = xmlStrdup((*from).message as *mut xmlChar) as *mut std::os::raw::c_char;
    file = xmlStrdup((*from).file as *mut xmlChar) as *mut std::os::raw::c_char;
    str1 = xmlStrdup((*from).str1 as *mut xmlChar) as *mut std::os::raw::c_char;
    str2 = xmlStrdup((*from).str2 as *mut xmlChar) as *mut std::os::raw::c_char;
    str3 = xmlStrdup((*from).str3 as *mut xmlChar) as *mut std::os::raw::c_char;
    if !(*to).message.is_null() {
        xmlFree.expect("non-null function pointer")((*to).message as
                                                        *mut std::os::raw::c_void);
    }
    if !(*to).file.is_null() {
        xmlFree.expect("non-null function pointer")((*to).file as
                                                        *mut std::os::raw::c_void);
    }
    if !(*to).str1.is_null() {
        xmlFree.expect("non-null function pointer")((*to).str1 as
                                                        *mut std::os::raw::c_void);
    }
    if !(*to).str2.is_null() {
        xmlFree.expect("non-null function pointer")((*to).str2 as
                                                        *mut std::os::raw::c_void);
    }
    if !(*to).str3.is_null() {
        xmlFree.expect("non-null function pointer")((*to).str3 as
                                                        *mut std::os::raw::c_void);
    }
