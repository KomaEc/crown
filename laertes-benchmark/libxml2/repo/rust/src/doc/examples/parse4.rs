 ...
 * copy: see Copyright for the status of this software.
 */
static mut desc: *mut FILE = 0 as *const FILE as *mut FILE;
/* *
 * readPacket:
 ...
 */
unsafe extern "C" fn readPacket(mut mem: *mut std::os::raw::c_char,
                                mut size: std::os::raw::c_int) -> std::os::raw::c_int {
    let mut res: std::os::raw::c_int = 0;
    res =
        fread(mem as *mut std::os::raw::c_void, 1 as std::os::raw::c_int as size_t,
              size as size_t, desc) as std::os::raw::c_int;
    return res;
}
/* *
 * example4Func:
 ...
 */
unsafe extern "C" fn example4Func(mut filename: *const std::os::raw::c_char) {
    let mut ctxt: xmlParserCtxtPtr =
        0 as *mut xmlParserCtxt; /* the resulting document tree */
    let mut chars: [std::os::raw::c_char; 4] = [0; 4];
     ...
     */
    res = readPacket(chars.as_mut_ptr(), 4 as std::os::raw::c_int);
     ...
     */
    ctxt =
        xmlCreatePushParserCtxt(0 as xmlSAXHandlerPtr, 0 as *mut std::os::raw::c_void,
                                chars.as_mut_ptr(), res, filename);
    if ctxt.is_null() {
        fprintf(stderr,
     ...
     */
        ...
        xmlParseChunk(ctxt, chars.as_mut_ptr(), res, 0 as std::os::raw::c_int);
    }
    /*
...
unsafe fn main_0(mut argc: std::os::raw::c_int, mut argv: *mut *mut std::os::raw::c_char)
 -> std::os::raw::c_int {
    ...
    if !desc.is_null() {
        example4Func(*argv.offset(1 as std::os::raw::c_int as isize));
        fclose(desc);
    } else {
        fprintf(stderr,
                b"Failed to parse %s\n\x00" as *const u8 as
