 ...
 */
unsafe extern "C" fn extractFile(mut filename: *const std::os::raw::c_char,
                                 mut pattern: *const xmlChar) -> xmlDocPtr {
	 ...
	 */
        if xmlTextReaderPreservePattern(reader, pattern,
                                        0 as *mut *const xmlChar) <
               0 as std::os::raw::c_int {
            fprintf(stderr,
                    b"%s : failed add preserve pattern %s\n\x00" as *const u8
                        as *const std::os::raw::c_char, filename,
                    pattern as *const std::os::raw::c_char);
        }
        /*
...
unsafe fn main_0(mut argc: std::os::raw::c_int, mut argv: *mut *mut std::os::raw::c_char)
 -> std::os::raw::c_int {
    let mut filename: *const std::os::raw::c_char =
        b"test3.xml\x00" as *const u8 as *const std::os::raw::c_char;
    let mut pattern: *const std::os::raw::c_char =
        b"preserved\x00" as *const u8 as *const std::os::raw::c_char;
    let mut doc: xmlDocPtr = 0 as *mut xmlDoc;
    if argc == 3 as std::os::raw::c_int {
        filename = *argv.offset(1 as std::os::raw::c_int as isize);
        pattern = *argv.offset(2 as std::os::raw::c_int as isize)
    }
    /*
    ...
    doc = extractFile(filename, pattern as *const xmlChar);
    if !doc.is_null() {
        /*
	 * ouptut the result.
	 */
