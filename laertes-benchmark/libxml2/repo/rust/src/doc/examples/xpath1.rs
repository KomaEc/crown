...
unsafe fn main_0(mut argc: std::os::raw::c_int, mut argv: *mut *mut std::os::raw::c_char)
 -> std::os::raw::c_int {
    ...
    /* Do the main job */
    if execute_xpath_expression(*argv.offset(1 as std::os::raw::c_int as isize),
                                *argv.offset(2 as std::os::raw::c_int as isize) as
                                    *mut xmlChar,
...
unsafe extern "C" fn usage(mut name: *const std::os::raw::c_char) {
    if !name.is_null() {
        ...
        __assert_fail(b"name\x00" as *const u8 as *const std::os::raw::c_char,
                      b"xpath1.c\x00" as *const u8 as *const std::os::raw::c_char,
                      66 as std::os::raw::c_int as std::os::raw::c_uint,
                      (*::std::mem::transmute::<&[u8; 25],
                                                &[std::os::raw::c_char; 25]>(b"void usage(const char *)\x00")).as_ptr());
    }
    fprintf(stderr,
            b"Usage: %s <xml-file> <xpath-expr> [<known-ns-list>]\n\x00" as
                *const u8 as *const std::os::raw::c_char, name);
    fprintf(stderr,
            b"where <known-ns-list> is a list of known namespaces\n\x00" as
                *const u8 as *const std::os::raw::c_char);
    fprintf(stderr,
            b"in \"<prefix1>=<href1> <prefix2>=href2> ...\" format\n\x00" as
                *const u8 as *const std::os::raw::c_char);
}
/* *
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn execute_xpath_expression(mut filename:
                                                      *const std::os::raw::c_char,
                                                  mut xpathExpr:
                                                      *const xmlChar,
                                                  mut nsList: *const xmlChar)
 -> std::os::raw::c_int {
    let mut doc: xmlDocPtr = 0 as *mut xmlDoc;
    let mut xpathCtx: xmlXPathContextPtr = 0 as *mut xmlXPathContext;
    let mut xpathObj: xmlXPathObjectPtr = 0 as *mut xmlXPathObject;
    if !filename.is_null() {
    } else {
        __assert_fail(b"filename\x00" as *const u8 as *const std::os::raw::c_char,
                      b"xpath1.c\x00" as *const u8 as *const std::os::raw::c_char,
                      90 as std::os::raw::c_int as std::os::raw::c_uint,
                      (*::std::mem::transmute::<&[u8; 77],
                                                &[std::os::raw::c_char; 77]>(b"int execute_xpath_expression(const char *, const xmlChar *, const xmlChar *)\x00")).as_ptr());
    }
    if !xpathExpr.is_null() {
    } else {
        __assert_fail(b"xpathExpr\x00" as *const u8 as *const std::os::raw::c_char,
                      b"xpath1.c\x00" as *const u8 as *const std::os::raw::c_char,
    ...
    if xpathCtx.is_null() {
        fprintf(stderr,
                b"Error: unable to create new XPath context\n\x00" as
                    *const u8 as *const std::os::raw::c_char);
        xmlFreeDoc(doc);
        return -(1 as std::os::raw::c_int)
    }
    ...
    if !nsList.is_null() &&
           register_namespaces(xpathCtx, nsList) < 0 as std::os::raw::c_int {
        fprintf(stderr,
                b"Error: failed to register namespaces list \"%s\"\n\x00" as
                    *const u8 as *const std::os::raw::c_char, nsList);
    ...
    if xpathObj.is_null() {
        fprintf(stderr,
                b"Error: unable to evaluate xpath expression \"%s\"\n\x00" as
                    *const u8 as *const std::os::raw::c_char, xpathExpr);
        xmlXPathFreeContext(xpathCtx);
        xmlFreeDoc(doc);
        return -(1 as std::os::raw::c_int)
    }
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn register_namespaces(mut xpathCtx: xmlXPathContextPtr,
                                             mut nsList: *const xmlChar)
 -> std::os::raw::c_int {
    ...
    if !xpathCtx.is_null() {
        ...
        __assert_fail(b"xpathCtx\x00" as *const u8 as *const std::os::raw::c_char,
                      b"xpath1.c\x00" as *const u8 as *const std::os::raw::c_char,
                      153 as std::os::raw::c_int as std::os::raw::c_uint,
                      (*::std::mem::transmute::<&[u8; 61],
                                                &[std::os::raw::c_char; 61]>(b"int register_namespaces(xmlXPathContextPtr, const xmlChar *)\x00")).as_ptr());
    }
    if !nsList.is_null() {
    } else {
        __assert_fail(b"nsList\x00" as *const u8 as *const std::os::raw::c_char,
                      b"xpath1.c\x00" as *const u8 as *const std::os::raw::c_char,
    ...
    if nsListDup.is_null() {
        fprintf(stderr,
                b"Error: unable to strdup namespaces list\n\x00" as *const u8
    ...
    while !next.is_null() {
        /* skip spaces */
        while *next as std::os::raw::c_int == ' ' as i32 { next = next.offset(1) }
        ...
        if next.is_null() {
            fprintf(stderr,
                    b"Error: invalid namespaces list format\n\x00" as
                        *const u8 as *const std::os::raw::c_char);
        ...
        *fresh0 = '\u{0}' as i32 as xmlChar;
        ...
        if xmlXPathRegisterNs(xpathCtx, prefix, href) != 0 as std::os::raw::c_int {
            fprintf(stderr,
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn print_xpath_nodes(mut nodes: xmlNodeSetPtr,
                                           mut output: *mut FILE) {
    ...
    if !output.is_null() {
        ...
        __assert_fail(b"output\x00" as *const u8 as *const std::os::raw::c_char,
                      b"xpath1.c\x00" as *const u8 as *const std::os::raw::c_char,
                      210 as std::os::raw::c_int as std::os::raw::c_uint,
                      (*::std::mem::transmute::<&[u8; 46],
                                                &[std::os::raw::c_char; 46]>(b"void print_xpath_nodes(xmlNodeSetPtr, FILE *)\x00")).as_ptr());
    }
    size = if !nodes.is_null() { (*nodes).nodeNr } else { 0 as std::os::raw::c_int };
    fprintf(output,
            b"Result (%d nodes):\n\x00" as *const u8 as *const std::os::raw::c_char,
            size);
    i = 0 as std::os::raw::c_int;
    while i < size {
        if !(*(*nodes).nodeTab.offset(i as isize)).is_null() {
        } else {
            __assert_fail(b"nodes->nodeTab[i]\x00" as *const u8 as
                              *const std::os::raw::c_char,
                          b"xpath1.c\x00" as *const u8 as *const std::os::raw::c_char,
        ...
        if (**(*nodes).nodeTab.offset(i as isize)).type_0 as std::os::raw::c_uint ==
               XML_NAMESPACE_DECL as std::os::raw::c_int as std::os::raw::c_uint {
            let mut ns: xmlNsPtr = 0 as *mut xmlNs;
            ns = *(*nodes).nodeTab.offset(i as isize) as xmlNsPtr;
            cur = (*ns).next as xmlNodePtr;
            if !(*cur).ns.is_null() {
                fprintf(output,
                        b"= namespace \"%s\"=\"%s\" for node %s:%s\n\x00" as
                            *const u8 as *const std::os::raw::c_char, (*ns).prefix,
                        (*ns).href, (*(*cur).ns).href, (*cur).name);
            } else {
                fprintf(output,
                        b"= namespace \"%s\"=\"%s\" for node %s\n\x00" as
                            *const u8 as *const std::os::raw::c_char, (*ns).prefix,
                        (*ns).href, (*cur).name);
            }
        } else if (**(*nodes).nodeTab.offset(i as isize)).type_0 as
                      std::os::raw::c_uint ==
                      XML_ELEMENT_NODE as std::os::raw::c_int as std::os::raw::c_uint {
            cur = *(*nodes).nodeTab.offset(i as isize);
            if !(*cur).ns.is_null() {
                fprintf(output,
                        b"= element node \"%s:%s\"\n\x00" as *const u8 as
                            *const std::os::raw::c_char, (*(*cur).ns).href,
                        (*cur).name);
            } else {
                fprintf(output,
                        b"= element node \"%s\"\n\x00" as *const u8 as
