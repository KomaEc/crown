 ...
 * copy: see Copyright for the status of this software.
 */
unsafe extern "C" fn processDoc(mut readerPtr: xmlTextReaderPtr) {
    let mut ret: std::os::raw::c_int = 0;
    let mut docPtr: xmlDocPtr = 0 as *mut xmlDoc;
    let mut URL: *const xmlChar = 0 as *const xmlChar;
    ret = xmlTextReaderRead(readerPtr);
    while ret == 1 as std::os::raw::c_int { ret = xmlTextReaderRead(readerPtr) }
    /*
    ...
    printf(b"%s: Processed ok\n\x00" as *const u8 as *const std::os::raw::c_char,
           URL as *const std::os::raw::c_char);
}
unsafe fn main_0(mut argc: std::os::raw::c_int, mut argv: *mut *mut std::os::raw::c_char)
 -> std::os::raw::c_int {
    let mut readerPtr: xmlTextReaderPtr = 0 as *mut xmlTextReader;
     ...
     */
    readerPtr =
        xmlReaderForFile(*argv.offset(1 as std::os::raw::c_int as isize),
                         0 as *const std::os::raw::c_char, 0 as std::os::raw::c_int);
    if readerPtr.is_null() {
        fprintf(stderr,
                b"%s: failed to create reader\n\x00" as *const u8 as
                    *const std::os::raw::c_char,
    ...
    while i < argc {
        xmlReaderNewFile(readerPtr, *argv.offset(i as isize),
                         0 as *const std::os::raw::c_char, 0 as std::os::raw::c_int);
        if readerPtr.is_null() {
            fprintf(stderr,
                    b"%s: failed to create reader\n\x00" as *const u8 as
     ...
     */
    docPtr = xmlTextReaderCurrentDoc(readerPtr);
    if !docPtr.is_null() { xmlFreeDoc(docPtr); }
    /*
     * Clean up the reader.
     */
