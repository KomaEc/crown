...
impl<N, E> Graph<N, E> {
    ...
    pub fn edges_enumerated(&self) -> impl Iterator<Item = (EdgeIndex, &Edge<E>)> {
        self.edges.iter().enumerate().map(|(idx, n)| (EdgeIndex(idx), n))
    }
...
impl<'g, N, E> SCCsConstruction<'g, N, E> {
    ...
    fn walk_vertex(&mut self, v: NodeIndex) {
        ...
        for u in self.graph.successor_nodes(v) {
            ...
            } else if self.on_stack[u.0] { // it's a back edge
                // it's a back edge
...
mod tests {
    ...
    fn test_depth_first_search() {
        ...
        assert_eq!(
            g.depth_traverse(NodeIndex(1), Direction::Outgoing).collect::<Vec<_>>(), 
