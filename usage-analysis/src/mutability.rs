...
// usage public
pub struct CrateResults<'tcx, 'a>(usage::CrateResults<'tcx, 'a, MutabilityAnalysis<'a>>);
...
impl<'tcx, 'a> CrateResults<'tcx, 'a> {
    pub fn collect(
        ...
        ownership: &'a dyn AnalysisResults,
    ) -> Self {
...
impl<'tcx, 'a> AnalysisResults for CrateResults<'tcx, 'a> {
...
impl usage::AnalysisResult for Mutability {
    const DEFAULT: Self = Mutability::Immutable;
}

#[derive(Clone)]
struct MutabilityAnalysis<'o> {
    ownership: &'o dyn AnalysisResults,
}

impl usage::Analysis for MutabilityAnalysis<'_> {
